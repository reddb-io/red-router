...
describe("grokBuildConfig", () => {
  it("creates independent main and per-type subagent model slots", () => {
    ...
    expect(parsed.default).toBe("red-router");
    expect(parsed.model).toMatchObject({
      model: "cx/gpt-5.6-sol",
      base_url: "http://127.0.0.1:25050/v1",
      context_window: 400000,
    });
    expect(parsed.subagentMappings).toMatchObject({
      "general-purpose": "red-router-general-purpose",
      explore: "red-router-explore",
      plan: "grok-4.5",
    });
  ...
  it("is idempotent and updates owned slots without duplicate sections", () => {
    ...
    expect(result.match(/^\[model\.red-router\]$/gm)).toHaveLength(1);
    expect(result.match(/^\[model\.red-router-general-purpose\]$/gm)).toHaveLength(1);
    expect(result.match(/^\[model\.red-router-explore\]$/gm)).toHaveLength(1);
    expect(result.match(/^# red-router-prev-subagent-explore/gm)).toHaveLength(1);
  ...
  it("blank override restores previous subagent mapping and removes owned slot", () => {
    ...
    expect(result).not.toContain("[model.red-router-explore]");
    expect(parsed.subagentMappings["general-purpose"]).toBe("red-router-general-purpose");
  });
  ...
  it("reset restores previous default and all previous subagent mappings", () => {
    ...
    expect(reset).not.toContain("[model.red-router-");
    expect(reset).not.toContain("red-router-prev-");
  ...
  it("removes mappings that were originally unset", () => {
    ...
    expect(parseGrokBuildConfig(applied).subagentMappings.plan).toBe("red-router-plan");
  ...
  it("legacy callers without subagentModels leave existing overrides untouched", () => {
    ...
    expect(parsed.subagentMappings.explore).toBe("red-router-explore");
  ...
  it("returns stable slot names only for supported subagent types", () => {
    expect(getGrokSubagentSlot("general-purpose")).toBe("red-router-general-purpose");
    expect(getGrokSubagentSlot("explore")).toBe("red-router-explore");
    expect(getGrokSubagentSlot("plan")).toBe("red-router-plan");
