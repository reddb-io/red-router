...
test("getTokenLimit: does not force 1M onto non-hyperagent providers serving the same model ids", () => {
  // Restored Windsurf has a provider-specific 200K pin for this variant. That
  // pin must beat the shared 1M base-model catalog and Hyperagent fallback.
  assert.equal(getTokenLimit("windsurf", "claude-opus-4.7-max"), 200_000);
