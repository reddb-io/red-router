...
// path, so x-rr-real-ip arrives straight from the client and a remote caller can claim to
...
describe("peer header trust", () => {
  ...
  afterEach(() => {
    ...
    delete process.env.REDROUTER_PEER_TOKEN;
  });
...
describe("login limiter client IP", () => {
  ...
  it("buckets spoofed peer IPs together so lockout cannot be rotated away", () => {
    const first = getClientIp(request("/api/auth/login", { "x-rr-real-ip": "1.1.1.1" }));
    const second = getClientIp(request("/api/auth/login", { "x-rr-real-ip": "2.2.2.2" }));
  ...
  it("still honours TRUST_PROXY for operators fronting red-router with a reverse proxy", () => {
