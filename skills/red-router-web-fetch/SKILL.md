---
name: red-router-web-fetch
description: Fetch URL → markdown / text / HTML via RedRouter /v1/web/fetch using Ollama Cloud / Firecrawl / Jina Reader / Tavily Extract / Exa Contents. Use when the user wants to scrape a webpage, extract URL content, read article, or convert a URL to markdown.
---
...
```bash
curl $REDROUTER_URL/v1/models/web | jq '.data[] | select(.kind=="webFetch") | .id'
# Per-provider params
curl "$REDROUTER_URL/v1/models/info?id=firecrawl/fetch"
```
...
`POST $REDROUTER_URL/v1/web/fetch`
...
### Jina Reader
```bash
curl -X POST $REDROUTER_URL/v1/web/fetch \
  -H "Authorization: Bearer $REDROUTER_KEY" \
  -H "Content-Type: application/json" \
  -d '{"model":"jina-reader","url":"https://github.com/reddb-io/red-router","format":"markdown"}'
```
...
```js
const r = await fetch(`${process.env.REDROUTER_URL}/v1/web/fetch`, {
  method: "POST",
