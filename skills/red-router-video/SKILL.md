---
name: red-router-video
description: Generate videos via RedRouter /v1/videos/generations using xAI Grok Imagine (grok-imagine-video). Async job flow - submit, poll request_id until done, download MP4. Use when the user wants to create, generate, or render a video, text-to-video (txt2vid), or image-to-video.
---

# RedRouter — Video Generation (xAI Grok Imagine)

Requires `REDROUTER_URL` (and `REDROUTER_KEY` if auth enabled). See https://raw.githubusercontent.com/ghcr.io/reddb-io/red-router/refs/heads/master/skills/red-router/SKILL.md for setup.

Requires a connected **xAI account** in the RedRouter dashboard — either **Grok Build OAuth** (SuperGrok / X Premium+ subscription sign-in) or a direct **xAI API key** from console.x.ai. The two are separate auth types with separate billing; the dashboard shows which one each connection uses.
...
```bash
curl -X POST "$REDROUTER_URL/v1/videos/generations" \
  -H "Authorization: Bearer $REDROUTER_KEY" \
...
# → {"request_id":"abc123"}   (response header x-red-router-connection-id: <id>)
```

Poll until done (echo the connection header back so the same account polls the job):

```bash
curl "$REDROUTER_URL/v1/videos/abc123" \
  -H "Authorization: Bearer $REDROUTER_KEY" \
...
- Jobs are **account-bound** upstream: poll with the same connection that created the job (`x-connection-id` header, value from the create response's `x-red-router-connection-id`).
