...
// Identifiers for killAllAppProcesses - only kill red-router/red-router specifically
const PROCESS_IDENTIFIERS = [
  'red-router',   // Legacy package name - existing installs
  'red-router' // Current package name (@reddb-io/red-router)
];
    ...
    try {
      ...
      if (platform === "win32") {
        ...
        try {
          ...
          const lines = output.split("\n").slice(1).filter(l => l.trim());
          lines.forEach(line => {
            // Whitelist: real node process running red-router/cli.js, or next-server.
            // Avoids killing editors/grep/strace/cursor that just have "red-router" in cmdline.
            const cmd = line.toLowerCase();
            const isAppProcess =
              (cmd.includes("node") && cmd.includes("red-router") && (cmd.includes("cli.js") || cmd.includes("\\red-router") || cmd.includes("/red-router")))
        ...
        try {
          ...
          lines.forEach(line => {
            // Whitelist: real node process running red-router/cli.js, or next-server.
            // Avoids killing grep/strace/editors/cursor that incidentally match "red-router".
            const cmd = line.toLowerCase();
            const isAppProcess =
              (cmd.includes("node") && cmd.includes("red-router") && (cmd.includes("cli.js") || cmd.includes("/red-router")))
              || cmd.includes("next-server");
            if (isAppProcess) {
              const parts = line.trim().split(/\s+/);
              const pid = parts[1];
      ...
      while (true) {
        ...
        if (choice === "update") {
          ...
          if (process.platform === "darwin") {
            ...
            console.log(`🔔 RedRouter is running in tray (PID: ${process.pid})`);
            console.log(`   Server: http://${displayHost}:${port}`);
          ...
          console.log(`🔔 RedRouter is now running in background (PID: ${bgProcess.pid})`);
          console.log(`   Server: http://${displayHost}:${port}`);
          console.log(`\n💡 You can close this terminal. Right-click tray icon to quit.\n`);
