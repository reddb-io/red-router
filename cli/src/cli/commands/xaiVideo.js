/**
 * `red-router xai video` — generate a Grok Imagine video through the local
 * red-router gateway and save the result as an MP4 file.
 *
...
const HELP = `
Usage: red-router xai video --prompt "..." [options]

Generate a Grok Imagine video via your local red-router gateway
...
Options:
  ...
  --api-key <key>         red-router API key (or env RED_ROUTER_API_KEY)
...
function parseArgs(argv) {
  const opts = {
    ...
    port: DEFAULT_PORT,
    host: DEFAULT_HOST,
    apiKey: process.env.RED_ROUTER_API_KEY || null,
...
async function run(argv) {
  ...
  try {
    ...
    const connectionId = create.headers["x-red-router-connection-id"] || null;
