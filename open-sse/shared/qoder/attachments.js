 ...
 * data-URIs instead, which red-router previously forwarded verbatim — 10MB
...
export function buildMultipartFile(buffer, { fieldName = "file", fileName, mediaType } = {}) {
  const boundary = `----red-routerQoder${Date.now().toString(16)}${Math.random().toString(16).slice(2)}`;
