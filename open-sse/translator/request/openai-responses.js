...
export function openaiResponsesToOpenAIRequest(model, body, stream, credentials) {
  ...
  // whole request (CodeBuddy 11155 "the reasoning content from the previous turn
  // must be passed back in thinking mode") whenever tools are declared. Reasoning
  // is attached here rather than at creation time so that a reasoning item which
  // arrives after the message — Codex emits the message first — still lands on the
  // turn it belongs to.
  ...
  for (const item of inputItems) {
    ...
    if (itemType === RESPONSES_ITEM.MESSAGE) {
      ...
      // here and carries its images in a user turn (#4058).
      const { text: outputContent, images: outputImages } = splitToolOutputContent(item.output);
      ...
      if (outputCallId) {
        result.messages.push({
          ...
          content: outputContent || (outputImages.length > 0 ? "[tool returned images - see the next message]" : "")
        });
        if (outputImages.length > 0) {
          result.messages.push({
            role: ROLE.USER,
            content: [{ type: OPENAI_BLOCK.TEXT, text: "[images returned by the tool result above]" }, ...outputImages]
          });
        }
      }
