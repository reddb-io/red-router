...
export function ensureToolCallIds(body) {
  ...
  // Ids of assistant tool calls that still have no result message. A tool message
  // that arrives WITHOUT `tool_call_id` (clients that lose the field when building
  // the payload) must be paired here, otherwise the message serializes without the
  // key and strict upstreams reject the entire request with HTTP 400
  ...
  for (let i = 0; i < body.messages.length; i++) {
    ...
    if (msg.role === "tool") {
      if (msg.tool_call_id && TOOL_ID_PATTERN.test(msg.tool_call_id)) {
        ...
        // Missing id: pair it with the oldest unanswered tool call; if there is
        // none, mint a deterministic id so the field is never absent.
