...
//
// Ported 1:1 from the legacy fork (open-sse/config/decisionSignals.js @ c66f917c).
...
export const HARNESS_BLOCK_PATTERNS: RegExp[] = [
...
export const HOUSEKEEPING_SENTINELS: string[] = [
...
export const PLAN_MODE_SENTINELS: string[] = [
  "Plan mode is active",
...
export const EXPLICIT_THINK_PATTERN =
  /\b(ultrathink|think (?:really |very )?hard(?:er)?|think deeply|think carefully|pense (?:bem|com cuidado|profundamente)|raciocine com cuidado)\b/i;
...
const phrases = (list: string[], flags = "iu") =>
  new RegExp(`(?<![\\p{L}\\p{N}_])(?:${list.join("|")})(?![\\p{L}\\p{N}_])`, flags);
...
export const FEEDBACK_AGREES_PATTERN =
  /^\s*(?:perfeito|isso mesmo|isso a[íi]|exatamente|[óo]timo|excelente|ok,? (?:segue|continua|pode seguir|manda ver)|pode seguir|perfect|exactly|great|lgtm|looks good|nice|ship it|ok,? (?:go on|continue|proceed)|go ahead)(?![\p{L}\p{N}_])/iu;
...
export const FRUSTRATION_MARKERS: { weight: number; pattern: RegExp }[] = [
  {
    weight: 0.4,
    pattern: phrases([
      "porra",
  ...
  },
  {
    ...
    pattern: phrases([
      "de novo",
      "outra vez",
      "pela (?:en[ée]sima|terceira|quarta|quinta) vez",
      "quantas vezes",
      "j[áa] (?:disse|falei)",
      "again",
      "how many times",
      "for the (?:third|fourth|fifth|nth|last) time",
      "i (?:already )?told you",
    ]),
  },
