...
function ensureClientToolsScript() {
  const scriptPath = path.join(os.tmpdir(), "red-router-devin-client-tools.mjs");
  ...
  async execute({ model, body, credentials, signal, log }) {
    ...
    const sseStream = new ReadableStream({
      start(controller) {
        ...
        // modify the filesystem on the host running red-router — only expose locally.
        ...
        sendRpc("initialize", {
          protocolVersion: "0.3",
          clientInfo: { name: "red-router", version: "1.0" },
          capabilities: {},
        });
      },
    });
