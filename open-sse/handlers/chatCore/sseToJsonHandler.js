 ...
 */
export async function handleForcedSSEToJson({ providerResponse, sourceFormat, targetFormat, provider, model, body, stream, translatedBody, finalBody, requestStartTime, connectionId, apiKey, clientRawRequest, onRequestSuccess, customToolNames, toolNameMap, trackDone, appendLog, decision, reqTag, log, errorContext }) {
    ...
    try {
      ...
      if (jsonResponse.status === "failed") {
        return createErrorResult(
          ...
          jsonResponse.error?.message || "Upstream SSE stream failed",
          undefined,
          errorContext
        );
      }
      ...
      // Claude-format clients need a Message body, not chat.completion —
      // Claude Code's non-streaming retry rejects the raw shape ("JSON but
