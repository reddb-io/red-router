...
const HARD_WORDS = /\b(why|design|architect\w*|debug\w*|investigat\w*|refactor\w*|prove|proof|optimi[sz]\w*|concurren\w*|race condition|deadlock|trade-?offs?|root cause|algorithm\w*|migrat\w*|security|vulnerab\w*|performance|analy[sz]\w*|por ?que|arquitetura|investig\w*|otimiz\w*)\b/i;
const EASY_WORDS = /\b(rename|typo|format\w*|reformat|list|show|print|translate|traduz\w*|lint|bump|update the version|add a comment|spell\w*|capitali[sz]e)\b/i;
const CODE_PATTERN = /```|\bfunction\b|=>|\bclass\s+\w+|\bdef\s+\w+\(|\bimport\s+[\w{]/;

const clamp = (n) => Math.max(0, Math.min(1, n));

/**
 * @param {object} signals extractSignals() output
 * @returns {{ score: number, reasons: string[] }}
 */
export function localDeliberation(signals = {}) {
  ...
  if (signals.userFeedback === "rejects" || signals.userFeedback === "corrects") add(0.1, "feedback");
  if (typeof signals.frustration === "number" && signals.frustration > 0) add(0.2 * signals.frustration, "frustration");
  ...
  if (signals.contextTokens > 60_000) add(0.1, "large_context");
...
export function tierMargin(score) {
