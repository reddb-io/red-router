...
//
// Ported 1:1 from the legacy fork (open-sse/decision/modelBriefs.js @ c66f917c).
// Adaptation: `getCapabilitiesForModel(provider, model)` (legacy providers/
// capabilities.js) is resolved through this base's getResolvedModelCapabilities
// (open-sse/services/modelCapabilities.ts) instead — same intent, new API.

import { getResolvedModelCapabilities } from "../services/modelCapabilities.ts";

type JsonRecord = Record<string, unknown>;

export const MODEL_BRIEFS: Record<string, string> = {
  // === Anthropic ===
  "claude-haiku-4.5":
...
function describeCapabilities(provider: string, model: string): string {
  let caps: JsonRecord = {};
  try {
    caps = getResolvedModelCapabilities({ provider, model }) as unknown as JsonRecord;
  } catch {
    return "";
  }
  const bits: string[] = [];
  if (caps.reasoning) bits.push("supports native reasoning");
  if (caps.contextWindow) bits.push(`${Math.round(Number(caps.contextWindow) / 1000)}k context`);
  if (caps.supportsVision || caps.vision) bits.push("reads images");
...
function vendorSuffix(id: string): string | null {
...
function matchSuffix(table: Record<string, string>, id: string): string | null {
