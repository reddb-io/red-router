...
function toolCriteria(tools: { name: string; description?: string }[]): JsonRecord {
  const limit = Math.max(
    80,
    Math.min(MAX_DESCRIPTION_CHARS, Math.floor(QUESTION_CHAR_BUDGET / tools.length))
  );
 ...
 */
export function buildToolQuestions(tools: { name: string; description?: string }[]): {
 ...
 */
export function buildModelQuestions(
  models: string[],
  criteriaFor: (model: string) => string | null,
  { deliberation = true } = {}
): { questions: JsonRecord } {
  const criteria: JsonRecord = {};
 ...
 */
export function shortlistTools(
...
function lastUserText(body: unknown): string {
  ...
  const turns = (b.messages || b.input || b.contents || []) as JsonRecord[];
...
function textOfContent(content: unknown): string {
  ...
  return content.map((part: unknown) => (part as JsonRecord)?.text || "").join(" ");
}

/** Names the conversation already called, so a repeated need survives the cut. */
function toolsUsedIn(body: unknown): Set<string> {
  const names = new Set<string>();
  const b = (body || {}) as JsonRecord;
  for (const turn of (b.messages as JsonRecord[]) || []) {
    const t = turn || {};
    for (const call of (t.tool_calls as JsonRecord[]) || []) {
      const name = ((call?.function as JsonRecord)?.name as string) || (call?.name as string);
      if (name) names.add(name);
    }
    for (const block of (Array.isArray(t.content) ? t.content : []) as JsonRecord[]) {
      if (block?.type === "tool_use" && block.name) names.add(block.name as string);
    }
  }
  return names;
}
