...
//
// Ported 1:1 from the legacy fork (open-sse/decision/decide.js @ c66f917c).
 ...
 */
export function rankByCost(models: string[], priceOf: PriceOf): string[] {
  return models
 ...
 */
export function cheapestWithinBand(
  pick: string,
...
export function resolveModelDecision({
  ...
  const probabilities: Record<string, number> = Object.fromEntries(
    Object.entries((pick.probabilities as Record<string, unknown>) || {}).filter(
      ([model, value]) =>
        models.includes(model) && typeof value === "number" && Number.isFinite(value)
    )
  ) as Record<string, number>;
  const sanitizedPick: Answer = { ...pick, probabilities };
  ...
  const chosen = priceOf
    ? cheapestWithinBand(pick.choice as string, probabilities, priceOf, TIE_BAND, models)
    : (pick.choice as string);
  ...
  const usable: ModelDecision = {
    ...
    ...(chosen !== pick.choice ? { downgradedFrom: pick.choice as string } : {}),
  };

  const leavesWarmCache =
    warmMember &&
  ...
  const hard = Number(deliberation.noul) >= 0.7 || needsDeliberation === true;
 ...
 */
export function winnerStrength(pick: unknown): number {
  const probs = Object.values((pick as Answer)?.probabilities || {}).filter(
...
export type ToolDecisionResult = {
  mode: "passthrough" | "none" | "forced" | "hint";
  reason?: string;
  confidence?: number;
  tool?: string;
};
...
export function resolveToolDecision({
  ...
  const allows = (mode: string) => rank[mode] <= ceiling;
  ...
  if (tools.some((name: unknown) => typeof name !== "string" || !SAFE_NAME.test(name as string))) {
  ...
  const pick = answers?.tool as Answer | undefined;
  const needs = answers?.needs_tool as Answer | undefined;
  if (!pick || pick.type !== "choice" || !needs || needs.type !== "noul") {
    return { mode: "passthrough", reason: "no_usable_answer" };
  }
  if (!tools.includes(pick.choice as string) && pick.choice !== NO_TOOL) {
    return { mode: "passthrough", reason: "unknown_tool" };
  }
  if (!((pick.confidence as number) >= minConfidence)) {
    return { mode: "passthrough", reason: "low_confidence", confidence: pick.confidence as number };
  }
  ...
  if (wantsTool ? Number(needs.noul) < 0.3 : Number(needs.noul) > 0.7) {
    return {
      mode: "passthrough",
      reason: "signals_disagree",
  ...
  if (!wantsTool) {
    ...
    if (allows("none")) return { mode: "none", confidence: pick.confidence as number };
    return {
      mode: "passthrough",
  ...
  const plan = plans.find((p: JsonRecord) => p.name === pick.choice);
  if (plan && plan.kind && plan.kind !== "function") {
    return {
      mode: "passthrough",
      reason: "hosted_tool_selected",
      confidence: pick.confidence as number,
    };
  }
  return { mode: "forced", tool: pick.choice as string, confidence: pick.confidence as number };
}
