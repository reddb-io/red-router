...
import { getClaudeCodeVersion } from "./claudeCodeVersion.js";

const CC_ENTRYPOINT = "sdk-cli";

// Generate the billing header expected from current Claude Code clients. The
// version is read per request so an adopted upstream minimum applies at once.
...
function generateBillingHeader(payload) {
  ...
  return `x-anthropic-billing-header: cc_version=${getClaudeCodeVersion()}.${buildHash}; cc_entrypoint=${CC_ENTRYPOINT}; cch=${cch};`;
}
