...
| `runner-cli`  | `runner-base`         | Добавя `git`, `docker.io`, `docker-compose` и глобалните CLI инструменти: `@openai/codex`, `@anthropic-ai/claude-code`, `droid`, `openclaw`. **Изберете това за агентни работни процеси.**                                                                                                      |
| `runner-web`  | `runner-base`         | Добавя Playwright + браузър Chromium (`--with-deps`) за доставчици на уеб сесии: `gemini-web`, `claude-web`, `claude-turnstile`. **Изберете това, когато използвате тези доставчици** — обикновеният образ дава грешка при заявка без него (вижте бележката за `-web` под „Канали за издания“). |

Изграждане на конкретна цел ръчно:
...
Три аргумента за изграждане контролират ресурсите, използвани от етапа `builder`. Те важат само по време на изграждане —
`OMNIROUTE_MEMORY_MB` (по-долу) е отделна настройка за средата на изпълнение.

| Аргумент за изграждане      | По подразбиране | Ефект                                                                                            |
| --------------------------- | --------------- | ------------------------------------------------------------------------------------------------ |
| `OMNIROUTE_USE_TURBOPACK`   | `1`             | При `0` изгражда с webpack. По-ниска пикова употреба на памет, но по-бавно.                      |
| `OMNIROUTE_BUILD_MEMORY_MB` | `6144`          | Горна граница на heap паметта на V8 (`--max-old-space-size`) за стартирания `next build`.        |
| `OMNIROUTE_BUILD_WORKERS`   | `2`             | Задава `CIRCLE_NODE_TOTAL`; Next извежда `workers = N - 1` за събиране на данните за страниците. |

`OMNIROUTE_BUILD_WORKERS` е настройката, която трябва да увеличите при мощна машина за изграждане, и тази, която трябва да
подозирате, когато изграждане с ограничени ресурси прекъсне **след** `✓ Compiled successfully`. Всеки
работен процес за данните на страниците е отделен процес, както и самият родителски процес `next build`;
възпроизвеждане на реален VPS (issue #7518) измери пиковия RSS на всеки процес на
...
процеса) е съобразена с хостваните от GitHub изпълняващи машини с 16 GB / 4 vCPU, които
...
`3` (→ 2 работни процеса) също не се побра, след като RSS на процес бе измерен
...
изграждането след това бива прекратено чрез SIGKILL от OOM механизма без какъвто и да е текст за грешка — то просто
спира по средата на `Creating an optimized production build`, което изглежда като забиване, а не
като недостиг на памет. Ако хостът за изграждане е с ограничени ресурси, сменете пакетиращия инструмент:
...
процес, като всеки от тях спазва `OMNIROUTE_BUILD_MEMORY_MB` поотделно. Задайте горната граница на
контейнера на приблизително над два пъти тази стойност, а не само веднъж.

Измерено в това дърво (`--target runner-base`, `OMNIROUTE_BUILD_MEMORY_MB=6144`):

| Пакетиращ инструмент | Горна граница на контейнера | Резултат                                                |
| -------------------- | --------------------------- | ------------------------------------------------------- |
| Turbopack            | 8 GiB / 16 GiB              | Прекратен от OOM и при двете, без съобщение             |
| webpack              | 8 GiB                       | Работният процес за изграждане е прекратен чрез SIGKILL |
| webpack              | 12 GiB                      | Успешно, с пикова употреба от 11.1 GiB                  |

### Настройки по подразбиране за средата на изпълнение
...
- Тъй като образът винаги задава тази стойност, собственият резервен механизъм на програмата за стартиране, калибриран спрямо RAM, никога не се прилага под Docker. Увеличете я изрично според работното натоварване (таблицата по-долу). `2048` все още е твърде малко за `/v1/responses` на агент за програмиране.

### Оперативна RAM за агенти за програмиране

Стандартната стойност от 1 GiB за Docker е минималното ниво за таблото и леки чатове, а не размер за продукционна среда. Дългите тела на `POST /v1/responses` (стотици съобщения, десетки инструменти) задържат множество графи в паметта по време на компресиране. Две припокриващи се заявки от по ~3 MiB / ~750k токена са предизвикали прекратяване на V8 при **12 GiB** old-space (`FATAL ERROR: Reached heap limit`), а също така са достигнали OOM ограничението на cgroup от 16 GiB. Вижте [#7849](https://github.com/diegosouzapw/OmniRoute/issues/7849).

Задайте **cgroup `--memory` над размера на heap паметта** — нативните буфери, SQLite и междинните данни при компресиране се намират извън V8.
...
| Две едновременни дълги заявки към `/v1/responses` | `10240`–`12288`                   | ≥12–16 GiB                               | Измерено прекратяване на V8 при heap памет от ~12 GiB                                                                                    |
| Три или повече едновременни дълги контекста       | не ги изпълнявайте в един процес  | изпълнявайте последователно / повече RAM | По подразбиране се допуска 1 тежка заявка в процес на изпълнение; увеличаването на броя без допълнителна RAM отново води до прекратяване |

`omniroute serve` на физическа машина калибрира около 35% от RAM (ограничено в диапазона `[512, 4096]`), когато `OMNIROUTE_MEMORY_MB` **не е зададена**. Docker винаги задава `1024`, така че това калибриране никога не се изпълнява в официалния образ.
