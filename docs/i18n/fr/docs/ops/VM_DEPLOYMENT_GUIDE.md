...
> **Conseil** : Pour une sécurité maximale, limitez les ports 80 et 443 aux adresses IP de Cloudflare uniquement. Consultez la section [Sécurité avancée](#advanced-security).

---

## 2. Installer OmniRoute

### 2.1 Créer le répertoire de configuration
...
### 2.4 Vérifier qu’il est en cours d’exécution
...
Maintenez les délais d’expiration des flux du proxy inverse alignés sur les variables d’environnement de délai d’expiration d’OmniRoute. Si vous augmentez
...
### Restreindre nginx aux adresses IP de Cloudflare
...
Consultez également [TUNNELS_GUIDE.md](./TUNNELS_GUIDE.md) pour obtenir le guide pas à pas du tunnel Cloudflare inclus dans le dépôt. Le worker autonome `omnirouteCloud/` se trouve dans un dépôt complémentaire distinct.
