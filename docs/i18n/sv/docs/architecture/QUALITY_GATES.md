...
## Översikt över grindar (~90 skript)
...
| ------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------- |
| `Build (advisory)`                               | Icke-utkast till kod-PR:er och Mergify-kögrenar; Node 24, `npm-ci-retry`, `check:node-runtime`, `npm run build` med `OMNIROUTE_USE_TURBOPACK=1`; ingen artefaktuppladdning eftersom inget efterföljande kvalitetsjobb använder den | **Rådgivande** (`continue-on-error: true`; ta bort efter en vecka med stabila körningar för release-PR:er) |
| `Docs Gates (fast-path)`                         | Dokumentations-/kod-PR:er; API-dokumentationsreferenser och docs-all                                                                                                                                                               | Ja                                                                                                         |
| `Fast Quality Gates`                             | Kod-PR:er; statiska kontroller, typkontroll, typkontroll av kontrollpanelen, berörda enhetstester                                                                                                                                  | Ja                                                                                                         |
| `Forgotten sibling tests`                        | Kod-PR:er; ändrade moduler spåras till statiska användare och möjliga syskontester; barrel- och dynamiska importvägar rapporteras som rådgivande diagnostik, med hänvisade undantag i tillåtelselistan                             | **Rådgivande**                                                                                             |
| `Vitest (fast-path)`                             | Kod-PR:er; snabb Vitest-svit                                                                                                                                                                                                       | Ja                                                                                                         |
| `Unit Tests fast-path`                           | Kod-PR:er; enhetstestsvit med 4 sharder                                                                                                                                                                                            | Ja                                                                                                         |
| `No new ESLint warnings`                         | Kod-PR:er; undertryckningsmedveten lint-spärr                                                                                                                                                                                      | Ja för PR:er från det egna ursprunget, rådgivande för forkar                                               |
| `Merge integrity (changelog + generated skills)` | Icke-utkast till PR:er; synkronisering av ändringslogg och genererade färdigheter                                                                                                                                                  | Ja för PR:er från det egna ursprunget, rådgivande för forkar                                               |

#### Rapport om bortglömda syskontester

`npm run check:forgotten-sibling-tests` återanvänder importmatcharen bakom kartan över testpåverkan.
För varje ändrad produktionsmodul rapporterar den deterministiska kedjor av typen
`changed module/symbol -> static consumer -> candidate sibling test` när det möjliga
testet saknas i pull request-diffen. Markdown-sammanfattningen och JSON-resultatet bevaras som
arbetsflödesartefakten `forgotten-sibling-tests` för kalibrering före en eventuell blockerande utrullning.

Barrel-återexporter och dynamiska importer är endast diagnostik för matchning; de skapar aldrig ett
blockerande fynd. Granskade undantag finns i
`config/quality/forgotten-sibling-allowlist.json`. Varje post måste ange användaren och det möjliga
testet, ge en specifik motivering och länka till ett GitHub-ärende eller en pull request. Felaktigt
utformade poster stängs som standard. Undantag kan inte undertrycka ett borttaget möjligt test eller
en diff som lägger till `.skip`/`.todo`; försvagning av assertioner och annan maskering hanteras
fortsatt av den oberoende blockerande grinden `check:test-masking`.
...
| `check:route-validation:t06`      | Zod-scheman finns för alla rutter (policy för nivå 6)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | Ja                                         |
| `check:any-budget:t11`            | Antalet `@ts-expect-error // any` överskrider inte budgeten (nivå 11-spärr)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | Ja                                         |
...
| `check:model-lifecycle`           | De tre manuellt underhållna routningstabellerna förblir konsekventa med den incheckade livscykelögonblicksbilden (#11503): `FITNESS_TABLE` (`taskFitness.ts`) poängsätter inget utfasat id som `REGISTRY` kan dirigera; varje mål för `BUILT_IN_ALIASES` finns i `REGISTRY` och saknas i ögonblicksbilden över utfasade id:n; varje utfasat id som fortfarande finns i `REGISTRY` vidarebefordras eller anges i `allowedRetiredInCatalog`; och varken källor eller mål i `DEFAULT_DEGRADATION_MAP` visas som utfasade i den ögonblicksbilden. Detta bevisar inte att en modell för närvarande tillhandahålls av en aktiv uppströmsleverantör. Offline — jämför mot `config/quality/model-lifecycle.json`, som uppdateras manuellt med `npm run quality:refresh-model-lifecycle` (nätverk; inte integrerat i CI). `allowedRetiredInCatalog` är en spärrmekanism för successiv avveckling: lägg endast till en post tillsammans med ett spårningsärende. | Ja                                         |
| `check:fetch-targets`             | Varje `fetch("/api/...")` i `src/` på klientsidan matchar en faktisk `route.ts`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | Ja                                         |
| `check:deps`                      | Alla beroenden som kan installeras med `npm install` i varje `package.json` i kodförrådet finns i `dependency-allowlist.json`; nya ofästa paket eller paket med stavfelsliknande namn flaggas                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | Ja                                         |
| `audit:deps`                      | `npm audit` (rot + electron) — inga varningar med hög/kritisk allvarlighetsgrad (överlappar OSV-kontrollen `check:vuln-ratchet`; se rationaliseringsloggen)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | Ja                                         |
| `check:lockfile`                  | Integriteten för `package-lock.json` — HTTPS-register, integritetshashar, inga värdåsidosättningar                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | Ja                                         |
| `check:licenses`                  | Tillåtelselista för SPDX-licenser för produktionsberoenden                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | Ja                                         |
| `check:tracked-artifacts`         | Inga byggartefakter eller incheckade symboliska länkar till `node_modules` (körs även i huskys pre-commit; pre-push är avsiktligt lättviktig — #6716)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | Ja                                         |
| `check:ai-attribution`            | Ingen AI-/bot-genererad `Co-Authored-By`-trailer eller AI-genereringsfot i PR-commits, rubrik eller beskrivning — hård regel #16 (i snabbkontrollsloopen i `quality.yml` för PR→`release/**` — läser händelsens nyttolast, gör inget utanför PR:er — samt ett steg enbart för PR:er i `ci.yml`-lintningen för PR→`main`; även huskys `commit-msg`-hook; mänskliga medförfattare är tillåtna; #14436)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
| `check:file-size`                 | Ingen källfil överskrider gränsen för respektive filändelse (spärrmekanism: frysta stora filer i listan `frozen`)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | Ja                                         |
| `check:error-helper`              | Felsvar i exekverare/hanterare använder `buildErrorBody()` / `sanitizeErrorMessage()` (hård regel #12)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | Ja                                         |
| `check:migration-numbering`       | SQL-filer för migreringar är sekventiellt numrerade, utan luckor eller dubbletter                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | Ja                                         |
| `check:public-creds`              | Inga bokstavliga OAuth-`client_id`/`client_secret`-värden eller Firebase Web-nycklar utanför `publicCreds.ts` (strikt regel nr 11)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | Ja                                         |
| `check:db-rules`                  | Ingen rå SQL utanför modulerna i `src/lib/db/`; inga barrel-importer från `localDb.ts` (strikta regler nr 2/nr 5)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | Ja                                         |
| `check:known-symbols`             | Leverantörsexekverare, routningsstrategier och översättare som registrerats i sina dispatch-tabeller matchar filerna på disken – inga föräldralösa eller odeklarerade symboler                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | Ja                                         |
| `check:route-guard-membership`    | Varje rutt som startar en underordnad process klassificeras av `isLocalOnlyPath()` (strikta regler nr 15/nr 17)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | Ja                                         |
| `check:test-discovery`            | Varje `*.test.ts`-/`*.spec.ts`-fil i kodförrådet samlas in av minst en testkörare (spärrmekanism: listan över föräldralösa filer i `test-discovery-baseline.json` kan bara krympa)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | Ja                                         |
| `check:agent-skills-sync`         | Genererade agent-skills-artefakter överensstämmer med sin källkatalog (ingen avvikelse)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| `check:provider-asset-provenance` | Leverantörslogotyper/-resurser har en registrerad provenienspost                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| `lint:json`                       | JSON-konfigurationsfiler kan parsas och uppfyller repots lintregler                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
| `check:dashboard-typecheck`       | `tsc` begränsat till `src/app/(dashboard)/**` (#7033) — den handplockade tillåtelselistan med 27 filer i `typecheck:core` inkluderar ingen dashboard-TSX, och `next build` typkontrollerar dem inte heller (`next.config.mjs` anger `ignoreBuildErrors: true`), så regressioner med övergivna identifierare där (#6625/#6909) var osynliga för CI. Jämför mot en fryst baslinje med antal per fil/per TS-kod (`config/quality/dashboard-typecheck-baseline.json`, samma mönster för kontroll av inaktuella poster som i `check:known-symbols`) — endast NYA fel utöver det baslinjeangivna antalet gör att grinden underkänns; skruva ned baslinjen med `--update` när ett befintligt fel har åtgärdats.                                                                                                                                                                                                                                               | Ja                                         |
...
| ---------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------- |
| `quality:collect`            | Skapar `quality-metrics.json` (antal ESLint-varningar, täckning från sammanslagen shard-rapport)                                                                                 | Ja (föregår ratchet) |
...
| `check:complexity`           | Cyklomatisk komplexitet på filnivå överskrider inte gränsen (ESLint-kärnreglerna `complexity` + `max-lines-per-function`)                                                        | Ja                   |
...
| `check:compression-budget`   | Budget för komprimeringsbenchmark — miniminivåerna för tokenbesparingar per motor får inte försämras                                                                             | Ja                   |
| `check:type-coverage`        | Ratchet för procentuell typning (`type-coverage`) försämras inte; ersätter till stor del `typecheck:noimplicit:core`                                                             | Ja                   |
| `check:codeql-ratchet`       | Antalet öppna CodeQL-varningar försämras inte (läses via `gh api`; hoppas över utan fel om token saknas) — uppdateringsfrekvens och manuell utlösning: se ”CodeQL-ratchet” nedan | Ja                   |
...
om en binärfil fortfarande saknas.
...
| `check:secrets`          | Sökning efter hemligheter (gitleaks) — hoppas över om binärfilen saknas                                                                                                                                           | **Rådgivande** |
...
| `check:openapi-breaking` | Bakåtinkompatibla ändringar i det publika API-kontraktet (`openapi.yaml`) jämfört med basgrenen (oasdiff) — genererar `openapiBreaking=N`; hoppas över om oasdiff saknas eller basspecifikationen inte kan hittas | **Rådgivande** |
...
| ↳ `check:docs-counts`          | Antal i löptext (antal leverantörer, antal migreringar osv.) ligger inom spärrfönstret för de faktiska antalen                                                   | Ja                       |
...
| ↳ `check:doc-links`            | Interna markdown-länkar i dokumentationen pekar på verkliga filer (formen `[text]`/`(path)`)                                                                     | Ja                       |
...
| `check:openapi-coverage`       | OpenAPI-specifikationen täcker minst en spärrjusterad lägstanivå av verkliga rutter                                                                              | Ja                       |
...
| `check:openapi-routes`         | Varje sökväg i `openapi.yaml` motsvarar en verklig `route.ts` (skydd mot hallucinationer)                                                                        | Ja                       |
| `check:docs-symbols`           | Varje `/api/...`-referens i `docs/**/*.md` motsvarar en verklig `route.ts` (skydd mot hallucinationer)                                                           | Ja                       |
...
| `check-translation-ratio`         | Andelen riktiga översättningar per språkversion (blad som är identiska med engelskan, platshållare eller saknas utanför undantagslistan) får inte överskrida `config/quality/i18n-translation-baseline.json` + marginal | **Rådgivande** |

Kräver `fetch-depth: 0` — grinden för värdeavvikelse jämför `en.json` mot den gemensamma basen.

#### `check-ui-value-drift` — grind för inaktuella översättningar

Fångar den enda i18n-regression som de andra grindarna strukturellt inte kan upptäcka: ett engelskt värde
skrivs om medan översättningarna som härletts från den _tidigare_ engelska texten lämnas kvar, så att
icke-engelskspråkiga användare fortsätter läsa självsäkert formulerad men numera felaktig text.

Detta levererades faktiskt. `oauthModal.googleOAuthWarning` skrevs om när inloggningshjälpen för Antigravity
lades till (#5203); **39 av 43 språkversioner** behöll text som uppmanade operatörer att "kopiera hela
URL:en och klistra in den nedan" — ett flöde som inte kan slutföras för den leverantören. Det
förblev oupptäckt fram till #8463 eftersom:

- `sync-ui-keys` endast fyller i nycklar som **saknas**, aldrig sådana som är **inaktuella**;
- `check-ui-keys-coverage` räknar nyckelns _närvaro_, så en inaktuell översättning räknas som täckt;
...
**Diffmedveten, inte baslinjestödd.** Den jämför `en.json` vid sammanslagningsbasen med
arbetsträdet; för varje nyckel vars engelska värde har ändrats är varje språkversion som
fortfarande innehåller en orörd översättning inaktuell. Detta **fryser avsiktligt befintlig skuld** — en diff
kan inte visa vilken gammal engelsk text en sedan länge befintlig översättning kom från, så spärren bedömer
endast det som den aktuella ändringen berör. Alternativet (en hashbaslinje per nyckel) skulle kräva
en genererad fil på ~600 KB, 3× större än den största befintliga baslinjen, med ändringar vid varje i18n-PR.
...
Om strängens **innebörd** har ändrats bör du helst **byta namn på nyckeln**: en ny nyckel kan inte ärva
...
Avslutas med 0 och `SKIP reason=base-unresolved` när baskatalogen inte kan läsas (ytlig
...
| Skript                          | Validerar                                | Blockerande                                                |
| ------------------------------- | ---------------------------------------- | ---------------------------------------------------------- |
| `validate_translation.py quick` | Översättningens fullständighet per språk | **Rådgivande** (`continue-on-error: true` för hela jobbet) |
...
| `check:test-masking`   | Ändrade testfiler minskar inte nettoantalet assertions och lägger inte till tautologier av typen `assert.ok(true)`                            | Ja          |
| `check:pr-evidence`    | PR-texten hänvisar till test-/VPS-belägg för ändringen (automatiserar hård regel nr 18 genom att söka i PR-texten — bräckligt, se backloggen) | Ja          |
...
| ---------------- | -------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- |
| `test:vitest`    | MCP-server (110 verktyg), autoCombo, cache — vitest-körverktyg | Ja                                                                                                             |
| `test:vitest:ui` | UI-komponenttester — vitest-körverktyg                         | **Blockerande** — befintliga fel undantas uttryckligen i `vitest.config.ts`; nya fel gör att jobbet misslyckas |

### Nattliga arbetsflöden (schemalagda, rådgivande)

Dessa körs enligt ett cron-schema (och via `workflow_dispatch`), aldrig för PR:er. Alla är rådgivande.

| Arbetsflöde            | Validerar                                                                                                                                                             | Blockerande    |
| ---------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------- |
| `nightly-property`     | fast-check-egenskapstester med ett slumpmässigt seed-värde och ett stort antal körningar                                                                              | **Rådgivande** |
| `nightly-resilience`   | spärr för heap-tillväxt, felinjektion med chaos-testning, k6-belastnings-/uthållighetstestning                                                                        | **Rådgivande** |
| `nightly-llm-security` | promptfoo-injektionsskydd (blockeringsläge) + garak-prober (hoppas över utan en leverantörshemlighet)                                                                 | **Rådgivande** |
...
| `nightly-mutation`     | poäng från Stryker-mutationstestning för den snabba enhetstestbanan — överlevande mutanter synliggör svaga assertions                                                 | **Rådgivande** |
