...
# హోస్ట్ ప్రొఫైల్ (Linux-ప్రథమం; హోస్ట్ CLI బైనరీలను రీడ్-ఓన్లీగా మౌంట్ చేస్తుంది)
...
ప్రధాన డిప్లాయ్మెంట్ విధానాల కోసం OmniRoute Compose ప్రొఫైల్లను అందిస్తుంది. మీ వాతావరణానికి సరిపోయేదాన్ని ఎంచుకోండి.
...
| `cli`             | `omniroute-cli`  | `omniroute providers/setup/doctor`ను మరియు బండిల్ చేసిన CLIలను (Codex, Claude Code, Droid, OpenClaw) పిలిచే ఏజెంటిక్ వర్క్ఫ్లోలు                        | `docker compose --profile cli up -d`         |
| `host`            | `omniroute-host` | `~/.local/bin`, `~/.codex`, `~/.claude` మొదలైనవాటిని రీడ్-ఓన్లీగా మౌంట్ చేయడం ద్వారా హోస్ట్ CLIలకు `network_mode` తరహా యాక్సెస్ కోరుకునే Linux హోస్ట్లు | `docker compose --profile host up -d`        |
| `cliproxyapi`     | `cliproxyapi`    | అప్స్ట్రీమ్ CLI ప్రాక్సీయింగ్ కోసం `8317` పోర్ట్లో [CLIProxyAPI](https://github.com/router-for-me/CLIProxyAPI) సైడ్కార్ను అమలు చేయండి                   | `docker compose --profile cliproxyapi up -d` |
...
> అనేక ప్రొఫైల్లను కలిపి ఉపయోగించవచ్చు: `docker compose --profile cli --profile cliproxyapi up -d`.
...
ఈ రిపోజిటరీ బహుళ-దశల Dockerfile (`Dockerfile`)తో అందించబడుతుంది. నాలుగు దశలు అందుబాటులో ఉన్నాయి; మీ వినియోగ సందర్భానికి సరైన `target`ను ఎంచుకోండి.

| దశ            | బేస్ ఇమేజ్            | ఉద్దేశ్యం                                                                                                                                                                                                                                                                                                        |
| ------------- | --------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `builder`     | `node:26-trixie-slim` | డిపెండెన్సీలను ఇన్స్టాల్ చేస్తుంది (`npm ci --legacy-peer-deps`), అలాగే `npm run build`ను అమలు చేస్తుంది (డిఫాల్ట్గా Turbopack — దిగువన ఉన్న బిల్డ్-సమయ వనరులు చూడండి)                                                                                                                                           |
| `runner-base` | `node:26-trixie-slim` | Next.js స్వతంత్ర అవుట్పుట్తో కూడిన ప్రొడక్షన్ రన్టైమ్. **ప్రొవైడర్ CLIలు ఏవీ బండిల్ చేయబడవు.**                                                                                                                                                                                                                   |
| `runner-cli`  | `runner-base`         | `git`, `docker.io`, `docker-compose`తో పాటు గ్లోబల్ CLIలను జోడిస్తుంది: `@openai/codex`, `@anthropic-ai/claude-code`, `droid`, `openclaw`. **ఏజెంట్ ఆధారిత వర్క్ఫ్లోల కోసం దీన్ని ఎంచుకోండి.**                                                                                                                   |
| `runner-web`  | `runner-base`         | వెబ్-సెషన్ ప్రొవైడర్లు `gemini-web`, `claude-web`, `claude-turnstile` కోసం Playwrightతో పాటు Chromium బ్రౌజర్ను (`--with-deps`) జోడిస్తుంది. **మీరు ఆ ప్రొవైడర్లను ఉపయోగించినప్పుడు దీన్ని ఎంచుకోండి** — ఇది లేకుండా సాధారణ ఇమేజ్ అభ్యర్థన సమయంలో విఫలమవుతుంది (విడుదల ఛానెల్ల కింద ఉన్న `-web` గమనికను చూడండి). |
...
`builder` దశ వినియోగించే వనరులను మూడు బిల్డ్ ఆర్గ్యుమెంట్లు నియంత్రిస్తాయి. ఇవి బిల్డ్ సమయంలో మాత్రమే వర్తిస్తాయి —
`OMNIROUTE_MEMORY_MB` (దిగువన) అనేది రన్టైమ్కు సంబంధించిన ప్రత్యేక నియంత్రణ.

| బిల్డ్ ఆర్గ్యుమెంట్         | డిఫాల్ట్ | ప్రభావం                                                                                                            |
| --------------------------- | -------- | ------------------------------------------------------------------------------------------------------------------ |
| `OMNIROUTE_USE_TURBOPACK`   | `1`      | `0` అయితే బదులుగా webpackతో బిల్డ్ చేస్తుంది. గరిష్ఠ మెమరీ వినియోగం తక్కువగా ఉంటుంది, కానీ నెమ్మదిగా ఉంటుంది.      |
| `OMNIROUTE_BUILD_MEMORY_MB` | `6144`   | ప్రారంభించబడిన `next build` కోసం V8 హీప్ గరిష్ఠ పరిమితి (`--max-old-space-size`).                                  |
| `OMNIROUTE_BUILD_WORKERS`   | `2`      | `CIRCLE_NODE_TOTAL`కు విలువను అందిస్తుంది; పేజీ-డేటా సేకరణ కోసం Next దీని నుంచి `workers = N - 1`ను లెక్కిస్తుంది. |

పెద్ద బిల్డర్లో పెంచాల్సింది `OMNIROUTE_BUILD_WORKERS`; పరిమిత వనరులున్న బిల్డ్
`✓ Compiled successfully` **తర్వాత** నిలిచిపోతే అనుమానించాల్సిందీ ఇదే. ప్రతి
పేజీ-డేటా వర్కర్ ఒక ప్రత్యేక ప్రాసెస్, అలాగే పేరెంట్ `next build` కూడా ప్రత్యేక
ప్రాసెసే; ప్రత్యక్ష VPS పునరుత్పత్తిలో (issue #7518), `NODE_OPTIONS` హీప్ ఫ్లాగ్తో
సంబంధం లేకుండా ప్రతి ప్రాసెస్ గరిష్ఠ RSS ~4.5 GBగా కొలవబడింది (Turbopack,
V8 హీప్ వెలుపల ఉన్న నేటివ్/Rust మెమరీలో కంపైల్ చేస్తుంది). `2` డిఫాల్ట్ విలువతో
(→ 1 వర్కర్, మొత్తం 2 ప్రాసెస్లు), పబ్లిష్ పైప్లైన్ ఉపయోగించే 16 GB / 4 vCPU
GitHub-హోస్టెడ్ రన్నర్లకు తగిన విధంగా పరిమాణం నిర్ణయించబడింది. `8` వద్ద
(→ 7 వర్కర్లు), ఆ రన్నర్లో మెమరీ అయిపోయింది మరియు buildkit
`ResourceExhausted: ... cannot allocate memory`తో ఆ దశను విఫలం చేసింది;
ప్రతి ప్రాసెస్ RSSను అంచనా వేయడానికి బదులుగా నేరుగా కొలిచినప్పుడు `3`
(→ 2 వర్కర్లు) కూడా సరిపోలేదు. `tests/unit/docker-build-memory-budget.test.ts`
కొలిచిన విలువ ఆధారంగా గణన చేస్తుంది; ఏ నియంత్రణ అయినా రన్నర్ సామర్థ్యాన్ని
మించితే విఫలమవుతుంది.

Turbopack, V8 హీప్కు **వెలుపల** ఉండే నేటివ్ Rust మెమరీలో కంపైల్ చేస్తుంది,
కాబట్టి `OMNIROUTE_BUILD_MEMORY_MB` దానిని పరిమితం చేయదు. మెమరీ పరిమితి ఉన్న
హోస్ట్లో, ఎలాంటి ఎర్రర్ టెక్స్ట్ లేకుండానే OOM కిల్లర్ బిల్డ్ను SIGKILL
చేస్తుంది — అది `Creating an optimized production build` మధ్యలోనే ఆగిపోతుంది;
అందువల్ల ఇది మెమరీ అయిపోవడం కంటే నిలిచిపోయినట్లు కనిపిస్తుంది. బిల్డ్ హోస్ట్కు
వనరుల పరిమితి ఉంటే, బండ్లర్లను మార్చండి:
...
`webpackBuildWorker` ప్రారంభించబడి ఉంటుంది, కాబట్టి `next build` ఒక పేరెంట్
**మరియు** ఒక వర్కర్ ప్రాసెస్ను అమలు చేస్తుంది; ప్రతి ఒక్కటీ
`OMNIROUTE_BUILD_MEMORY_MB`ను విడివిడిగా పాటిస్తుంది. కంటైనర్ పరిమితిని ఆ
విలువకు ఒక రెట్టు కాకుండా, సుమారు రెండు రెట్ల కంటే ఎక్కువగా సెట్ చేయండి.
...
| webpack   | 8 GiB           | బిల్డ్ వర్కర్ SIGKILL ద్వారా నిలిపివేయబడింది                  |
| webpack   | 12 GiB          | విజయవంతమైంది, గరిష్ఠంగా 11.1 GiBకు చేరుకుంది                  |

### రన్టైమ్ డిఫాల్ట్లు

`runner-base` ద్వారా ఎక్స్పోర్ట్ చేయబడే డిఫాల్ట్లు: `PORT=20128`, `HOSTNAME=0.0.0.0`, `OMNIROUTE_MEMORY_MB=1024`, `NODE_OPTIONS=--max-old-space-size=1024`, `DATA_DIR=/app/data`, `OMNIROUTE_MIGRATIONS_DIR=/app/migrations`.

Dockerలో మెమరీ ప్రవర్తన:

- ఇమేజ్ `OMNIROUTE_MEMORY_MB=1024`ని సెట్ చేసి, దాని నుంచి `NODE_OPTIONS=--max-old-space-size=1024`ని ఉత్పన్నం చేస్తుంది.
- అసలు సర్వర్ ప్రాసెస్ను standalone launcher ప్రారంభిస్తుంది; ఇది `OMNIROUTE_MEMORY_MB`ని చదివి, `--max-old-space-size=<OMNIROUTE_MEMORY_MB>`ని జోడిస్తుంది.
- చివరిగా పునరావృతమైన `--max-old-space-size` విలువను Node ఉపయోగిస్తుంది, కాబట్టి `OMNIROUTE_MEMORY_MB`ని సెట్ చేయడం ద్వారా అమల్లో ఉండే Docker heap పరిమితిని నియంత్రించవచ్చు.
- ఇమేజ్ దీన్ని ఎల్లప్పుడూ సెట్ చేస్తుంది కాబట్టి, launcher స్వంత RAM-ఆధారిత fallback Dockerలో ఎప్పటికీ వర్తించదు. workloadకు అనుగుణంగా దీన్ని స్పష్టంగా పెంచండి (దిగువ పట్టిక చూడండి). coding-agent `/v1/responses` కోసం `2048` ఇప్పటికీ చాలా తక్కువ.

### coding agentల కోసం runtime RAM

1 GiB Docker డిఫాల్ట్ అనేది dashboard/తేలికపాటి-chat కోసం కనీస స్థాయి మాత్రమే, production పరిమాణం కాదు. పొడవైన `POST /v1/responses` bodyలు (వందలాది messages, పదుల సంఖ్యలో tools) compression సమయంలో అనేక in-memory graphలను నిల్వ ఉంచుతాయి. ఒకేసారి అమలైన సుమారు ~3 MiB / ~750k-token పరిమాణం గల రెండు requestలు **12 GiB** old-space వద్ద V8ని నిలిపివేశాయి (`FATAL ERROR: Reached heap limit`), అలాగే 16 GiB cgroup OOM పరిమితిని కూడా చేరుకున్నాయి. [#7849](https://github.com/diegosouzapw/OmniRoute/issues/7849) చూడండి.

**cgroup `--memory`ని heap కంటే ఎక్కువగా** కేటాయించండి — native bufferలు, SQLite మరియు compression intermediateలు V8 వెలుపల ఉంటాయి.

| Workload                                          | `OMNIROUTE_MEMORY_MB`       | Container / cgroup                  | గమనికలు                                                                                                             |
| ------------------------------------------------- | --------------------------- | ----------------------------------- | ------------------------------------------------------------------------------------------------------------------- |
| Dashboard, ఒక తేలికపాటి chat                      | `1024` (ఇమేజ్ డిఫాల్ట్)     | ≥2 GiB                              |                                                                                                                     |
...
| ఒకేసారి మూడు లేదా అంతకంటే ఎక్కువ పొడవైన contextలు | ఒకే processలో అమలు చేయవద్దు | క్రమబద్ధంగా అమలు చేయండి / మరింత RAM | డిఫాల్ట్ heavyweight admissionలో 1 in-flight మాత్రమే ఉంటుంది; RAM పెంచకుండా దీన్ని పెంచితే abort మళ్లీ సంభవిస్తుంది |

bare metalపై `omniroute serve`, `OMNIROUTE_MEMORY_MB` **సెట్ చేయనప్పుడు**, RAMలో సుమారు 35%ను (`[512, 4096]` పరిధికి పరిమితం చేసి) కేటాయిస్తుంది. Docker ఎల్లప్పుడూ `1024`ని సెట్ చేస్తుంది కాబట్టి, అధికారిక ఇమేజ్లో ఆ calibration ఎప్పటికీ అమలు కాదు.
...
[ENVIRONMENT.md](../reference/ENVIRONMENT.md)లో డాక్యుమెంట్ చేసిన డిఫాల్ట్లకు అదనంగా, Docker కింద రన్ చేస్తున్నప్పుడు కింది వేరియబుల్స్ అత్యంత ముఖ్యమైనవి:

| వేరియబుల్                     | ప్రయోజనం                                                                                                                                                                                                                                                                                | డిఫాల్ట్                   |
| ----------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------- |
| `OMNIROUTE_WS_BRIDGE_SECRET`  | WebSocket బ్రిడ్జ్ కోసం భాగస్వామ్య సీక్రెట్. **ప్రొడక్షన్లో తప్పనిసరి** — బలమైన యాదృచ్ఛిక స్ట్రింగ్గా సెట్ చేయండి.                                                                                                                                                                      | సెట్ చేయబడలేదు (అందించాలి) |
| `REDIS_URL`                   | రేట్ లిమిటర్ / క్యాష్ బ్యాకెండ్ కోసం కనెక్షన్ స్ట్రింగ్                                                                                                                                                                                                                                 | `redis://redis:6379`       |
| `REDIS_PORT`                  | బండిల్ చేసిన Redis కంటైనర్ కోసం హోస్ట్ వైపు పోర్ట్                                                                                                                                                                                                                                      | `6379`                     |
...
| `DASHBOARD_PORT` / `API_PORT` | డ్యాష్బోర్డ్ (20128), API (20129) కోసం బహిర్గత పోర్ట్లను ఓవర్రైడ్ చేస్తుంది                                                                                                                                                                                                             | `20128` / `20129`          |
| `APP_BIND_HOST`               | docker-compose డ్యాష్బోర్డ్/API/live-WS పోర్ట్లను ప్రచురించే హోస్ట్ ఇంటర్ఫేస్. `REQUIRE_API_KEY=false` (డిఫాల్ట్)తో, `0.0.0.0` అనామక `/v1` ప్రాక్సీని LANకు బహిర్గతం చేస్తుంది — `REQUIRE_API_KEY=true` ఉన్నప్పుడు లేదా ముందు రివర్స్ ప్రాక్సీ ఉన్నప్పుడు మాత్రమే పరిధిని విస్తరించండి. | `127.0.0.1`                |
| `CLIPROXY_BIND_HOST`          | docker-compose `cliproxyapi` సైడ్కార్ను ప్రచురించే హోస్ట్ ఇంటర్ఫేస్ — దాని డేటా వాల్యూమ్లో ప్రొవైడర్ క్రెడెన్షియల్స్ ఉంటాయి.                                                                                                                                                            | `127.0.0.1`                |
| `OMNIROUTE_PLUGINS_DIR`       | రన్టైమ్ ప్లగిన్ స్కానర్ చదివే మరియు ఇన్స్టాల్ చేసే డైరెక్టరీ. ప్లగిన్లు బైండ్-మౌంట్ చేయబడినప్పుడు దీన్ని సెట్ చేయండి: డిఫాల్ట్ `HOME`ను అనుసరిస్తుంది, అయితే ఇమేజ్ దాన్ని ఎక్స్పోర్ట్ చేయకపోవచ్చు.                                                                                      | `~/.omniroute/plugins`     |
| `OMNIROUTE_BASE_PATH`         | యాప్ రివర్స్ ప్రాక్సీ వెనుక ప్రచురించబడినప్పుడు ఉపయోగించే URL సబ్పాత్ (ఉదా. `/omniroute`)                                                                                                                                                                                               | _(ఖాళీ = రూట్)_            |
| `NEXT_PUBLIC_BASE_URL`        | సబ్పాత్తో సహా పబ్లిక్ బ్రౌజర్ ఆరిజిన్ (ఉదా. `https://host/omniroute`)                                                                                                                                                                                                                   | సెట్ చేయబడలేదు             |
| `PROD_DASHBOARD_PORT`         | `docker-compose.prod.yml` కోసం హోస్ట్ వైపు డ్యాష్బోర్డ్ పోర్ట్                                                                                                                                                                                                                          | `20130`                    |
| `CLIPROXYAPI_PORT`            | `cliproxyapi` సైడ్కార్ కోసం హోస్ట్ వైపు పోర్ట్                                                                                                                                                                                                                                          | `8317`                     |
...
స్థిరమైన విడుదలలు, క్రియాశీల విడుదల-బ్రాంచ్ పరీక్ష మరియు డెవలప్మెంట్ బిల్డ్ల కోసం OmniRoute వేర్వేరు Docker ఛానల్లను ప్రచురిస్తుంది.

| ఛానల్                           | మూలం                                   | మార్పు సామర్థ్యం           | సిఫార్సు చేసిన వినియోగం                                                                                                      |
| ------------------------------- | -------------------------------------- | -------------------------- | ---------------------------------------------------------------------------------------------------------------------------- |
| `:<version>` / `:<version>-web` | సంతకం చేసిన/వెర్షన్ కేటాయించిన విడుదల  | మార్పులేనిది               | ఖచ్చితమైన విడుదలను పిన్ చేసే ప్రొడక్షన్ డిప్లాయ్మెంట్లు                                                                      |
| `:latest` / `:latest-web`       | అత్యధిక **ప్రచురిత** స్థిరమైన SemVer   | మారగల స్థిరమైన పాయింటర్    | SemVer ప్రచురణ జాబ్ **తర్వాత** స్థిరమైన విడుదలలను అనుసరిస్తుంది — `main`ను లేదా విడుదల కాని `release/v*` కమిట్లను అనుసరించదు |
| `:next` / `:next-web`           | ప్రస్తుత డిఫాల్ట్ `release/v*` బ్రాంచ్ | మారగల ప్రీ-రిలీజ్ పాయింటర్ | క్రియాశీల విడుదల బ్రాంచ్లో చేరినప్పటికీ ఇంకా స్థిరమైన విడుదలలో లేని పరిష్కారాలను పరీక్షించడం                                 |
| `:main` / `:main-web`           | `main` బ్రాంచ్                         | మారగల డెవలప్మెంట్ పాయింటర్ | డెవలప్మెంట్ మరియు ఇంటిగ్రేషన్ పరీక్షలకు మాత్రమే                                                                              |

#### వెబ్-సెషన్ ప్రొవైడర్లు: `-web` ఇమేజ్లు

పైన ఉన్న ప్రతి ఛానల్కు ఒక `-web` ట్యాగ్ (`:latest-web`, `:<version>-web`, `:next-web`, `:main-web`) కూడా ఉంటుంది. ఇది `runner-web` దశ నుండి బిల్డ్ చేయబడుతుంది — అదే ఇమేజ్కు Playwright మరియు Chromium బ్రౌజర్ను జోడించిన రూపం. సాధారణ ఇమేజ్ Chromium **లేకుండా** వస్తుంది; `gemini-web`, `claude-web` మరియు `claude-turnstile`కు అది అవసరం.

వైఫల్యం ప్రారంభ సమయంలో కాకుండా వాయిదా వేయబడుతుంది: ఆ ప్రొవైడర్లు తమ మోడల్లను జాబితా చేసి డ్యాష్బోర్డ్లో కనెక్ట్ అయినట్లు కనిపిస్తాయి, మొదటి అభ్యర్థన మాత్రమే కింది లోపంతో విఫలమవుతుంది
...
మీరు ఆ ప్రొవైడర్లను ఉపయోగిస్తే, మీరు ఇప్పటికే ఉపయోగిస్తున్న ఛానల్కు చెందిన `-web` ట్యాగ్ను పుల్ చేయండి — మరేదీ మారదు. npm/CLI ఇన్స్టాలేషన్లో (Docker ఇమేజ్ లేకుండా), దీనికి సమానంగా లోపించిన భాగం బ్రౌజర్ బైనరీ: హోస్ట్పై `npx playwright install chromium`ను అమలు చేయండి.

#### ప్రీ-రిలీజ్ ఛానల్ను ఉపయోగించడం
...
విడుదల-బ్రాంచ్ బిల్డ్ ఎప్పటికీ `latest`ను మార్చలేదు; అర్హత కలిగిన స్థిరమైన సెమాంటిక్ వెర్షన్ మాత్రమే స్థిరమైన పాయింటర్ను ప్రమోట్ చేయగలదు. `next` ఇమేజ్లు విడుదల ఇమేజ్ తనిఖీని మరియు CRITICAL-వల్నరబిలిటీ నిరోధక గేట్ను కొనసాగిస్తాయి.

**`latest` అనేది git తాజాదనానికి హామీ కాదు.** `main`లో లేదా క్రియాశీల `release/v*` బ్రాంచ్లో విలీనమైన పరిష్కారాలు, స్థిరమైన SemVer ఇమేజ్ ప్రచురించబడి ప్రచురణ జాబ్ `:latest`ను ప్రమోట్ చేసే వరకు **`:latest`లో ఉండవు** (ఆ SemVerకు ఉన్న అదే డైజెస్ట్). GitHubలో పరిష్కారం ఇప్పటికే కనిపిస్తున్నప్పటికీ `latest` స్థిరంగా ఉన్నట్లు కనిపిస్తే, విడుదల బ్రాంచ్ను పరీక్షించడానికి `:next`ను పుల్ చేయండి లేదా SemVer ట్యాగ్ కోసం వేచి ఉండండి.

| మీకు కావలసినది                                                                  | ఉపయోగించాల్సింది                               |
| ------------------------------------------------------------------------------- | ---------------------------------------------- |
| మార్పు చెందకూడని GitOps / ప్రొడక్షన్                                            | `:X.Y.Z`ను (లేదా ఇమేజ్ డైజెస్ట్ను) పిన్ చేయండి |
| ప్రచురిత స్థిరమైన విడుదలలను అనుసరిస్తూ ప్రతి విడుదలకు రీక్రియేషన్ను అంగీకరించడం | `:latest`                                      |
| విడుదల కాని `release/v*` కమిట్లను పరీక్షించడం                                   | `:next` (ప్రొడక్షన్ కోసం కాదు)                 |
