...
## Inwentaryzacja bramek (~90 skryptów)
...
### Szybka ścieżka PR-ów wydaniowych (`quality.yml`)

`.github/workflows/quality.yml` jest uruchamiany dla PR-ów kierowanych do `release/**`. Umożliwia płynną pracę
na gałęziach współtwórców dzięki szybkim bramkom filtrowanym według ścieżek, a także udostępnia jeden doradczy sygnał z kompilacji produkcyjnej dla
zmian w kodzie:

| Zadanie                                          | Zakres                                                                                                                                                                                                                                                             | Blokowanie                                                                                               |
| ------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------- |
| `Build (advisory)`                               | PR-y z kodem niebędące wersjami roboczymi oraz gałęzie kolejki Mergify; Node 24, `npm-ci-retry`, `check:node-runtime`, `npm run build` z `OMNIROUTE_USE_TURBOPACK=1`; bez przesyłania artefaktu, ponieważ nie korzysta z niego żadne późniejsze zadanie jakościowe | **Doradcze** (`continue-on-error: true`; usunąć po tygodniu stabilnych uruchomień dla PR-ów wydaniowych) |
| `Docs Gates (fast-path)`                         | PR-y z dokumentacją/kodem; odwołania do dokumentacji API oraz docs-all                                                                                                                                                                                             | Tak                                                                                                      |
| `Fast Quality Gates`                             | PR-y z kodem; kontrole statyczne, sprawdzanie typów, sprawdzanie typów panelu, testy jednostkowe dla zmienionych elementów                                                                                                                                         | Tak                                                                                                      |
| `Forgotten sibling tests`                        | PR-y z kodem; śledzenie zmienionych modułów do statycznych konsumentów i potencjalnych powiązanych testów; ścieżki barrel oraz importów dynamicznych są raportowane jako diagnostyka doradcza wraz z przywołanymi wyjątkami z listy dozwolonych                    | **Doradcze**                                                                                             |
...
#### Raport zapomnianych powiązanych testów

`npm run check:forgotten-sibling-tests` ponownie wykorzystuje mechanizm rozwiązywania importów używany przez mapę wpływu na testy.
Dla każdego zmienionego modułu produkcyjnego raportuje deterministyczne łańcuchy
`zmieniony moduł/symbol -> statyczny konsument -> potencjalny powiązany test`, gdy potencjalny
test nie występuje w różnicach pull requestu. Podsumowanie w formacie Markdown oraz wynik JSON są zachowywane jako
artefakt przepływu pracy `forgotten-sibling-tests` na potrzeby kalibracji przed wdrożeniem trybu blokującego.

Ponowne eksporty barrel oraz importy dynamiczne stanowią wyłącznie diagnostykę rozwiązywania zależności; nigdy nie powodują
...
test, zawierać konkretne uzasadnienie oraz odsyłać do zgłoszenia GitHub lub pull requestu. Nieprawidłowo sformułowane wpisy powodują
odrzucenie. Wyjątki nie mogą pomijać usuniętego potencjalnego testu ani różnic dodających `.skip`/`.todo`;
osłabianie asercji oraz inne formy maskowania pozostają w gestii niezależnie blokującej
bramki `check:test-masking`.

### Zadanie: `lint`

Uruchamiane dla każdego PR-a do `main`. W przypadku niepowodzenia blokuje scalenie.

| Skrypt (`npm run ...`)            | Sprawdza                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | Blokujący                                    |
| --------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------- |
| `check:node-runtime`              | Wersja Node.js mieści się w obsługiwanym zakresie                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | Tak                                          |
| `check:cycles`                    | Importy cykliczne — wszystkie moduły w `src/` + `open-sse/`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | Tak                                          |
| `check:route-validation:t06`      | Schematy Zod są obecne we wszystkich trasach (zasady poziomu 6)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | Tak                                          |
| `check:any-budget:t11`            | Liczba wystąpień `@ts-expect-error // any` nie przekracza limitu (zapadka poziomu 11)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | Tak                                          |
| `check:provider-consistency`      | Każdy dostawca w `providers.ts` ma odpowiadający mu wpis w `providerRegistry.ts` (i odwrotnie, w ramach listy dozwolonych elementów)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | Tak                                          |
| `check:model-lifecycle`           | Trzy ręcznie utrzymywane tabele routingu pozostają spójne z zapisaną w repozytorium migawką cyklu życia (#11503): `FITNESS_TABLE` (`taskFitness.ts`) nie ocenia żadnego wycofanego identyfikatora, do którego `REGISTRY` może kierować żądania; każdy element docelowy `BUILT_IN_ALIASES` występuje w `REGISTRY` i nie występuje w migawce wycofanych identyfikatorów; każdy wycofany identyfikator nadal znajdujący się w `REGISTRY` jest przekierowywany lub wymieniony w `allowedRetiredInCatalog`; a żadne źródło ani element docelowy `DEFAULT_DEGRADATION_MAP` nie jest oznaczone jako wycofane w tej migawce. Nie dowodzi to, że model jest obecnie udostępniany przez działającą usługę nadrzędną. Tryb offline — porównuje z `config/quality/model-lifecycle.json`, odświeżanym ręcznie za pomocą `npm run quality:refresh-model-lifecycle` (wymaga sieci; nie jest podłączone do CI). `allowedRetiredInCatalog` jest mechanizmem stopniowego ograniczania: wpis należy dodawać wyłącznie wraz ze zgłoszeniem służącym do śledzenia prac. | Tak                                          |
| `check:fetch-targets`             | Każde `fetch("/api/...")` w klienckim kodzie `src/` wskazuje na rzeczywisty plik `route.ts`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | Tak                                          |
| `check:deps`                      | Wszystkie zależności możliwe do zainstalowania za pomocą `npm install` ze wszystkich plików `package.json` w repozytorium znajdują się w `dependency-allowlist.json`; nowe nieprzypięte pakiety lub pakiety podszywające się pod inne są oznaczane                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | Tak                                          |
| `audit:deps`                      | `npm audit` (katalog główny + electron) — brak ostrzeżeń o wysokim/krytycznym poziomie ważności (częściowo pokrywa się z osv `check:vuln-ratchet`; zobacz Rejestr zadań racjonalizacyjnych)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | Tak                                          |
...
| `check:ai-attribution`            | Brak stopki `Co-Authored-By` dotyczącej AI/bota ani stopki informującej o wygenerowaniu przez AI w commitach, tytule lub treści PR — Twarda reguła #16 (w pętli szybkich kontroli `quality.yml` dla PR→`release/**` — odczytuje dane zdarzenia, nie wykonuje żadnych działań poza PR — oraz w kroku wykonywanym wyłącznie dla PR w zadaniu lint pliku `ci.yml` dla PR→`main`; także w hooku `commit-msg` husky; współautorzy będący ludźmi są dozwoleni; #14436)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| `check:vitest-exclusions`         | Każde wykluczenie Vitest wskazuje zgłoszenie śledzące i występuje w `config/quality/vitest-exclusions.json` (#13204)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | Tak                                          |
| `check:file-size`                 | Żaden plik źródłowy nie przekracza limitu dla danego rozszerzenia (mechanizm zapadkowy: istniejące duże pliki znajdują się na liście `frozen`)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | Tak                                          |
| `check:error-helper`              | Odpowiedzi z błędami w wykonawcach/procedurach obsługi używają `buildErrorBody()` / `sanitizeErrorMessage()` (Twarda reguła #12)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | Tak                                          |
| `check:migration-numbering`       | Pliki migracji SQL są numerowane sekwencyjnie, bez luk ani duplikatów                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | Tak                                          |
| `check:public-creds`              | Brak jawnie zapisanych wartości OAuth `client_id`/`client_secret` lub kluczy Firebase Web poza `publicCreds.ts` (Twarda reguła nr 11)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | Tak                                          |
| `check:db-rules`                  | Brak surowego kodu SQL poza modułami `src/lib/db/`; brak importów zbiorczych z `localDb.ts` (Twarde reguły nr 2/5)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | Tak                                          |
| `check:known-symbols`             | Moduły wykonawcze dostawców, strategie routingu i translatory zarejestrowane w odpowiednich tabelach dyspozytorskich są zgodne z plikami na dysku — brak osieroconych lub niezadeklarowanych symboli                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | Tak                                          |
...
| `check:test-discovery`            | Każdy plik `*.test.ts` / `*.spec.ts` w repozytorium jest wykrywany przez co najmniej jeden program uruchamiający testy (mechanizm zapadkowy: lista osieroconych plików w `test-discovery-baseline.json` może się tylko zmniejszać)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | Tak                                          |
| `check:agent-skills-sync`         | Wygenerowane artefakty agent-skills są zgodne z katalogiem źródłowym (brak rozbieżności)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| `typecheck:noimplicit:core`       | Rygorystyczne `noImplicitAny` — rozwiązanie ukierunkowane na przyszłość; wiele istniejących miejsc wywołań nadal wymaga adnotacji                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | **Informacyjne** (`continue-on-error: true`) |
| `check:dashboard-typecheck`       | `tsc` ograniczone do `src/app/(dashboard)/**` (#7033) — starannie dobrana lista 27 plików dozwolonych przez `typecheck:core` nie obejmuje żadnych plików TSX panelu, a `next build` również nigdy nie sprawdza ich typów (`next.config.mjs` ustawia `ignoreBuildErrors: true`), dlatego regresje związane z osieroconymi identyfikatorami (#6625/#6909) pozostawały niewidoczne dla CI. Różnice są porównywane z zamrożoną wartością bazową liczby błędów według pliku i kodu TS (`config/quality/dashboard-typecheck-baseline.json`, ten sam wzorzec egzekwowania aktualności co w `check:known-symbols`) — bramka kończy się niepowodzeniem tylko w przypadku NOWYCH błędów wykraczających poza bazową liczbę; gdy istniejący wcześniej błąd zostanie naprawiony, zmniejsz wartość bazową za pomocą `--update`.                                                                                                                                                                                                                                  | Tak                                          |
...
| ---------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------- |
| `quality:collect`            | Generuje `quality-metrics.json` (liczba ostrzeżeń ESLint, pokrycie na podstawie scalonego raportu fragmentów)                                                                                             | Tak (poprzedza mechanizm zapadkowy) |
...
| `check:cognitive-complexity` | Mechanizm zapadkowy złożoności poznawczej (`eslint-plugin-sonarjs`) — oddzielne uruchomienie ESLint; CI uruchamia oba kontrole połączone w pojedynczym kroku `check:complexity-ratchets`                  | Tak                                 |
| `check:dead-code`            | Mechanizm zapadkowy nieużywanych eksportów / plików (knip) nie uległ pogorszeniu względem wartości bazowej                                                                                                | Tak                                 |
| `check:compression-budget`   | Budżet testu porównawczego kompresji — minimalne oszczędności tokenów dla poszczególnych silników nie mogą ulec pogorszeniu                                                                               | Tak                                 |
| `check:type-coverage`        | Mechanizm zapadkowy procentowego pokrycia typami (`type-coverage`) nie uległ pogorszeniu; w dużej mierze zastępuje `typecheck:noimplicit:core`                                                            | Tak                                 |
| `check:codeql-ratchet`       | Liczba otwartych alertów CodeQL nie uległa pogorszeniu (odczyt przez `gh api`; łagodne pominięcie bez tokenu) — częstotliwość odświeżania i ręczne wyzwalanie: patrz „Mechanizm zapadkowy CodeQL” poniżej | Tak                                 |

### Zadanie: `quality-extended`

Całe zadanie ma charakter doradczy (`continue-on-error: true`). Mechanizmy zapadkowe oparte na npm są
uruchamiane faktycznie; zewnętrzne skanery są instalowane za pomocą `gh release download` i samoczynnie pomijają działanie (kod wyjścia 0),
gdy plik binarny jest nadal niedostępny.

| Skrypt                   | Sprawdza                                                                                                                                                                                                               | Blokujący    |
| ------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------ |
| `check:circular-deps`    | Brak zależności cyklicznych (dpdm)                                                                                                                                                                                     | **Doradczy** |
...
| `check:vuln-ratchet`     | Podatności zależności (osv-scanner) nie uległy pogorszeniu — pomijane w przypadku braku pliku binarnego                                                                                                                | **Doradczy** |
| `check:workflows`        | Lintowanie przepływów pracy (actionlint + zizmor) — pomijane w przypadku braku plików binarnych                                                                                                                        | **Doradczy** |
| `check:openapi-breaking` | Niezgodne wstecznie zmiany w publicznym kontrakcie API (`openapi.yaml`) względem gałęzi bazowej (oasdiff) — generuje `openapiBreaking=N`; pomijane, jeśli brakuje oasdiff lub nie można rozpoznać specyfikacji bazowej | **Doradczy** |

### Zadanie: `docs-sync-strict`

Uruchamiane przy każdym PR do `main`. W przypadku niepowodzenia blokuje scalenie.
...
| ↳ `check:docs-counts`          | Liczby w tekście (liczba dostawców, liczba migracji itp.) mieszczą się w oknie zapadkowym względem rzeczywistych wartości                                                        | Tak                         |
| ↳ `check:env-doc-sync`         | Każda zmienna środowiskowa z `.env.example` jest udokumentowana w tabeli dokumentacji i odwrotnie                                                                                | Tak                         |
| ↳ `check:deprecated-versions`  | Brak ciągów znaków oznaczających przestarzałe wersje w dokumentacji                                                                                                              | Tak                         |
| ↳ `check:doc-links`            | Wewnętrzne odnośniki markdown w dokumentacji wskazują na istniejące pliki (format `[tekst]`/`(ścieżka)`)                                                                         | Tak                         |
| ↳ `check:fabricated-docs`      | Trasy, zmienne środowiskowe, polecenia CLI, nazwy hooków i ścieżki plików przywołane w dokumentacji istnieją w bazie kodu. Twarda brama z `--strict`; bez tej flagi błąd miękki. | Tak (przez `--strict` w CI) |
| `check:cli-i18n`               | Ciągi znaków poleceń CLI są obecne we wszystkich plikach lokalizacji i18n                                                                                                        | Tak                         |
| `check:openapi-coverage`       | Specyfikacja OpenAPI obejmuje co najmniej określony zapadkowo minimalny poziom rzeczywistych tras                                                                                | Tak                         |
...
| Skrypt                               | Sprawdza                                                                                                                                                                                                                                  | Blokujący    |
| ------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------ |
| `check-ui-keys-coverage` (wbudowany) | Pokrycie kluczy i18n interfejsu użytkownika wynosi ≥ 65%                                                                                                                                                                                  | Tak          |
| `check-ui-value-drift` (wbudowany)   | Zmiana angielskiej **wartości** nie pozostawia żadnego nieaktualnego tłumaczenia                                                                                                                                                          | Tak          |
| `check-new-key-coverage` (wbudowany) | **Nowy** angielski klucz jest przetłumaczony w każdej lokalizacji — znacznik `__MISSING__:` jest odrzucany                                                                                                                                | Tak          |
| `check-translation-ratio`            | Współczynnik rzeczywistych tłumaczeń dla każdej lokalizacji (elementy identyczne z angielskimi / zastępcze / brakujące poza listą dozwolonych) nie może przekraczać wartości z `config/quality/i18n-translation-baseline.json` + margines | **Doradczy** |

Wymaga `fetch-depth: 0` — brama wykrywająca rozbieżności wartości porównuje `en.json` z bazą scalania.

#### `check-ui-value-drift` — brama nieaktualnych tłumaczeń

Wykrywa jedną regresję i18n, której pozostałe bramy z założenia nie potrafią wykryć: angielska wartość
zostaje zmieniona, a tłumaczenia utworzone na podstawie _poprzedniego_ tekstu angielskiego pozostają,
przez co użytkownicy nieanglojęzyczni nadal czytają sformułowany w przekonujący sposób, lecz obecnie błędny tekst.

Ten problem faktycznie trafił do wydania. `oauthModal.googleOAuthWarning` zmieniono po dodaniu narzędzia pomocniczego
logowania Antigravity (#5203); w **39 z 43 lokalizacji** pozostał tekst instruujący operatorów, aby „skopiowali
pełny adres URL i wkleili go poniżej” — przepływ, którego nie da się ukończyć dla tego dostawcy. Problem
...
- `check-translation-drift` śledzi kopie dokumentacji `docs/i18n/<locale>/**.md` —
  nigdy nie odczytuje `src/i18n/messages/*.json`. Blokuje w zadaniu `docs-sync-strict` od ponownej
  synchronizacji 2026-09: zmień główny dokument → `npm run i18n:run -- --files=<doc>` (na poziomie sekcji, niewielki koszt).

**Uwzględnia różnice, bez bazowego pliku odniesienia.** Porównuje `en.json` w bazie scalania
z drzewem roboczym; dla każdego klucza, którego angielska wartość uległa zmianie, każde
tłumaczenie nadal zawierające niezmienioną wartość jest nieaktualne. To celowo **zamraża
wcześniej istniejący dług** — różnica nie pozwala ustalić, z której starej angielskiej
wersji pochodzi istniejące od dawna tłumaczenie, dlatego bramka ocenia wyłącznie elementy
objęte bieżącą zmianą. Alternatywa (bazowy skrót dla każdego klucza) wymagałaby
wygenerowanego pliku o rozmiarze ~600 KB, 3× większego od największego istniejącego pliku
bazowego, zmieniającego się przy każdym PR dotyczącym i18n.

Można spełnić ten warunek na dwa sposoby:

1. zaktualizować odpowiednie tłumaczenia albo
2. ustawić ich wartości na `__MISSING__:<nowy tekst angielski>` — środowisko uruchomieniowe
   wyświetli wtedy poprawiony tekst angielski (`src/i18n/request.ts::deepMergeFallback`,
   #7258), a klucz zostanie dodany do kolejki tłumaczeń.

Jeśli zmieniło się **znaczenie** ciągu, preferowana jest **zmiana nazwy klucza**: nowy
klucz nie może odziedziczyć nieaktualnego tłumaczenia. Ten wzorzec zastosowano w #8463.
...
Kończy działanie z kodem 0 i komunikatem `SKIP reason=base-unresolved`, gdy nie można
odczytać katalogu bazowego (płytki klon bez bazowego refa), analogicznie do
`check-openapi-breaking`.

### Zadanie: `i18n`

Pełna macierz walidacji i18n (jedno zadanie na język). Całe zadanie ma charakter doradczy.

| Skrypt                          | Co weryfikuje                            | Blokowanie                                                  |
| ------------------------------- | ---------------------------------------- | ----------------------------------------------------------- |
| `validate_translation.py quick` | Kompletność tłumaczeń dla każdego języka | **Doradcze** (`continue-on-error: true` dla całego zadania) |

### Zadanie: `pr-test-policy`

Uruchamiane tylko dla pull requestów.

| Skrypt                 | Co weryfikuje                                                                                                                                               | Blokowanie |
| ---------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------- |
| `check:pr-test-policy` | PR-y zmieniające kod produkcyjny w `src/`, `open-sse/`, `electron/` lub `bin/` muszą zawierać nowe albo zaktualizowane testy (Twarda reguła #8)             | Tak        |
...
| `check:pr-evidence`    | Opis PR-a zawiera dowody z testów/VPS dotyczące zmiany (automatyzuje Twardą regułę #18 przez przeszukiwanie treści PR-a — rozwiązanie kruche, zob. Backlog) | Tak        |

### Zadanie: `test-vitest`

Uruchamiane po `build`. W przypadku niepowodzenia blokuje scalanie.

| Zestaw           | Co weryfikuje                                                          | Blokowanie                                                                                                                      |
| ---------------- | ---------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- |
| `test:vitest`    | Serwer MCP (110 narzędzi), autoCombo, pamięć podręczna — runner vitest | Tak                                                                                                                             |
| `test:vitest:ui` | Testy komponentów interfejsu — runner vitest                           | **Blokujące** — wcześniej istniejące błędy są jawnie wykluczone w `vitest.config.ts`; nowe błędy powodują niepowodzenie zadania |

### Nocne przepływy pracy (zaplanowane, doradcze)

Są uruchamiane według harmonogramu cron (oraz przez `workflow_dispatch`), nigdy dla PR-ów. Wszystkie mają charakter doradczy.

| Przepływ pracy         | Co weryfikuje                                                                                                                                                                        | Blokowanie   |
| ---------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------ |
| `nightly-property`     | Testy właściwości fast-check z losowym ziarnem i dużą liczbą uruchomień                                                                                                              | **Doradcze** |
| `nightly-resilience`   | Bramka wzrostu sterty, wstrzykiwanie awarii typu chaos, testy obciążeniowe/długotrwałe k6                                                                                            | **Doradcze** |
| `nightly-llm-security` | Ochrona przed wstrzyknięciami promptfoo (tryb blokowania) + sondy garak (pomijane bez sekretu dostawcy)                                                                              | **Doradcze** |
| `nightly-schemathesis` | Fuzzing kontraktu OpenAPI (schemathesis) względem działającej usługi OmniRoute z użyciem `docs/openapi.yaml` — ujawnia naruszenia specyfikacji / nieobsłużone błędy 500 (Faza 8 B.4) | **Doradcze** |
| `nightly-mutation`     | Wynik testów mutacyjnych Stryker dla szybkiej ścieżki testów jednostkowych — mutanci, którzy przetrwali, ujawniają słabe asercje                                                     | **Doradcze** |
| `nightly-compat`       | Macierz zgodności wersji Node w obsługiwanych zakresach `engines.node`                                                                                                               | **Doradcze** |
