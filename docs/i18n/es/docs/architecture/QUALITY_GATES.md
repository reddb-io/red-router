...
La fuente de referencia de CI es `.github/workflows/ci.yml`.

### Vía rápida para PRs de release (`quality.yml`)

`.github/workflows/quality.yml` se ejecuta en los PRs dirigidos a `release/**`. Permite que las ramas de los colaboradores
sigan avanzando mediante gates rápidos filtrados por rutas, además de una señal informativa de compilación de producción para los cambios de
código:

| Job                                              | Alcance                                                                                                                                                                                                                                                    | Bloqueante                                                                                                            |
| ------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------- |
| `Build (advisory)`                               | PRs de código que no sean borradores y ramas de cola de Mergify; Node 24, `npm-ci-retry`, `check:node-runtime`, `npm run build` con `OMNIROUTE_USE_TURBOPACK=1`; no se cargan artefactos porque ningún job de calidad posterior los consume                | **Informativo** (`continue-on-error: true`; eliminar después de una semana de ejecuciones estables en PRs de release) |
| `Docs Gates (fast-path)`                         | PRs de documentación/código; referencias de documentación de la API y docs-all                                                                                                                                                                             | Sí                                                                                                                    |
| `Fast Quality Gates`                             | PRs de código; comprobaciones estáticas, comprobación de tipos, comprobación de tipos del dashboard y pruebas unitarias afectadas                                                                                                                          | Sí                                                                                                                    |
| `Forgotten sibling tests`                        | PRs de código; módulos modificados rastreados hasta consumidores estáticos y posibles pruebas hermanas; las rutas de barrel e importación dinámica se notifican como diagnósticos informativos, con referencia a las excepciones de la lista de permitidos | **Informativo**                                                                                                       |
| `Vitest (fast-path)`                             | PRs de código; suite rápida de Vitest                                                                                                                                                                                                                      | Sí                                                                                                                    |
| `Unit Tests fast-path`                           | PRs de código; suite de pruebas unitarias en 4 shards                                                                                                                                                                                                      | Sí                                                                                                                    |
| `No new ESLint warnings`                         | PRs de código; protección de lint que tiene en cuenta las supresiones                                                                                                                                                                                      | Sí para el mismo origen, informativo para forks                                                                       |
| `Merge integrity (changelog + generated skills)` | PRs que no sean borradores; sincronización del changelog y de las skills generadas                                                                                                                                                                         | Sí para el mismo origen, informativo para forks                                                                       |
...
Para cada módulo de producción modificado, informa de cadenas deterministas
...
artefacto `forgotten-sibling-tests` del workflow para su calibración antes de cualquier implementación bloqueante.
...
candidata, proporcionar una justificación específica y enlazar a un issue o pull request de GitHub. Las entradas con formato incorrecto
provocan un fallo por defecto. Las excepciones no pueden suprimir una prueba candidata eliminada ni un diff que añada `.skip`/`.todo`;
el debilitamiento de aserciones y otros tipos de enmascaramiento siguen siendo responsabilidad del gate bloqueante e independiente
`check:test-masking`.

### Job: `lint`

Se ejecuta en cada PR dirigido a `main`. Bloquea la fusión si falla.

| Script (`npm run ...`)            | Valida                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | Bloqueante                                  |
| --------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------- |
| `check:node-runtime`              | La versión de Node.js se encuentra dentro del rango compatible                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | Sí                                          |
| `check:cycles`                    | Importaciones circulares — todos los módulos de `src/` + `open-sse/`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | Sí                                          |
| `check:route-validation:t06`      | Los esquemas de Zod están presentes en todas las rutas (política de nivel 6)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | Sí                                          |
| `check:any-budget:t11`            | El número de `@ts-expect-error // any` no supera el límite (catraca de nivel 11)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | Sí                                          |
| `check:provider-consistency`      | Cada proveedor en `providers.ts` tiene una entrada correspondiente en `providerRegistry.ts` (y viceversa, dentro de la lista de permitidos)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | Sí                                          |
| `check:model-lifecycle`           | Las tres tablas de enrutamiento mantenidas manualmente permanecen coherentes con la instantánea del ciclo de vida incluida en el repositorio (#11503): `FITNESS_TABLE` (`taskFitness.ts`) no puntúa ningún id retirado que `REGISTRY` pueda enrutar; cada destino de `BUILT_IN_ALIASES` está presente en `REGISTRY` y ausente de la instantánea de ids retirados; cada id retirado que aún esté en `REGISTRY` se redirige o figura en `allowedRetiredInCatalog`; y ninguna fuente ni destino de `DEFAULT_DEGRADATION_MAP` aparece como retirado en esa instantánea. Esto no demuestra que un modelo sea servido actualmente por un upstream activo. Sin conexión: compara con `config/quality/model-lifecycle.json`, que se actualiza manualmente con `npm run quality:refresh-model-lifecycle` (requiere red; no está integrado en CI). `allowedRetiredInCatalog` es un mecanismo de reducción progresiva: añade una entrada únicamente con una incidencia de seguimiento. | Sí                                          |
| `check:fetch-targets`             | Cada `fetch("/api/...")` en el código del cliente dentro de `src/` se resuelve a un `route.ts` real                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | Sí                                          |
| `check:deps`                      | Todas las dependencias instalables mediante `npm install` de cada `package.json` del repositorio están en `dependency-allowlist.json`; se marcan los paquetes nuevos sin versión fijada o que podrían ser objeto de slopsquatting                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | Sí                                          |
| `audit:deps`                      | `npm audit` (raíz + electron): sin avisos de gravedad alta o crítica (se solapa con `check:vuln-ratchet` de osv; consulta el Backlog de racionalización)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | Sí                                          |
...
| `check:ai-attribution`            | Sin pie `Co-Authored-By` de IA/bot ni pie de generación por IA en los commits, el título o el cuerpo del PR — Regla estricta n.º 16 (en el bucle de comprobaciones rápidas de `quality.yml` para PR→`release/**` — lee la carga útil del evento y no realiza ninguna acción fuera de los PR — y en un paso exclusivo para PR en el lint de `ci.yml` para PR→`main`; también en el hook `commit-msg` de husky; se permiten coautores humanos; #14436)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `check:vitest-exclusions`         | Cada exclusión de Vitest menciona una incidencia de seguimiento y aparece en `config/quality/vitest-exclusions.json` (#13204)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | Sí                                          |
...
| `check:migration-numbering`       | Los archivos SQL de migración están numerados secuencialmente, sin saltos ni duplicados                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | Sí                                          |
...
| `check:db-rules`                  | No hay SQL sin procesar fuera de los módulos de `src/lib/db/`; no hay importaciones de barril desde `localDb.ts` (reglas estrictas n.º 2 y n.º 5)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | Sí                                          |
...
| `check:route-guard-membership`    | Todas las rutas que generan un proceso secundario están clasificadas mediante `isLocalOnlyPath()` (reglas estrictas n.º 15 y n.º 17)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | Sí                                          |
| `check:test-discovery`            | Todos los archivos `*.test.ts` / `*.spec.ts` del repositorio son recopilados por al menos un ejecutor de pruebas (mecanismo de trinquete: la lista de archivos huérfanos de `test-discovery-baseline.json` solo puede reducirse)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | Sí                                          |
| `check:agent-skills-sync`         | Los artefactos generados de agent-skills coinciden con su catálogo de origen (sin desviaciones)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| `check:dashboard-typecheck`       | `tsc` limitado a `src/app/(dashboard)/**` (#7033): la lista de permitidos seleccionada de 27 archivos de `typecheck:core` no incluye ningún TSX del panel, y `next build` tampoco comprueba sus tipos (`next.config.mjs` establece `ignoreBuildErrors: true`), por lo que las regresiones de identificadores huérfanos allí (#6625/#6909) eran invisibles para CI. Compara con una línea base fija del recuento por archivo y por código TS (`config/quality/dashboard-typecheck-baseline.json`, el mismo patrón de aplicación de obsolescencia que `check:known-symbols`): solo los errores NUEVOS que superen el recuento de la línea base hacen que falle la comprobación; redúzcala con `--update` cuando se corrija un error preexistente.                                                                                                                                                                                                                             | Sí                                          |
...
| ---------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------- |
| `quality:collect`            | Genera `quality-metrics.json` (número de advertencias de ESLint, cobertura del informe combinado de fragmentos)                                                                                                      | Sí (previo al ratchet) |
...
| `check:cognitive-complexity` | Ratchet de complejidad cognitiva (`eslint-plugin-sonarjs`): pasada de ESLint independiente; CI ejecuta ambas combinadas en el único paso `check:complexity-ratchets`                                                 | Sí                     |
| `check:dead-code`            | El ratchet de exportaciones/archivos sin usar (knip) no empeora con respecto a la línea base                                                                                                                         | Sí                     |
| `check:compression-budget`   | Presupuesto del benchmark de compresión: los valores mínimos de ahorro de tokens por motor no deben empeorar                                                                                                         | Sí                     |
...
El trabajo completo es informativo (`continue-on-error: true`). Los ratchets basados en npm se ejecutan
realmente; los escáneres externos se instalan mediante `gh release download` y se omiten automáticamente (código de salida 0)
si un binario sigue sin estar presente.

| Script                   | Valida                                                                                                                                                                                                                        | Bloqueante      |
| ------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------- |
| `check:circular-deps`    | No hay dependencias circulares (dpdm)                                                                                                                                                                                         | **Informativo** |
| `check:bundle-size`      | El tamaño del bundle no supera el límite                                                                                                                                                                                      | **Informativo** |
| `check:secrets`          | Análisis de secretos (gitleaks): se omite si el binario no está presente                                                                                                                                                      | **Informativo** |
| `check:vuln-ratchet`     | Las vulnerabilidades de las dependencias (osv-scanner) no empeoran; se omite si el binario no está presente                                                                                                                   | **Informativo** |
| `check:workflows`        | Lint de workflows (actionlint + zizmor): se omite si los binarios no están presentes                                                                                                                                          | **Informativo** |
| `check:openapi-breaking` | Cambios incompatibles en el contrato de la API pública (`openapi.yaml`) con respecto a la rama base (oasdiff): genera `openapiBreaking=N`; se omite si oasdiff no está presente o no se puede resolver la especificación base | **Informativo** |

### Trabajo: `docs-sync-strict`

Se ejecuta en cada PR dirigida a `main`. Bloquea la fusión si falla.

| Script                         | Valida                                                                                                                                                                                                                 | Bloqueante                     |
| ------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------ |
| `check:docs-all`               | Metapuerta que ejecuta secuencialmente las 6 puertas secundarias indicadas a continuación                                                                                                                              | Sí                             |
| ↳ `check:docs-sync`            | Coherencia de versiones entre CHANGELOG / OpenAPI / llm.txt                                                                                                                                                            | Sí                             |
| ↳ `check:docs-counts`          | Los recuentos en el texto (cantidad de proveedores, cantidad de migraciones, etc.) están dentro de la ventana de ajuste progresivo de los recuentos reales                                                             | Sí                             |
| ↳ `check:env-doc-sync`         | Cada variable de entorno de `.env.example` está documentada en una tabla de la documentación, y viceversa                                                                                                              | Sí                             |
| ↳ `check:deprecated-versions`  | No hay cadenas de versiones obsoletas en la documentación                                                                                                                                                              | Sí                             |
| ↳ `check:doc-links`            | Los enlaces markdown internos de la documentación apuntan a archivos reales (formato `[texto]`/`(ruta)`)                                                                                                               | Sí                             |
| ↳ `check:fabricated-docs`      | Las rutas, variables de entorno, comandos de CLI, nombres de hooks y rutas de archivo citados en la documentación existen en el código fuente. Puerta estricta mediante `--strict`; fallo no bloqueante sin la opción. | Sí (mediante `--strict` en CI) |
| `check:cli-i18n`               | Las cadenas de comandos de la CLI están presentes en todos los archivos de configuración regional de i18n                                                                                                              | Sí                             |
| `check:openapi-coverage`       | La especificación OpenAPI cubre al menos un mínimo ajustado progresivamente de las rutas reales                                                                                                                        | Sí                             |
| `check:openapi-security-tiers` | Las anotaciones de niveles de seguridad de `openapi.yaml` son coherentes con las clasificaciones de `routeGuard.ts`                                                                                                    | **Informativo**                |
| `check:openapi-routes`         | Cada ruta de `openapi.yaml` apunta a un archivo `route.ts` real (contra alucinaciones)                                                                                                                                 | Sí                             |
| `check:docs-symbols`           | Cada referencia `/api/...` de `docs/**/*.md` apunta a un archivo `route.ts` real (contra alucinaciones)                                                                                                                | Sí                             |
| `i18n translation drift`       | Claves sin traducir en los archivos de configuración regional de i18n; solo se emite una advertencia                                                                                                                   | **Informativo**                |

### Trabajo: `i18n-ui-coverage`

| Script                              | Valida                                                                                                                                                                                                                                               | Bloqueante      |
| ----------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------- |
| `check-ui-keys-coverage` (en línea) | La cobertura de claves de i18n de la interfaz de usuario es ≥ 65 %                                                                                                                                                                                   | Sí              |
| `check-ui-value-drift` (en línea)   | Un **valor** en inglés reescrito no deja traducciones obsoletas                                                                                                                                                                                      | Sí              |
| `check-new-key-coverage` (en línea) | Una clave en inglés **nueva** está traducida en cada configuración regional; se rechaza un marcador `__MISSING__:`                                                                                                                                   | Sí              |
| `check-translation-ratio`           | La proporción de traducciones reales por configuración regional (valores idénticos al inglés, marcadores de posición o elementos faltantes fuera de la lista de permitidos) no debe superar `config/quality/i18n-translation-baseline.json` + margen | **Informativo** |
...
Detecta la única regresión de i18n que las demás puertas no pueden detectar estructuralmente: se reescribe un valor en inglés
y las traducciones derivadas del inglés _anterior_ permanecen, por lo que
los usuarios que no usan inglés siguen leyendo un texto formulado con seguridad, pero ahora incorrecto.

Esto llegó realmente a producción. `oauthModal.googleOAuthWarning` se reescribió cuando se incorporó el asistente
de inicio de sesión de Antigravity (#5203); **39 de 43 configuraciones regionales** conservaron un texto que indicaba a los operadores que debían «copiar la
URL completa y pegarla a continuación», un flujo que no puede completarse con ese proveedor. Pasó
desapercibido hasta #8463 porque:

- `sync-ui-keys` solo rellena las claves que están **ausentes**, nunca las que están **obsoletas**;
- `check-ui-keys-coverage` cuenta la _presencia_ de claves, por lo que una traducción obsoleta se considera cubierta;
- `check-translation-drift` realiza el seguimiento de los reflejos de documentación de `docs/i18n/<locale>/**.md`;
...
**Consciente del diff, no respaldado por una línea base.** Compara `en.json` en la base de fusión con el
...
traducción intacta está obsoleta. Esto **congela deliberadamente la deuda preexistente**: un diff
no puede revelar de qué versión antigua del texto en inglés procede una traducción existente desde hace tiempo, por lo que la comprobación evalúa
únicamente lo que afecta el cambio actual. La alternativa (una línea base de hashes por clave) costaría
un archivo generado de ~600 KB, 3 veces más grande que la mayor línea base existente, y cambiaría en cada PR de i18n.

Hay dos formas de satisfacerla:

1. actualizar las traducciones afectadas, o
2. establecerlas en `__MISSING__:<nuevo texto en inglés>`; en ese caso, el entorno de ejecución sirve el texto en inglés corregido
   (`src/i18n/request.ts::deepMergeFallback`, #7258) y la clave queda en cola para su traducción.

Si cambió el **significado** de la cadena, es preferible **renombrar la clave**: una clave nueva no puede heredar
una traducción obsoleta. Ese es el patrón que utilizó #8463.

```bash
npm run i18n:check-value-drift          # estricto (lo que ejecuta CI)
npm run i18n:check-value-drift:warn     # solo informa
BASE_REF=origin/release/vX.Y.Z npm run i18n:check-value-drift
```

Finaliza con código 0 y `SKIP reason=base-unresolved` cuando no se puede leer el catálogo base (un clon
superficial sin la referencia base), al igual que `check-openapi-breaking`.

### Trabajo: `i18n`

Matriz completa de validación de i18n (un trabajo por configuración regional). El trabajo completo es informativo.

| Script                          | Valida                                                 | Bloqueo                                                        |
| ------------------------------- | ------------------------------------------------------ | -------------------------------------------------------------- |
...
| Script                 | Valida                                                                                                                                                                          | Bloqueo |
| ---------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------- |
| `check:pr-test-policy` | Las PR que cambian código de producción en `src/`, `open-sse/`, `electron/` o `bin/` deben incluir o actualizar pruebas (Regla estricta n.º 8)                                  | Sí      |
...
| `check:pr-evidence`    | El cuerpo de la PR cita pruebas o evidencias de VPS para el cambio (automatiza la Regla estricta n.º 18 buscando en el texto de la PR; es frágil, véase la lista de pendientes) | Sí      |

### Trabajo: `test-vitest`

Se ejecuta después de `build`. Bloquea la fusión si falla.

| Suite            | Valida                                                              | Bloqueo                                                                                                                              |
| ---------------- | ------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ |
| `test:vitest`    | Servidor MCP (110 herramientas), autoCombo, caché — ejecutor vitest | Sí                                                                                                                                   |
| `test:vitest:ui` | Pruebas de componentes de IU — ejecutor vitest                      | **Bloqueante**: los fallos preexistentes se excluyen explícitamente en `vitest.config.ts`; los fallos nuevos hacen fallar el trabajo |
...
| Flujo de trabajo       | Valida                                                                                                                                                                                                 | Bloqueo         |
| ---------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------- |
| `nightly-property`     | Pruebas de propiedades de fast-check con una semilla aleatoria y un número elevado de ejecuciones                                                                                                      | **Informativo** |
| `nightly-resilience`   | Comprobación del crecimiento del heap, inyección caótica de fallos y pruebas de carga/resistencia con k6                                                                                               | **Informativo** |
| `nightly-llm-security` | Protección contra inyección de promptfoo (modo de bloqueo) y sondas de garak (se omiten sin un secreto del proveedor)                                                                                  | **Informativo** |
| `nightly-schemathesis` | Fuzzing de contratos OpenAPI (schemathesis) contra una instancia activa de OmniRoute mediante `docs/openapi.yaml`; detecta infracciones de la especificación y errores 500 no controlados (Fase 8 B.4) | **Informativo** |
...
| `nightly-compat`       | Matriz de compatibilidad del motor de Node para los rangos compatibles de `engines.node`                                                                                                               | **Informativo** |
