...
რეპოზიტორია მოიცავს მრავალეტაპიან Dockerfile-ს (`Dockerfile`). ხელმისაწვდომია ოთხი ეტაპი; თქვენი გამოყენების შემთხვევისთვის აირჩიეთ შესაბამისი `target`.

| ეტაპი         | საბაზო იმიჯი          | დანიშნულება                                                                                                                                                                                                                                                                                                                           |
| ------------- | --------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
| `runner-base` | `node:26-trixie-slim` | საწარმოო გაშვების გარემო Next.js-ის დამოუკიდებელი შედეგით. **პროვაიდერების CLI-ები არ მოჰყვება.**                                                                                                                                                                                                                                     |
...
| `runner-web`  | `runner-base`         | ამატებს Playwright-სა და Chromium ბრაუზერს (`--with-deps`) ვებსესიის პროვაიდერებისთვის: `gemini-web`, `claude-web`, `claude-turnstile`. **აირჩიეთ ეს, როდესაც აღნიშნულ პროვაიდერებს იყენებთ** — ჩვეულებრივი იმიჯი მის გარეშე მოთხოვნის შესრულებისას შეცდომით დასრულდება (იხილეთ `-web`-ის შენიშვნა გამოშვების არხების განყოფილებაში). |
...
სამი აგების არგუმენტი აკონტროლებს `builder` ეტაპის რესურსულ დანახარჯს. ისინი მხოლოდ აგების დროს გამოიყენება —
`OMNIROUTE_MEMORY_MB` (ქვემოთ) ცალკე პარამეტრია გაშვების დროისთვის.

| აგების არგუმენტი            | ნაგულისხმევი მნიშვნელობა | ეფექტი                                                                                                                  |
| --------------------------- | ------------------------ | ----------------------------------------------------------------------------------------------------------------------- |
| `OMNIROUTE_USE_TURBOPACK`   | `1`                      | `0` სანაცვლოდ webpack-ით აგებს. მეხსიერების ნაკლები პიკური მოხმარება, თუმცა უფრო ნელია.                                 |
| `OMNIROUTE_BUILD_MEMORY_MB` | `6144`                   | V8 heap-ის ზედა ზღვარი (`--max-old-space-size`) გაშვებული `next build`-ისთვის.                                          |
| `OMNIROUTE_BUILD_WORKERS`   | `2`                      | მნიშვნელობას გადასცემს `CIRCLE_NODE_TOTAL`-ს; გვერდის მონაცემების შეგროვებისთვის Next განსაზღვრავს `workers = N - 1`-ს. |

`OMNIROUTE_BUILD_WORKERS` არის პარამეტრი, რომელიც მძლავრ ამგებზე უნდა გაზარდოთ და
რომელიც უნდა შეამოწმოთ, როდესაც შეზღუდული რესურსების მქონე აგება **`✓ Compiled successfully`-ის შემდეგ**
წყდება. გვერდის მონაცემების თითოეული worker ცალკე პროცესია, ისევე როგორც მშობელი
`next build`; VPS-ზე რეალურ პირობებში ჩატარებულმა გამეორებამ (issue #7518) აჩვენა,
რომ თითოეული პროცესის პიკური RSS დაახლოებით 4.5 GB იყო, `NODE_OPTIONS`-ის heap-ის
ალმისგან დამოუკიდებლად (Turbopack კომპილაციას V8 heap-ის გარეთ, ნატიურ/Rust
მეხსიერებაში ასრულებს). ნაგულისხმევი მნიშვნელობა `2` (→ 1 worker, ჯამში 2
პროცესი) გათვლილია 16 GB / 4 vCPU GitHub-ის მიერ ჰოსტირებულ runner-ებზე,
რომლებსაც გამოქვეყნების pipeline იყენებს. `8`-ის შემთხვევაში (→ 7 worker)
ამ runner-ს მეხსიერება ამოეწურა და buildkit-მა ეტაპი დაასრულა შეცდომით
`ResourceExhausted: ... cannot allocate memory`; `3`-იც (→ 2 worker) კი
ვერ ჩაეტია მას შემდეგ, რაც თითოეული პროცესის RSS პირდაპირ გაიზომა და არა
არაპირდაპირ გამოითვალა. `tests/unit/docker-build-memory-budget.test.ts`
გაზომილი მაჩვენებლის საფუძველზე ასრულებს გამოთვლებს და შეცდომით სრულდება, თუ
რომელიმე პარამეტრი runner-ის შესაძლებლობებს აჭარბებს.

Turbopack კომპილაციას ასრულებს ნატიურ Rust მეხსიერებაში, რომელიც V8 heap-ის
**გარეთაა**, ამიტომ `OMNIROUTE_BUILD_MEMORY_MB` მას არ ზღუდავს. მეხსიერების
ზღვრის მქონე ჰოსტზე აგებას OOM killer ტექსტური შეცდომის გარეშე SIGKILL-ით
წყვეტს — ის უბრალოდ ჩერდება `Creating an optimized production build`-ის
შუაში, რაც მეხსიერების ამოწურვის ნაცვლად გაჭედვას ჰგავს. თუ აგების ჰოსტს
...
`webpackBuildWorker` ჩართულია, ამიტომ `next build` უშვებს როგორც მშობელ, ასევე
worker პროცესს და თითოეული მათგანი ცალ-ცალკე ითვალისწინებს
`OMNIROUTE_BUILD_MEMORY_MB`-ს. კონტეინერის ზღვარი დააყენეთ დაახლოებით ამ
მნიშვნელობის ორმაგზე მეტი და არა მხოლოდ ერთხელ აღებულ მნიშვნელობაზე.

გაზომილია ამ ხეზე (`--target runner-base`, `OMNIROUTE_BUILD_MEMORY_MB=6144`):

| Bundler   | კონტეინერის ზღვარი | შედეგი                                       |
| --------- | ------------------ | -------------------------------------------- |
| Turbopack | 8 GiB / 16 GiB     | ორივე შემთხვევაში OOM-ით შეწყდა, უხმაუროდ    |
| webpack   | 8 GiB              | აგების worker SIGKILL-ით შეწყდა              |
| webpack   | 12 GiB             | წარმატებით დასრულდა, პიკმა 11.1 GiB შეადგინა |

### გაშვების დროის ნაგულისხმევი მნიშვნელობები
...
- იმიჯი ადგენს `OMNIROUTE_MEMORY_MB=1024`-ს და მისგან ქმნის `NODE_OPTIONS=--max-old-space-size=1024`-ს.
- სერვერის ფაქტობრივ პროცესს უშვებს დამოუკიდებელი გამშვები, რომელიც კითხულობს `OMNIROUTE_MEMORY_MB`-ს და ამატებს `--max-old-space-size=<OMNIROUTE_MEMORY_MB>`-ს.
- Node იყენებს განმეორებით მითითებული `--max-old-space-size`-ის ბოლო მნიშვნელობას, ამიტომ `OMNIROUTE_MEMORY_MB`-ის დაყენება აკონტროლებს Docker-ის heap-ის ეფექტურ ლიმიტს.
- ვინაიდან იმიჯი მას ყოველთვის ადგენს, გამშვების საკუთარი, RAM-ის მიხედვით დაკალიბრებული სარეზერვო მნიშვნელობა Docker-ში არასოდეს გამოიყენება. სამუშაო დატვირთვისთვის მნიშვნელობა პირდაპირ გაზარდეთ (იხილეთ ქვემოთ მოცემული ცხრილი). `2048` მაინც ძალიან მცირეა coding-agent-ის `/v1/responses` მოთხოვნებისთვის.

### სამუშაო დროის RAM coding agent-ებისთვის

Docker-ის ნაგულისხმევი 1 GiB განკუთვნილია მხოლოდ dashboard-ისა და მსუბუქი ჩატისთვის და არა საწარმოო გარემოსთვის. გრძელი `POST /v1/responses` სხეულები (ასობით შეტყობინება, ათობით ხელსაწყო) შეკუმშვის დროს მეხსიერებაში რამდენიმე გრაფს ერთდროულად ინარჩუნებს. ორმა ერთმანეთთან გადაფარულმა, დაახლოებით 3 MiB / დაახლოებით 750k-token მოთხოვნამ V8-ის ავარიული შეწყვეტა გამოიწვია **12 GiB** old-space-ზე (`FATAL ERROR: Reached heap limit`) და ასევე მიაღწია 16 GiB cgroup-ის OOM ლიმიტს. იხილეთ [#7849](https://github.com/diegosouzapw/OmniRoute/issues/7849).

cgroup-ის **`--memory` heap-ზე მაღალ მნიშვნელობაზე დააყენეთ** — native buffer-ები, SQLite და შეკუმშვის შუალედური მონაცემები V8-ის გარეთ თავსდება.
...
| ორი პარალელური გრძელი `/v1/responses`    | `10240`–`12288`                          | ≥12–16 GiB              | გაზომვისას V8 ავარიულად შეწყდა დაახლოებით 12 GiB heap-ზე                                                                                   |
| სამი ან მეტი პარალელური გრძელი კონტექსტი | ერთ პროცესზე არ გამოიყენოთ               | სერიალიზაცია / მეტი RAM | მძიმე მოთხოვნებისთვის ნაგულისხმევად დაშვებულია 1 მიმდინარე მოთხოვნა; RAM-ის გაზრდის გარეშე ამ ლიმიტის აწევა კვლავ იწვევს ავარიულ შეწყვეტას |

`omniroute serve` bare metal-ზე აკალიბრებს RAM-ის დაახლოებით 35%-ს (`[512, 4096]` დიაპაზონში), როდესაც `OMNIROUTE_MEMORY_MB` **დაყენებული არ არის**. Docker ყოველთვის ადგენს `1024`-ს, ამიტომ ოფიციალურ იმიჯში ეს კალიბრაცია არასოდეს სრულდება.
