...
> **Para usuários**: Procurando soluções rápidas? Consulte a [Referência rápida](#referência-rápida) abaixo.
...
**Ainda com problemas?** Consulte a [solução de problemas detalhada](#detailed-troubleshooting) abaixo ou peça ajuda no [Discord](https://discord.gg/U47eFqAXCn).
...
2. **`deprecated prebuild-install@7.1.3`** — um utilitário transitivo para obtenção de binários nativos. Ele não é
   usado para instalar o binding de transporte fixado `wreq-js` e não indica que a configuração
   de transporte do provedor de cookies da web falhou.
...
Se uma solicitação do Gemini Web retornar `503` com uma mensagem informando que o Playwright Chromium
...
OmniRoute para que o cache do navegador pertença à mesma instalação do Playwright:
...
Reinicie o OmniRoute após a instalação e tente novamente a solicitação do Gemini Web. Se você
...
| Falha no login / página em branco                          | Verifique a versão do Node.js — consulte [Compatibilidade com o Node.js](#nodejs-compatibility) abaixo                                                                                      |
...
| Docker `curl: (56) Recv failure: Connection reset by peer` | O vínculo de porta do Docker pode estar sendo direcionado ao IPv6. Use `-p 127.0.0.1:20128:20128` para forçar o IPv4 ou teste com `curl -4`. Consulte [IPv6 no Docker](#docker-ipv6) abaixo |
...
| O Kaspersky identifica o aplicativo Desktop como um Trojan | Falso positivo comportamental no instalador não assinado — consulte [Falsos positivos de antivírus](#antivirus-false-positives) abaixo                                                      |
...
O arquivo é uma documentação inerte, sem nenhum conteúdo executável. Você pode restaurá-lo com segurança
da quarentena.
...
2. **Relate o falso positivo** — <https://www.avast.com/false-positive-file-form.php>,
   anexando o `README.md` em quarentena. Essa é a correção que ajuda a todos, pois se trata
   da heurística do fornecedor reagindo de forma excessiva a um arquivo de texto.
...
que avalia o que o instalador _faz_, em vez de compará-lo com malware conhecido. Quando
...
Os arquivos sinalizados são componentes padrão de dependências de código aberto declaradas e incluídas com o
aplicativo para desktop, por exemplo:
...
- `resources/app/.build/next/node_modules/playwright-<hash>/lib/…/agentParser.js` e
  ...
  usada para login em provedores dentro do aplicativo e chat apoiado por navegador.
- `resources/app/.build/next/node_modules/@wreq-js/binding-win32-<arch>-msvc-<hash>/wreq-js.win32-<arch>-msvc.node`
  — o binding nativo `wreq-js` com versão fixada, usado para HTTP com impressão digital de navegador em provedores
...
1. **Verifique primeiro o download** (isso descarta a possibilidade de um arquivo adulterado). Cada versão publica
   ...
   faça o download novamente apenas pela [página de versões do GitHub](https://github.com/diegosouzapw/OmniRoute/releases).
2. **Restaure + exclua** — restaure os itens revertidos da quarentena e adicione uma exclusão
   para `%LOCALAPPDATA%\Programs\OmniRoute` (Kaspersky → Configurações → Ameaças e Exclusões)
   e, em seguida, reinstale.
3. **Relate o falso positivo** — <https://opentip.kaspersky.com/>. Relatos de falsos positivos enviados por usuários
   realmente aceleram a inclusão na lista de permissões.
...
**Causa:** O backend do Kiro impõe uma única sessão ativa por registro de cliente OIDC.
...
A partir da v3.8.0, cada nova conexão do Kiro criada por meio de **Importar token**,
...
As conexões importadas _antes_ da v3.8.0 não têm um registro de cliente
por conexão. Essas conexões continuam usando o endpoint compartilhado de atualização de autenticação social.
Para obter isolamento, exclua a conexão antiga em Painel → Provedores e adicione-a novamente
...
Defina `APP_LOG_TO_FILE=true` no arquivo `.env`. Os logs da aplicação são gravados em `logs/`.
...
- O endpoint de conclusões de chat retorna uma resposta `503` que pode ser repetida, cujo código de erro é
  `chat_admission_busy`.
...
Se a CLI exibir `⚠ Server did not respond within 60s`, mas o servidor estiver
...
continua sendo inicializado em segundo plano e ficará acessível assim que a inicialização for concluída.
...
- **Painel de integridade**: Acesse **Dashboard → Health** para ver o status do sistema em tempo real
