...
### 1.3 Atualize o sistema

```bash
apt update && apt upgrade -y
```

### 1.4 Instale o Docker
...
> **Dica**: Para obter o máximo de segurança, restrinja as portas 80 e 443 apenas aos IPs da Cloudflare. Consulte a seção [Segurança avançada](#advanced-security).

---

## 2. Instale o OmniRoute

### 2.1 Crie o diretório de configuração
...
### 3.1 Gere o certificado SSL (Cloudflare Origin)
...
Mantenha os tempos limite de streaming do proxy reverso alinhados às variáveis de ambiente de tempo limite do OmniRoute. Se você aumentar
...
## 7. Implantar no Cloudflare Workers (Opcional)
...
Consulte também [TUNNELS_GUIDE.md](./TUNNELS_GUIDE.md) para obter o passo a passo do Cloudflare Tunnel incluído no repositório. O worker independente em `omnirouteCloud/` está localizado em um repositório complementar separado.
