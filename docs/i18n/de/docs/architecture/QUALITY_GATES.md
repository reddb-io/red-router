...
`.github/workflows/quality.yml` wird für PRs ausgeführt, deren Ziel `release/**` ist. Der Workflow hält
...
| ------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------- |
| `Build (advisory)`                               | Nicht als Entwurf markierte Code-PRs und Mergify-Queue-Branches; Node 24, `npm-ci-retry`, `check:node-runtime`, `npm run build` mit `OMNIROUTE_USE_TURBOPACK=1`; kein Artefakt-Upload, da kein nachgelagerter Qualitätsjob ihn verwendet                               | **Nicht blockierend** (`continue-on-error: true`; nach einer Woche stabiler Release-PR-Läufe entfernen) |
...
| `Forgotten sibling tests`                        | Code-PRs; geänderte Module werden bis zu statischen Konsumenten und infrage kommenden benachbarten Tests nachverfolgt; Barrel- und Dynamic-Import-Pfade werden als nicht blockierende Diagnosen gemeldet, einschließlich referenzierter Ausnahmen aus der Positivliste | **Nicht blockierend**                                                                                   |
| `Vitest (fast-path)`                             | Code-PRs; schnelle Vitest-Suite                                                                                                                                                                                                                                        | Ja                                                                                                      |
| `Unit Tests fast-path`                           | Code-PRs; Unit-Test-Suite mit 4 Shards                                                                                                                                                                                                                                 | Ja                                                                                                      |
| `No new ESLint warnings`                         | Code-PRs; unterdrückungsabhängiger Lint-Schutz                                                                                                                                                                                                                         | Ja für eigenen Ursprung, nicht blockierend für Forks                                                    |
| `Merge integrity (changelog + generated skills)` | Nicht als Entwurf markierte PRs; Synchronisierung von Changelog und generierten Skills                                                                                                                                                                                 | Ja für eigenen Ursprung, nicht blockierend für Forks                                                    |
...
Wird bei jedem PR auf `main` ausgeführt. Blockiert bei einem Fehlschlag den Merge.
...
| `check:provider-consistency`      | Jeder Provider in `providers.ts` hat einen entsprechenden Eintrag in `providerRegistry.ts` (und umgekehrt, innerhalb der Allowlist)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | Ja                                         |
| `check:model-lifecycle`           | Die drei manuell gepflegten Routingtabellen bleiben mit dem eingecheckten Lifecycle-Snapshot (#11503) konsistent: `FITNESS_TABLE` (`taskFitness.ts`) bewertet keine eingestellte ID, die `REGISTRY` routen kann; jedes Ziel aus `BUILT_IN_ALIASES` ist in `REGISTRY` vorhanden und fehlt im Snapshot der eingestellten IDs; jede eingestellte ID, die noch in `REGISTRY` enthalten ist, wird weitergeleitet oder in `allowedRetiredInCatalog` aufgeführt; und weder Quelle noch Ziel aus `DEFAULT_DEGRADATION_MAP` sind in diesem Snapshot als eingestellt verzeichnet. Dies beweist nicht, dass ein Modell derzeit von einem aktiven Upstream bereitgestellt wird. Offline — Vergleich mit `config/quality/model-lifecycle.json`, das manuell mit `npm run quality:refresh-model-lifecycle` aktualisiert wird (Netzwerkzugriff; nicht in CI eingebunden). `allowedRetiredInCatalog` ist eine schrittweise abzubauende Sperrklinke: Einen Eintrag nur zusammen mit einem Tracking-Issue hinzufügen. | Ja                                         |
| `check:fetch-targets`             | Jeder Aufruf von `fetch("/api/...")` im clientseitigen `src/` wird zu einer vorhandenen `route.ts` aufgelöst                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | Ja                                         |
| `check:deps`                      | Alle mit `npm install` installierbaren Abhängigkeiten in jeder `package.json` im Repository sind in `dependency-allowlist.json` enthalten; neue nicht fixierte oder durch Slopsquatting entstandene Pakete werden gemeldet                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | Ja                                         |
| `audit:deps`                      | `npm audit` (Root + Electron) — keine Warnungen mit hohem/kritischem Schweregrad (überschneidet sich mit OSV-`check:vuln-ratchet`; siehe Rationalisierungs-Backlog)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | Ja                                         |
...
| `check:file-size`                 | Keine Quelldatei überschreitet die Obergrenze der jeweiligen Dateierweiterung (Ratsche: eingefrorene große Dateien in der Liste `frozen`)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | Ja                                         |
| `check:error-helper`              | Fehlerantworten in Executors/Handlern verwenden `buildErrorBody()` / `sanitizeErrorMessage()` (Harte Regel Nr. 12)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | Ja                                         |
| `check:migration-numbering`       | Migration-SQL-Dateien sind fortlaufend nummeriert, ohne Lücken oder Duplikate                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | Ja                                         |
| `check:public-creds`              | Keine literalen OAuth-`client_id`-/`client_secret`- oder Firebase-Webschlüssel außerhalb von `publicCreds.ts` (Strikte Regel Nr. 11)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | Ja                                         |
| `check:db-rules`                  | Kein direktes SQL außerhalb der Module in `src/lib/db/`; keine Barrel-Imports aus `localDb.ts` (Strikte Regeln Nr. 2/Nr. 5)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | Ja                                         |
...
| `check:route-guard-membership`    | Jede Route, die einen untergeordneten Prozess startet, wird durch `isLocalOnlyPath()` klassifiziert (Strikte Regeln Nr. 15/Nr. 17)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | Ja                                         |
| `check:test-discovery`            | Jede `*.test.ts`- / `*.spec.ts`-Datei im Repository wird von mindestens einem Test-Runner erfasst (Ratchet-Prinzip: Die Liste verwaister Dateien in `test-discovery-baseline.json` darf nur kürzer werden)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | Ja                                         |
| `check:agent-skills-sync`         | Generierte Agent-Skills-Artefakte stimmen mit ihrem Quellkatalog überein (keine Abweichung)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| `check:provider-asset-provenance` | Anbieterlogos/-Assets verfügen über einen dokumentierten Herkunftsnachweis                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `lint:json`                       | JSON-Konfigurationsdateien können geparst werden und erfüllen die Lint-Regeln des Repositories                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| `check:type-coverage`        | Ratchet für den typisierten Prozentanteil (`type-coverage`) verschlechtert sich nicht; ersetzt `typecheck:noimplicit:core` weitgehend                                                                   | Ja                           |
| `check:codeql-ratchet`       | Anzahl offener CodeQL-Warnungen verschlechtert sich nicht (liest über `gh api`; kontrolliertes Überspringen ohne Token) — Aktualisierungsintervall und manueller Auslöser: siehe „CodeQL-Ratchet“ unten | Ja                           |

### Job: `quality-extended`

Der gesamte Job ist nur informativ (`continue-on-error: true`). Die npm-basierten Ratchets werden
...
| `check:bundle-size`      | Bundle-Größe überschreitet den Grenzwert nicht                                                                                                                                                                                         | **Nur informativ** |
...
| `check:workflows`        | Workflow-Linting (actionlint + zizmor) — wird übersprungen, wenn Binärdateien fehlen                                                                                                                                                   | **Nur informativ** |
...
| Skript                         | Validiert                                                                                                                                                                                 | Blockierend                |
| ------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------- |
| `check:docs-all`               | Meta-Gate, das die 6 nachfolgenden Sub-Gates nacheinander ausführt                                                                                                                        | Ja                         |
| ↳ `check:docs-sync`            | Versionskonsistenz zwischen CHANGELOG / OpenAPI / llm.txt                                                                                                                                 | Ja                         |
| ↳ `check:docs-counts`          | Zahlen im Fließtext (Anbieteranzahl, Migrationsanzahl usw.) liegen innerhalb des Ratchet-Fensters der tatsächlichen Zahlen                                                                | Ja                         |
...
| ↳ `check:deprecated-versions`  | Keine veralteten Versionsangaben in der Dokumentation                                                                                                                                     | Ja                         |
| ↳ `check:doc-links`            | Interne Markdown-Links in der Dokumentation verweisen auf vorhandene Dateien (Form `[text]`/`(path)`)                                                                                     | Ja                         |
| ↳ `check:fabricated-docs`      | In der Dokumentation erwähnte Routen, Umgebungsvariablen, CLI-Befehle, Hook-Namen und Dateipfade sind in der Codebasis vorhanden. Striktes Gate mit `--strict`; ohne Flag weicher Fehler. | Ja (über `--strict` in CI) |
| `check:cli-i18n`               | Zeichenfolgen für CLI-Befehle sind in allen i18n-Locale-Dateien vorhanden                                                                                                                 | Ja                         |
| `check:openapi-coverage`       | Die OpenAPI-Spezifikation deckt mindestens einen per Ratchet festgelegten Mindestwert der tatsächlichen Routen ab                                                                         | Ja                         |
| `check:openapi-security-tiers` | Sicherheitsebenen-Anmerkungen in `openapi.yaml` stimmen mit den Klassifizierungen in `routeGuard.ts` überein                                                                              | **Hinweis**                |
| `check:openapi-routes`         | Jeder Pfad in `openapi.yaml` verweist auf eine tatsächlich vorhandene `route.ts` (Halluzinationsschutz)                                                                                   | Ja                         |
| `check:docs-symbols`           | Jede `/api/...`-Referenz in `docs/**/*.md` verweist auf eine tatsächlich vorhandene `route.ts` (Halluzinationsschutz)                                                                     | Ja                         |
...
| `check-translation-ratio`         | Der Anteil echter Übersetzungen pro Locale (mit dem Englischen identische / Platzhalter- / fehlende Einträge außerhalb der Positivliste) darf `config/quality/i18n-translation-baseline.json` + Toleranz nicht überschreiten | **Hinweis** |
...
Erkennt die eine i18n-Regression, die die anderen Gates strukturell nicht erkennen können: Ein englischer Wert
wird neu formuliert, während die aus dem _vorherigen_ englischen Text abgeleiteten Übersetzungen bestehen bleiben, sodass
nicht englischsprachige Benutzer weiterhin einen selbstsicher formulierten, aber inzwischen falschen Text lesen.
...
- `check-ui-keys-coverage` die _Anwesenheit_ von Schlüsseln zählt, sodass eine veraltete Übersetzung als abgedeckt gewertet wird;
...
2. sie auf `__MISSING__:<new english>` setzen — zur Laufzeit wird dann der korrigierte englische Text ausgeliefert
   (`src/i18n/request.ts::deepMergeFallback`, #7258) und der Schlüssel zur Übersetzung vorgemerkt.

Wenn sich die **Bedeutung** des Textes geändert hat, sollte der **Schlüssel umbenannt** werden: Ein neuer Schlüssel kann keine
veraltete Übersetzung übernehmen. Dieses Muster wurde in #8463 verwendet.
...
Wird der Basiskatalog nicht gefunden, endet die Prüfung mit Code 0 und `SKIP reason=base-unresolved` (flacher
Klon ohne den Basis-Ref), analog zu `check-openapi-breaking`.

### Job: `i18n`

Vollständige i18n-Validierungsmatrix (ein Job pro Locale). Der gesamte Job ist unverbindlich.

| Skript                          | Validiert                                 | Blockierend                                                        |
| ------------------------------- | ----------------------------------------- | ------------------------------------------------------------------ |
| `validate_translation.py quick` | Vollständigkeit der Übersetzung je Locale | **Unverbindlich** (`continue-on-error: true` für den gesamten Job) |
...
| `test:vitest:ui` | UI-Komponententests — vitest-Test-Runner                      | **Blockierend** — bereits bestehende Fehler sind in `vitest.config.ts` ausdrücklich ausgeschlossen; neue Fehler lassen den Job fehlschlagen |

### Nächtliche Workflows (geplant, unverbindlich)

Diese werden nach einem Cron-Zeitplan (und über `workflow_dispatch`) ausgeführt, niemals bei PRs. Sie sind alle unverbindlich.

| Workflow               | Validiert                                                                                                                                                                                         | Blockierend       |
| ---------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------- |
| `nightly-property`     | fast-check-Property-Tests mit zufälligem Seed und hoher Anzahl von Durchläufen                                                                                                                    | **Unverbindlich** |
| `nightly-resilience`   | Heap-Wachstums-Gate, Chaos-Fehlerinjektion, k6-Last-/Dauertests                                                                                                                                   | **Unverbindlich** |
| `nightly-llm-security` | promptfoo-Injection-Guard (Blockiermodus) + garak-Probes (werden ohne Provider-Secret übersprungen)                                                                                               | **Unverbindlich** |
| `nightly-schemathesis` | OpenAPI-Vertrags-Fuzzing (schemathesis) gegen eine aktive OmniRoute-Instanz unter Verwendung von `docs/openapi.yaml` — deckt Spezifikationsverletzungen/unbehandelte 500-Fehler auf (Phase 8 B.4) | **Unverbindlich** |
| `nightly-mutation`     | Stryker-Mutationstestbewertung für die schnelle Unit-Test-Strecke — überlebende Mutanten zeigen schwache Assertions auf                                                                           | **Unverbindlich** |
| `nightly-compat`       | Kompatibilitätsmatrix der Node-Engine über die unterstützten `engines.node`-Bereiche hinweg                                                                                                       | **Unverbindlich** |
