...
## Инвентаризация проверок (~90 скриптов)

Скрипты находятся в `scripts/check/` (проверки политик) и `scripts/quality/` (механизм трещотки).
Единственным источником истины для CI является `.github/workflows/ci.yml`.

### Ускоренный путь для PR в релизные ветки (`quality.yml`)

`.github/workflows/quality.yml` запускается для PR, нацеленных на `release/**`. Он позволяет
не задерживать работу в ветках участников благодаря быстрым проверкам с фильтрацией по путям,
а также предоставляет один рекомендательный сигнал производственной сборки для изменений кода:

| Задача                                           | Область действия                                                                                                                                                                                                                                     | Блокировка                                                                                                       |
| ------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------- |
| `Build (advisory)`                               | Не черновые PR с кодом и ветки очереди Mergify; Node 24, `npm-ci-retry`, `check:node-runtime`, `npm run build` с `OMNIROUTE_USE_TURBOPACK=1`; артефакт не загружается, поскольку ни одна последующая задача контроля качества его не использует      | **Рекомендательная** (`continue-on-error: true`; удалить после одной недели стабильных запусков для релизных PR) |
...
| `Forgotten sibling tests`                        | PR с кодом; отслеживание изменённых модулей до статических потребителей и потенциальных соседних тестов; пути через barrel-экспорты и динамические импорты выводятся как рекомендательная диагностика с указанными исключениями из списка разрешений | **Рекомендательная**                                                                                             |
| `Vitest (fast-path)`                             | PR с кодом; быстрый набор тестов vitest                                                                                                                                                                                                              | Да                                                                                                               |
| `Unit Tests fast-path`                           | PR с кодом; набор модульных тестов в 4 шардах                                                                                                                                                                                                        | Да                                                                                                               |
| `No new ESLint warnings`                         | PR с кодом; проверка линтинга с учётом подавлений                                                                                                                                                                                                    | Да для PR из этого репозитория, рекомендательная для форков                                                      |
| `Merge integrity (changelog + generated skills)` | Не черновые PR; синхронизация журнала изменений и сгенерированных навыков                                                                                                                                                                            | Да для PR из этого репозитория, рекомендательная для форков                                                      |
...
Для каждого изменённого производственного модуля он выводит детерминированные цепочки
`изменённый модуль/символ -> статический потребитель -> потенциальный соседний тест`, если потенциальный
тест отсутствует в изменениях pull request. Сводка Markdown и результат JSON сохраняются как
артефакт рабочего процесса `forgotten-sibling-tests` для калибровки перед возможным переводом проверки в блокирующий режим.

Barrel-реэкспорты и динамические импорты используются только для диагностики разрешения; они никогда не создают
блокирующее замечание. Проверенные исключения находятся в
`config/quality/forgotten-sibling-allowlist.json`. Каждая запись должна указывать потребителя и потенциальный
тест, содержать конкретное обоснование и ссылку на задачу GitHub или pull request. Некорректные записи приводят
к блокирующему отказу. Исключения не могут подавить удаление потенциального теста или изменение, добавляющее `.skip`/`.todo`;
ослабление утверждений и другие способы маскировки остаются в зоне ответственности независимо блокирующей
проверки `check:test-masking`.
...
| `check:model-lifecycle`           | Три поддерживаемые вручную таблицы маршрутизации остаются согласованными с зафиксированным в репозитории снимком жизненного цикла (#11503): `FITNESS_TABLE` (`taskFitness.ts`) не оценивает ни один выведенный из эксплуатации идентификатор, который может маршрутизировать `REGISTRY`; каждая цель `BUILT_IN_ALIASES` присутствует в `REGISTRY` и отсутствует в снимке выведенных из эксплуатации идентификаторов; каждый выведенный из эксплуатации идентификатор, всё ещё находящийся в `REGISTRY`, перенаправляется или указан в `allowedRetiredInCatalog`; ни один источник или цель `DEFAULT_DEGRADATION_MAP` не помечены в этом снимке как выведенные из эксплуатации. Это не доказывает, что модель в настоящее время обслуживается действующим внешним сервисом. Проверка выполняется автономно — путём сравнения с `config/quality/model-lifecycle.json`, который обновляется вручную с помощью `npm run quality:refresh-model-lifecycle` (требуется сеть; не подключено к CI). `allowedRetiredInCatalog` — это механизм постепенного сокращения списка: добавляйте запись только при наличии задачи для её отслеживания. | Да                                               |
| `check:fetch-targets`             | Каждый вызов `fetch("/api/...")` в клиентской части `src/` разрешается в реально существующий `route.ts`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | Да                                               |
| `check:deps`                      | Все зависимости, устанавливаемые через `npm install`, во всех файлах `package.json` репозитория присутствуют в `dependency-allowlist.json`; новые пакеты с незакреплёнными версиями или подозрительными именами, имитирующими известные пакеты, помечаются                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | Да                                               |
| `audit:deps`                      | `npm audit` (корневой проект + Electron) — отсутствие предупреждений высокой/критической степени опасности (частично дублирует OSV-проверку `check:vuln-ratchet`; см. список задач по рационализации)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | Да                                               |
| `check:lockfile`                  | Целостность `package-lock.json` — реестр HTTPS, хеши целостности, отсутствие переопределений хостов                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | Да                                               |
| `check:licenses`                  | Список разрешённых лицензий SPDX для производственных зависимостей                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | Да                                               |
| `check:tracked-artifacts`         | Отсутствие артефактов сборки / зафиксированных символических ссылок `node_modules` (также запускается в husky pre-commit; pre-push намеренно сделан лёгким — #6716)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | Да                                               |
| `check:ai-attribution`            | Отсутствие трейлера `Co-Authored-By` от ИИ/бота или примечания о генерации с помощью ИИ в коммитах, заголовке или описании PR — жёсткое правило №16 (в цикле быстрых проверок `quality.yml` для PR→`release/**` — читает полезную нагрузку события и ничего не делает вне PR — и отдельный шаг только для PR в проверке lint файла `ci.yml` для PR→`main`; также хук husky `commit-msg`; соавторы-люди разрешены; #14436)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| `check:vitest-exclusions`         | Каждое исключение Vitest содержит ссылку на отслеживаемую задачу и присутствует в `config/quality/vitest-exclusions.json` (#13204)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | Да                                               |
...
| `check:migration-numbering`       | SQL-файлы миграций имеют последовательную нумерацию, без пропусков и дубликатов                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | Да                                               |
| `check:public-creds`              | Отсутствуют литеральные значения OAuth `client_id`/`client_secret` и веб-ключи Firebase за пределами `publicCreds.ts` (жёсткое правило №11)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | Да                                               |
...
| `check:known-symbols`             | Исполнители провайдеров, стратегии маршрутизации и трансляторы, зарегистрированные в соответствующих таблицах диспетчеризации, соответствуют файлам на диске — нет потерянных или необъявленных символов                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | Да                                               |
...
| `check:agent-skills-sync`         | Сгенерированные артефакты agent-skills соответствуют исходному каталогу (без расхождений)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| `check:provider-asset-provenance` | Для логотипов/ресурсов провайдеров зафиксирована информация о происхождении                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| `typecheck:noimplicit:core`       | Строгий режим `noImplicitAny` — задел на будущее; многие существующие места вызова всё ещё требуют аннотаций                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | **Рекомендательный** (`continue-on-error: true`) |
| `check:dashboard-typecheck`       | `tsc`, ограниченный областью `src/app/(dashboard)/**` (#7033), — подготовленный список разрешённых 27 файлов в `typecheck:core` не включает ни одного dashboard TSX, а `next build` также никогда не проверяет их типы (`next.config.mjs` задаёт `ignoreBuildErrors: true`), поэтому регрессии с необъявленными идентификаторами в них (#6625/#6909) были невидимы для CI. Выполняется сравнение с зафиксированной базовой линией количества ошибок по каждому файлу и коду TS (`config/quality/dashboard-typecheck-baseline.json`, с той же схемой контроля устаревания, что и в `check:known-symbols`): проверка завершается неудачно только при появлении НОВЫХ ошибок сверх базового количества; уменьшайте базовую линию с помощью `--update` после исправления существующей ошибки.                                                                                                                                                                                                                                                                                                                                            | Да                                               |

### Задание: `quality-gate`

Запускается после `test-coverage`. При неудачном выполнении блокирует слияние.

| Скрипт                       | Что проверяет                                                                                                                                                                                      | Блокирующий               |
| ---------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------- |
| `quality:collect`            | Создаёт `quality-metrics.json` (количество предупреждений ESLint, покрытие из объединённого отчёта по сегментам)                                                                                   | Да (предшествует ratchet) |
| `quality:ratchet`            | Ни одна метрика в `quality-baseline.json` не ухудшилась (предупреждения ESLint ≤ базового значения; покрытие ≥ базового значения)                                                                  | Да                        |
| `check:duplication`          | Дублирование кода (jscpd@4) не превышает базовое значение в `quality-baseline.json`                                                                                                                | Да                        |
| `check:complexity`           | Цикломатическая сложность на уровне файлов не превышает установленный предел (встроенные правила ESLint `complexity` + `max-lines-per-function`)                                                   | Да                        |
| `check:cognitive-complexity` | Ratchet когнитивной сложности (`eslint-plugin-sonarjs`) — отдельный проход ESLint; CI запускает обе проверки вместе как единый шаг `check:complexity-ratchets`                                     | Да                        |
| `check:dead-code`            | Ratchet неиспользуемых экспортов / файлов (knip) не ухудшается относительно базового значения                                                                                                      | Да                        |
| `check:compression-budget`   | Бюджет теста производительности сжатия — минимальные пороги экономии токенов для каждого движка не должны снижаться                                                                                | Да                        |
| `check:type-coverage`        | Ratchet доли типизированного кода (`type-coverage`) не ухудшается; в значительной степени включает в себя `typecheck:noimplicit:core`                                                              | Да                        |
| `check:codeql-ratchet`       | Количество открытых предупреждений CodeQL не увеличивается (данные получаются через `gh api`; корректный пропуск без токена) — периодичность обновления и ручной запуск: см. «CodeQL ratchet» ниже | Да                        |

### Задание: `quality-extended`

Всё задание носит рекомендательный характер (`continue-on-error: true`). Ratchet-проверки на базе npm выполняются
полноценно; внешние сканеры устанавливаются через `gh release download` и самостоятельно пропускают проверку (exit 0),
если бинарный файл по-прежнему отсутствует.

| Скрипт                   | Что проверяет                                                                                                                                                                                                           | Блокирующий          |
| ------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------- |
| `check:circular-deps`    | Отсутствие циклических зависимостей (dpdm)                                                                                                                                                                              | **Рекомендательный** |
| `check:bundle-size`      | Размер бандла не превышает установленный предел                                                                                                                                                                         | **Рекомендательный** |
| `check:secrets`          | Поиск секретов (gitleaks) — пропускается, если бинарный файл отсутствует                                                                                                                                                | **Рекомендательный** |
| `check:vuln-ratchet`     | Уязвимости зависимостей (osv-scanner) не увеличиваются — пропускается, если бинарный файл отсутствует                                                                                                                   | **Рекомендательный** |
| `check:workflows`        | Линтинг рабочих процессов (actionlint + zizmor) — пропускается, если бинарные файлы отсутствуют                                                                                                                         | **Рекомендательный** |
| `check:openapi-breaking` | Критические изменения публичного контракта API (`openapi.yaml`) относительно базовой ветки (oasdiff) — выводит `openapiBreaking=N`; пропускается, если oasdiff отсутствует или базовую спецификацию невозможно получить | **Рекомендательный** |
...
| ------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------- |
| `check:docs-all`               | Мета-проверка, последовательно запускающая 6 приведённых ниже проверок                                                                                                                                 | Да                     |
| ↳ `check:docs-sync`            | Согласованность версий в CHANGELOG / OpenAPI / llm.txt                                                                                                                                                 | Да                     |
| ↳ `check:docs-counts`          | Числовые данные в тексте (количество провайдеров, миграций и т. д.) находятся в пределах допустимого окна относительно фактических значений                                                            | Да                     |
| ↳ `check:env-doc-sync`         | Каждая переменная окружения из `.env.example` задокументирована в таблице документации, и наоборот                                                                                                     | Да                     |
| ↳ `check:deprecated-versions`  | В документации отсутствуют строки с устаревшими версиями                                                                                                                                               | Да                     |
| ↳ `check:doc-links`            | Внутренние markdown-ссылки в документации указывают на существующие файлы (в формате `[текст]`/`(путь)`)                                                                                               | Да                     |
| ↳ `check:fabricated-docs`      | Упомянутые в документации маршруты, переменные окружения, команды CLI, имена хуков и пути к файлам существуют в кодовой базе. Строгая проверка с `--strict`; без флага ошибка не блокирует выполнение. | Да (с `--strict` в CI) |
| `check:cli-i18n`               | Строки команд CLI присутствуют во всех файлах локалей i18n                                                                                                                                             | Да                     |
| `check:openapi-coverage`       | Спецификация OpenAPI охватывает как минимум пороговое количество реальных маршрутов                                                                                                                    | Да                     |
...
### Задание: `i18n-ui-coverage`

| Скрипт                                | Что проверяет                                                                                                                                                                                                                   | Блокирующая проверка |
| ------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------- |
| `check-ui-keys-coverage` (встроенный) | Покрытие ключей i18n интерфейса составляет ≥ 65%                                                                                                                                                                                | Да                   |
| `check-ui-value-drift` (встроенный)   | После изменения английского **значения** не остаётся устаревшего перевода                                                                                                                                                       | Да                   |
| `check-new-key-coverage` (встроенный) | **Новый** английский ключ переведён во всех локалях — маркер `__MISSING__:` недопустим                                                                                                                                          | Да                   |
| `check-translation-ratio`             | Доля реальных переводов для каждой локали (листья, идентичные английским / содержащие заполнители / отсутствующие вне списка исключений) не должна превышать `config/quality/i18n-translation-baseline.json` + допустимый запас | **Рекомендательная** |

Требуется `fetch-depth: 0` — проверка расхождения значений сравнивает `en.json` с базой слияния.

#### `check-ui-value-drift` — проверка устаревших переводов

Выявляет единственную регрессию i18n, которую другие проверки не способны обнаружить
структурно: английское значение переписывается, а переводы, созданные на основе _предыдущего_
английского текста, остаются без изменений, поэтому пользователи неанглоязычных локалей
продолжают читать уверенно сформулированный, но теперь уже неверный текст.

Это действительно попало в релиз. `oauthModal.googleOAuthWarning` был переписан при добавлении
помощника входа Antigravity (#5203); **в 39 из 43 локалей** остался текст, предлагавший операторам
«скопировать полный URL-адрес и вставить его ниже» — процесс, который невозможно завершить
для этого провайдера. Проблема оставалась незамеченной до #8463, потому что:
...
- `check-translation-drift` отслеживает зеркальные копии документации `docs/i18n/<locale>/**.md` —
  файлы `src/i18n/messages/*.json` он не читает. Блокирует выполнение в задании `docs-sync-strict`
  после повторной синхронизации 2026-09: измените основной документ → `npm run i18n:run -- --files=<doc>`
  (на уровне раздела, с низкими затратами).

**Учитывает diff, а не опирается на базовый файл.** Проверка сравнивает `en.json` в базе слияния
с рабочим деревом; для каждого ключа, английское значение которого изменилось, локаль,
по-прежнему содержащая неизменённый перевод, считается устаревшей. Это намеренно
**фиксирует ранее накопленный долг** — по diff невозможно определить, из какой старой
английской строки был получен давно существующий перевод, поэтому проверка оценивает
только то, чего касается текущее изменение. Альтернатива (базовый файл с хешем для каждого
ключа) потребовала бы генерируемый файл размером около 600 КБ, что в 3 раза больше
крупнейшего существующего базового файла и приводило бы к изменениям при каждом i18n PR.

Есть два способа пройти проверку:

1. обновить затронутые переводы или
2. задать для них значение `__MISSING__:<новый английский текст>` — тогда во время выполнения
   будет возвращаться исправленный английский текст
   (`src/i18n/request.ts::deepMergeFallback`, #7258), а ключ попадёт в очередь на перевод.

Если изменился **смысл** строки, предпочтительнее **переименовать ключ**: новый ключ не может
унаследовать устаревший перевод. Этот подход использовался в #8463.
...
Завершается с кодом 0 и сообщением `SKIP reason=base-unresolved`, если базовый каталог
невозможно прочитать (поверхностный клон без базовой ссылки), аналогично
`check-openapi-breaking`.

### Задание: `i18n`

Полная матрица проверки i18n (по одному заданию на локаль). Всё задание носит
рекомендательный характер.

| Скрипт                          | Что проверяет                      | Блокирует                                             |
| ------------------------------- | ---------------------------------- | ----------------------------------------------------- |
| `validate_translation.py quick` | Полноту перевода для каждой локали | **Нет** (`continue-on-error: true` для всего задания) |
...
| Скрипт                 | Что проверяет                                                                                                                                   | Блокирует |
| ---------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- | --------- |
| `check:pr-test-policy` | PR, изменяющие рабочий код в `src/`, `open-sse/`, `electron/` или `bin/`, должны добавлять или обновлять тесты (жёсткое правило № 8)            | Да        |
| `check:test-masking`   | Изменённые тестовые файлы не уменьшают общее количество проверок и не добавляют тавтологии `assert.ok(true)`                                    | Да        |
| `check:pr-evidence`    | Текст PR содержит сведения о тестировании/VPS для изменения (автоматизирует жёсткое правило № 18 поиском по тексту PR — ненадёжно, см. Backlog) | Да        |

### Задание: `test-vitest`

Выполняется после `build`. При ошибке блокирует слияние.

| Набор            | Что проверяет                                          | Блокирует                                                                                                    |
| ---------------- | ------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------ |
| `test:vitest`    | MCP-сервер (110 инструментов), autoCombo, кеш — vitest | Да                                                                                                           |
| `test:vitest:ui` | Тесты компонентов интерфейса — vitest                  | **Да** — ранее существовавшие сбои явно исключены в `vitest.config.ts`; новые сбои приводят к ошибке задания |

### Ночные рабочие процессы (по расписанию, рекомендательные)

Они запускаются по расписанию cron (а также через `workflow_dispatch`) и никогда не
выполняются для PR. Все они носят рекомендательный характер.

| Рабочий процесс        | Что проверяет                                                                                                                                                                      | Блокирует |
| ---------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------- |
| `nightly-property`     | Тесты свойств fast-check со случайным начальным значением и большим количеством запусков                                                                                           | **Нет**   |
| `nightly-resilience`   | Ограничение роста кучи, хаотическое внедрение сбоев, нагрузочное/длительное тестирование k6                                                                                        | **Нет**   |
| `nightly-llm-security` | Защиту promptfoo от инъекций (режим блокировки) и проверки garak (пропускаются без секрета провайдера)                                                                             | **Нет**   |
| `nightly-schemathesis` | Фаззинг контракта OpenAPI (schemathesis) для работающего OmniRoute с использованием `docs/openapi.yaml` — выявляет нарушения спецификации / необработанные ошибки 500 (этап 8 B.4) | **Нет**   |
| `nightly-mutation`     | Показатель мутационного тестирования Stryker для быстрого набора модульных тестов — выжившие мутанты выявляют слабые проверки                                                      | **Нет**   |
| `nightly-compat`       | Матрицу совместимости движка Node для поддерживаемых диапазонов `engines.node`                                                                                                     | **Нет**   |
