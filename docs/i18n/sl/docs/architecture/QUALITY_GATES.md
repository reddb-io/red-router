...
## Inventar kontrolnih mehanizmov (~90 skriptov)

Skripti so v `scripts/check/` (kontrolni mehanizmi pravilnikov) in `scripts/quality/` (mehanizem postopnega zaostrovanja).
Avtoritativni vir za CI je `.github/workflows/ci.yml`.

### Hitra pot za PR-je za izdajo (`quality.yml`)

`.github/workflows/quality.yml` se izvaja za PR-je, usmerjene v `release/**`. Veje sodelujočih ohranja pretočne s hitrimi kontrolnimi mehanizmi, filtriranimi glede na poti, ter enim svetovalnim signalom produkcijske gradnje za spremembe kode:

| Opravilo                                         | Obseg                                                                                                                                                                                                                                                    | Blokiranje                                                                                                 |
| ------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------- |
| `Build (advisory)`                               | PR-ji s kodo, ki niso osnutki, in veje čakalne vrste Mergify; Node 24, `npm-ci-retry`, `check:node-runtime`, `npm run build` z `OMNIROUTE_USE_TURBOPACK=1`; brez nalaganja artefaktov, ker jih ne uporablja nobeno nadaljnje opravilo za kakovost        | **Svetovalno** (`continue-on-error: true`; odstranite po enem tednu stabilnega izvajanja PR-jev za izdajo) |
| `Docs Gates (fast-path)`                         | PR-ji z dokumentacijo/kodo; sklici na dokumentacijo API-ja in docs-all                                                                                                                                                                                   | Da                                                                                                         |
| `Fast Quality Gates`                             | PR-ji s kodo; statična preverjanja, preverjanje tipov, preverjanje tipov nadzorne plošče, zadevni testi enot                                                                                                                                             | Da                                                                                                         |
| `Forgotten sibling tests`                        | PR-ji s kodo; sledenje spremenjenim modulom do statičnih porabnikov in kandidatnih sorodnih testov; poti prek zbirnih modulov in dinamičnih uvozov so sporočene kot svetovalna diagnostika, skupaj z navedenimi izjemami na seznamu dovoljenih elementov | **Svetovalno**                                                                                             |
| `Vitest (fast-path)`                             | PR-ji s kodo; hitra zbirka testov Vitest                                                                                                                                                                                                                 | Da                                                                                                         |
| `Unit Tests fast-path`                           | PR-ji s kodo; zbirka testov enot v 4 delih                                                                                                                                                                                                               | Da                                                                                                         |
| `No new ESLint warnings`                         | PR-ji s kodo; kontrolni mehanizem lintanja, ki upošteva opustitve                                                                                                                                                                                        | Da za PR-je iz istega izvora, svetovalno za razvejitve                                                     |
| `Merge integrity (changelog + generated skills)` | PR-ji, ki niso osnutki; dnevnik sprememb in sinhronizacija ustvarjenih veščin                                                                                                                                                                            | Da za PR-je iz istega izvora, svetovalno za razvejitve                                                     |
...
`spremenjen modul/simbol -> statični porabnik -> kandidatni sorodni test`, kadar kandidatni
test ni prisoten v razlikah zahtevka za vključitev sprememb. Povzetek Markdown in rezultat JSON se shranita kot
artefakt delovnega toka `forgotten-sibling-tests` za umerjanje pred morebitno uvedbo blokiranja.
...
test, podati natančno utemeljitev ter vsebovati povezavo do težave ali zahtevka za vključitev sprememb v GitHubu. Nepravilno oblikovani vnosi
povzročijo varno zavrnitev. Izjeme ne morejo skriti izbrisanega kandidatnega testa ali razlike, ki dodaja `.skip`/`.todo`;
slabljenje trditev in drugo prikrivanje ostajata v domeni neodvisno blokirajočega
kontrolnega mehanizma `check:test-masking`.
...
| `check:route-validation:t06`      | Prisotnost shem Zod na vseh poteh (pravilnik ravni 6)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | Da                                         |
| `check:any-budget:t11`            | Število `@ts-expect-error // any` ne presega omejitve (zapornica ravni 11)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | Da                                         |
...
| `check:model-lifecycle`           | Tri ročno vzdrževane usmerjevalne tabele ostajajo skladne z uveljavljenim posnetkom življenjskega cikla (#11503): `FITNESS_TABLE` (`taskFitness.ts`) ne točkuje nobenega opuščenega ID-ja, ki ga lahko usmerja `REGISTRY`; vsak cilj `BUILT_IN_ALIASES` je prisoten v `REGISTRY` in odsoten iz posnetka opuščenih ID-jev; vsak opuščeni ID, ki je še vedno v `REGISTRY`, je preusmerjen ali naveden v `allowedRetiredInCatalog`; noben vir ali cilj `DEFAULT_DEGRADATION_MAP` pa v tem posnetku ni označen kot opuščen. To ne dokazuje, da model trenutno zagotavlja delujoči zunanji ponudnik. Brez povezave — primerja s `config/quality/model-lifecycle.json`, ki se ročno osveži z `npm run quality:refresh-model-lifecycle` (zahteva omrežje; ni vključeno v CI). `allowedRetiredInCatalog` je postopno krčljiv omejevalni seznam: vnos dodajte samo skupaj z zadevo za sledenje. | Da                                         |
| `check:fetch-targets`             | Vsak `fetch("/api/...")` v odjemalskem delu `src/` se razreši v dejanski `route.ts`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | Da                                         |
| `check:deps`                      | Vse odvisnosti, ki jih je mogoče namestiti z `npm install`, v vseh datotekah `package.json` v repozitoriju so navedene v `dependency-allowlist.json`; nove odpetе ali tipkarskemu zavajanju podobne pakete označi                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | Da                                         |
| `audit:deps`                      | `npm audit` (korenski projekt + electron) — brez opozoril visoke/kritične stopnje (prekriva se z osv `check:vuln-ratchet`; glejte seznam za racionalizacijo)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | Da                                         |
...
| `check:tracked-artifacts`         | Brez artefaktov gradnje / potrjenih simbolnih povezav `node_modules` (izvede se tudi v huskyjevem preverjanju pred potrditvijo; preverjanje pred potiskom je namenoma nezahtevno — #6716)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | Da                                         |
| `check:ai-attribution`            | Brez zaključne vrstice `Co-Authored-By` umetne inteligence/bota ali noge o ustvarjanju z umetno inteligenco v potrditvah, naslovu ali telesu PR-ja — strogo pravilo št. 16 (v zanki hitrih preverjanj `quality.yml` za PR→`release/**` — prebere koristno vsebino dogodka, zunaj PR-jev pa ne naredi ničesar — ter kot korak samo za PR-je v preverjanju lint datoteke `ci.yml` za PR→`main`; tudi huskyjev kavelj `commit-msg`; človeški soavtorji so dovoljeni; #14436)                                                                                                                                                                                                                                                                                                                                                                                                              |
| `check:vitest-exclusions`         | Vsaka izključitev Vitest navede težavo za sledenje in je navedena v `config/quality/vitest-exclusions.json` (#13204)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | Da                                         |
| `check:file-size`                 | Nobena izvorna datoteka ne presega omejitve za posamezno pripono (raglja: velike zamrznjene datoteke na seznamu `frozen`)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | Da                                         |
| `check:error-helper`              | Odgovori o napakah v izvajalnikih/obravnavalnikih uporabljajo `buildErrorBody()` / `sanitizeErrorMessage()` (strogo pravilo št. 12)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | Da                                         |
| `check:migration-numbering`       | Migracijske datoteke SQL so zaporedno oštevilčene, brez vrzeli ali podvojenih številk                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | Da                                         |
| `check:public-creds`              | Zunaj datoteke `publicCreds.ts` ni dobesednih vrednosti OAuth `client_id`/`client_secret` ali spletnih ključev Firebase (strogo pravilo št. 11)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | Da                                         |
| `check:db-rules`                  | Zunaj modulov `src/lib/db/` ni surove kode SQL; ni zbirnih uvozov iz `localDb.ts` (strogi pravili št. 2/5)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | Da                                         |
| `check:known-symbols`             | Izvajalniki ponudnikov, strategije usmerjanja in prevajalniki, registrirani v svojih usmerjevalnih tabelah, se ujemajo z datotekami na disku — brez osirotelih ali nedeklariranih simbolov                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | Da                                         |
| `check:route-guard-membership`    | Vsaka pot, ki ustvari podrejeni proces, je razvrščena s funkcijo `isLocalOnlyPath()` (strogi pravili št. 15/17)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | Da                                         |
| `check:test-discovery`            | Vsako datoteko `*.test.ts` / `*.spec.ts` v repozitoriju zazna vsaj en izvajalnik testov (varovalo: seznam osirotelih datotek v `test-discovery-baseline.json` se lahko le skrajšuje)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | Da                                         |
| `check:agent-skills-sync`         | Ustvarjeni artefakti agentovih veščin se ujemajo z izvornim katalogom (brez odstopanj)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `check:provider-asset-provenance` | Logotipi/sredstva ponudnikov imajo zabeležen vnos o izvoru                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| `lint:json`                       | Konfiguracijske datoteke JSON se pravilno razčlenijo in izpolnjujejo pravila preverjanja repozitorija                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| `check:dashboard-typecheck`       | `tsc`, omejen na `src/app/(dashboard)/**` (#7033) — skrbno izbran seznam 27 datotek v `typecheck:core` ne vključuje nobene datoteke TSX nadzorne plošče, prav tako jih `next build` nikoli ne preverja glede tipov (`next.config.mjs` nastavi `ignoreBuildErrors: true`), zato regresije osirotelih identifikatorjev v njih (#6625/#6909) za CI niso bile vidne. Primerjava z zamrznjenim izhodiščem števila napak za posamezno datoteko in kodo TS (`config/quality/dashboard-typecheck-baseline.json`, enak vzorec uveljavljanja zastarelosti kot pri `check:known-symbols`) — preverjanje ne uspe le pri NOVIH napakah, ki presegajo izhodiščno število; ko je obstoječa napaka odpravljena, prag znižajte z `--update`.                                                                                                                                                            | Da                                         |
...
| ---------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------- |
| `quality:collect`            | Ustvari `quality-metrics.json` (število opozoril ESLint, pokritost iz združenega poročila drobcev)                                                                                            | Da (predhodnik mehanizma ratchet) |
...
| `check:cognitive-complexity` | Mehanizem ratchet za kognitivno kompleksnost (`eslint-plugin-sonarjs`) — ločen prehod ESLint; CI oba združi v en korak `check:complexity-ratchets`                                            | Da                                |
| `check:dead-code`            | Mehanizem ratchet za neuporabljene izvoze/datoteke (knip) ne nazaduje glede na izhodišče                                                                                                      | Da                                |
| `check:compression-budget`   | Proračun primerjalnega preizkusa stiskanja — spodnje meje prihranka žetonov za posamezen mehanizem ne smejo nazadovati                                                                        | Da                                |
| `check:type-coverage`        | Mehanizem ratchet za delež kode z določenimi tipi (`type-coverage`) ne nazaduje; v veliki meri nadomešča `typecheck:noimplicit:core`                                                          | Da                                |
| `check:codeql-ratchet`       | Število odprtih opozoril CodeQL ne nazaduje (bere prek `gh api`; brez žetona korak korektno preskoči) — pogostost osveževanja in ročni sprožilec: glejte »Mehanizem ratchet za CodeQL« spodaj | Da                                |

### Opravilo: `quality-extended`

Celotno opravilo je svetovalne narave (`continue-on-error: true`). Mehanizmi ratchet,
ki temeljijo na npm, se dejansko izvedejo; zunanji pregledovalniki se namestijo prek
`gh release download` in se sami preskočijo (izhodna koda 0), če izvedljiva datoteka
še vedno ni prisotna.
...
| `check:secrets`          | Iskanje skrivnosti (gitleaks) — preskoči, če izvedljiva datoteka ni prisotna                                                                                                                        | **Svetovalno** |
| `check:vuln-ratchet`     | Ranljivosti odvisnosti (osv-scanner) ne nazadujejo — preskoči, če izvedljiva datoteka ni prisotna                                                                                                   | **Svetovalno** |
| `check:workflows`        | Preverjanje delovnih tokov (actionlint + zizmor) — preskoči, če izvedljivi datoteki nista prisotni                                                                                                  | **Svetovalno** |
| `check:openapi-breaking` | Prelomne spremembe javne pogodbe API (`openapi.yaml`) glede na osnovno vejo (oasdiff) — ustvari `openapiBreaking=N`; preskoči, če oasdiff ni prisoten ali osnovne specifikacije ni mogoče razrešiti | **Svetovalno** |
...
| ------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------- |
| `check:docs-all`               | Metapregrada, ki zaporedno zažene spodnjih 6 podpregrad                                                                                                                                            | Da                        |
| ↳ `check:docs-sync`            | Skladnost različic v CHANGELOG / OpenAPI / llm.txt                                                                                                                                                 | Da                        |
| ↳ `check:docs-counts`          | Števila v besedilu (število ponudnikov, število migracij itd.) so znotraj dovoljenega okna glede na dejanska števila                                                                               | Da                        |
| ↳ `check:env-doc-sync`         | Vsaka okoljska spremenljivka v `.env.example` je dokumentirana v tabeli dokumentacije in obratno                                                                                                   | Da                        |
| ↳ `check:deprecated-versions`  | V dokumentaciji ni opuščenih nizov različic                                                                                                                                                        | Da                        |
| ↳ `check:doc-links`            | Notranje povezave Markdown v dokumentaciji kažejo na obstoječe datoteke (oblika `[besedilo]`/`(pot)`)                                                                                              | Da                        |
| ↳ `check:fabricated-docs`      | Poti, okoljske spremenljivke, ukazi CLI, imena kavljev in datotečne poti, navedene v dokumentaciji, obstajajo v kodni bazi. Stroga pregrada prek `--strict`; brez zastavice neuspeh ni blokirajoč. | Da (prek `--strict` v CI) |
| `check:cli-i18n`               | Nizi ukazov CLI so prisotni v vseh področnih datotekah i18n                                                                                                                                        | Da                        |
| `check:openapi-coverage`       | Specifikacija OpenAPI pokriva vsaj postopno zviševani spodnji prag dejanskih poti                                                                                                                  | Da                        |
...
| `check:openapi-routes`         | Vsaka pot v `openapi.yaml` kaže na obstoječo datoteko `route.ts` (preprečevanje halucinacij)                                                                                                       | Da                        |
| `check:docs-symbols`           | Vsak sklic `/api/...` v `docs/**/*.md` kaže na obstoječo datoteko `route.ts` (preprečevanje halucinacij)                                                                                           | Da                        |
...
| `check-ui-value-drift` (vgrajeno)   | Spremenjena angleška **vrednost** ne pušča za seboj zastarelega prevoda                                                                                                                                                           | Da             |
| `check-new-key-coverage` (vgrajeno) | **Nov** angleški ključ je preveden v vsakem področnem jeziku — oznaka `__MISSING__:` je zavrnjena                                                                                                                                 | Da             |
| `check-translation-ratio`           | Delež dejanskih prevodov po področnih jezikih (vnosi, ki so enaki angleškim, vsebujejo označbo mesta ali manjkajo in niso na seznamu dovoljenih izjem) ne sme preseči `config/quality/i18n-translation-baseline.json` + tolerance | **Svetovalno** |

Zahteva `fetch-depth: 0` — pregrada za odstopanje vrednosti primerja spremembe v `en.json` glede na osnovo združitve.

#### `check-ui-value-drift` — pregrada za zastarele prevode

Zazna tisto regresijo i18n, ki je druge pregrade strukturno ne morejo opaziti: angleška vrednost
...
pomočnika za prijavo Antigravity (#5203); **39 od 43 področnih jezikov** je ohranilo besedilo, ki je skrbnikom
naročalo, naj »kopirajo celoten URL in ga prilepijo spodaj« — potek, ki ga pri tem ponudniku ni mogoče dokončati.
Težava je ostala neopažena do #8463, ker:
...
- `check-translation-drift` spremlja dokumentacijske zrcalne različice `docs/i18n/<locale>/**.md` —
  datotek `src/i18n/messages/*.json` nikoli ne bere. V opravilu `docs-sync-strict` je preverjanje blokirajoče od
  ponovne sinhronizacije 2026-09: uredite osrednji dokument → `npm run i18n:run -- --files=<doc>` (na ravni razdelka, hitro).

**Upošteva razlike in ne temelji na izhodišču.** Primerja `en.json` na skupnem predniku veje z
delovnim drevesom; za vsak ključ, katerega angleška vrednost se je spremenila, je vsak jezik, ki še vedno vsebuje
nedotaknjen prevod, zastarel. S tem namenoma **zamrzne obstoječi dolg** — iz razlike
ni mogoče ugotoviti, iz katere stare angleške različice izvira dolgo nespremenjen prevod, zato preverjanje ocenjuje
samo tisto, česar se dotika trenutna sprememba. Alternativa (izhodišče z zgoščeno vrednostjo za vsak ključ) bi zahtevala
približno 600 KB veliko ustvarjeno datoteko, 3-krat večjo od največjega obstoječega izhodišča, ki bi se spreminjala ob vsakem i18n PR-ju.

Preverjanje je mogoče uspešno opraviti na dva načina:

1. posodobite zadevne prevode ali
2. jih nastavite na `__MISSING__:<new english>` — izvajalno okolje nato prikaže popravljeno angleško besedilo
...
```bash
npm run i18n:check-value-drift          # strogo preverjanje (izvaja ga CI)
...
kloniranje brez sklica na osnovo), enako kot `check-openapi-breaking`.
...
Izvede se samo pri zahtevah za spojitev.
...
| `check:test-masking`   | Spremenjene testne datoteke ne zmanjšajo neto števila trditev in ne dodajajo tavtologij `assert.ok(true)`                                                         | Da         |
| `check:pr-evidence`    | Telo PR-ja navaja dokazila o testih/VPS za spremembo (avtomatizira strogo pravilo št. 18 s preiskovanjem besedila PR-ja — krhko, glejte seznam zaostalih opravil) | Da         |

### Opravilo: `test-vitest`

Izvede se po `build`. Ob neuspehu blokira spojitev.
...
Izvajajo se po urniku cron (in prek `workflow_dispatch`), nikoli pri PR-jih. Vsi so svetovalni.
...
| `nightly-resilience`   | preverjanje rasti kopice, kaotično vbrizgavanje napak, obremenitveni/dolgotrajni testi k6                                                                                   | **Svetovalno** |
| `nightly-llm-security` | zaščita pred vbrizgavanjem promptfoo (način blokiranja) + sonde garak (preskočene brez skrivnosti ponudnika)                                                                | **Svetovalno** |
...
| `nightly-mutation`     | Rezultat mutacijskega testiranja Stryker za hitro vejo testov enot — preživeli mutanti razkrijejo šibke trditve                                                             | **Svetovalno** |
| `nightly-compat`       | Matrika združljivosti izvajalnika Node za podprte razpone `engines.node`                                                                                                    | **Svetovalno** |
