...
המאגר כולל Dockerfile רב-שלבי (`Dockerfile`). ארבעה שלבים זמינים; בחרו את ה-`target` המתאים למקרה השימוש שלכם.
...
| `runner-web`  | `runner-base`         | מוסיף את Playwright ודפדפן Chromium (`--with-deps`) עבור ספקי סשן אינטרנט: `gemini-web`,‏ `claude-web`,‏ `claude-turnstile`. **בחרו באפשרות זו בעת שימוש בספקים האלה** — התמונה הרגילה נכשלת בזמן הבקשה בלעדיה (ראו את ההערה על `-web` תחת ערוצי הפצה). |
...
שלושה ארגומנטים של בנייה שולטים בעלות המשאבים של השלב `builder`. הם תקפים בזמן הבנייה בלבד —
`OMNIROUTE_MEMORY_MB` (להלן) הוא פרמטר נפרד לזמן הריצה.

| ארגומנט בנייה               | ברירת מחדל | השפעה                                                                                       |
| --------------------------- | ---------- | ------------------------------------------------------------------------------------------- |
| `OMNIROUTE_USE_TURBOPACK`   | `1`        | הערך `0` מבצע בנייה באמצעות webpack במקום זאת. צריכת זיכרון מרבית נמוכה יותר, אך איטי יותר. |
...
`OMNIROUTE_BUILD_WORKERS` הוא הפרמטר שיש להגדיל במכונת בנייה חזקה, וזה שיש
לחשוד בו כאשר בנייה בסביבה מוגבלת נכשלת **לאחר** `✓ Compiled successfully`. כל
תהליך עובד של נתוני דפים הוא תהליך נפרד, וכך גם תהליך האב `next build` עצמו;
שחזור על VPS פעיל (בעיה #7518) מדד שיא RSS של
~4.5 GB לכל תהליך, ללא תלות בדגל הערימה `NODE_OPTIONS` (‏Turbopack מבצע הידור
בזיכרון מקורי/Rust מחוץ לערימת V8). ברירת המחדל `2` (→ עובד אחד, 2
תהליכים בסך הכול) מותאמת לרצים המתארחים ב-GitHub עם 16 GB / 4 vCPU, שבהם
משתמש צינור ההפצה. בערך `8` (→ 7 עובדים), הזיכרון ברץ הזה אזל
...
Turbopack מבצע הידור בזיכרון Rust מקורי שנמצא **מחוץ** לערימת V8, ולכן
...
כמו מחסור בזיכרון. אם מארח הבנייה מוגבל, החליפו כלי איגוד:
...
וכל אחד מהם מכבד את `OMNIROUTE_BUILD_MEMORY_MB` בנפרד. הגדירו את תקרת הזיכרון
של הקונטיינר לקצת יותר מפי שניים מערך זה, ולא לפעם אחת בלבד.

נמדד בעץ זה (`--target runner-base`,‏ `OMNIROUTE_BUILD_MEMORY_MB=6144`):

| כלי איגוד | תקרת הקונטיינר | תוצאה                                  |
| --------- | -------------- | -------------------------------------- |
| Turbopack | 8 GiB / 16 GiB | נהרג על ידי OOM בשניהם, ללא הודעה      |
| webpack   | 8 GiB          | תהליך עובד הבנייה נהרג באמצעות SIGKILL |
...
- תהליך השרת בפועל מופעל באמצעות המפעיל העצמאי, שקורא את `OMNIROUTE_MEMORY_MB` ומוסיף `--max-old-space-size=<OMNIROUTE_MEMORY_MB>`.
- Node משתמש בערך האחרון של `--max-old-space-size` כאשר הדגל מופיע מספר פעמים, ולכן הגדרת `OMNIROUTE_MEMORY_MB` שולטת במגבלת הערימה האפקטיבית ב-Docker.
- מכיוון שהתמונה תמיד מגדירה אותו, ברירת המחדל החלופית של המפעיל, המכוילת לפי זיכרון ה-RAM, לעולם אינה חלה תחת Docker. הגדילו אותו במפורש בהתאם לעומס העבודה (בטבלה שלהלן). גם `2048` קטן מדי עבור `/v1/responses` של סוכני תכנות.

### זיכרון RAM בזמן ריצה עבור סוכני תכנות

ברירת המחדל של Docker, ‏1 GiB, היא רף מינימלי עבור לוח הבקרה/צ'אט קל, ולא גודל המתאים לסביבת ייצור. גופי `POST /v1/responses` ארוכים (מאות הודעות, עשרות כלים) מחזיקים בזיכרון מספר גרפים במהלך הדחיסה. שתי בקשות חופפות בגודל של כ-‎3 MiB / כ-750 אלף טוקנים גרמו ל-V8 להפסיק לפעול עם מרחב ישן של **12 GiB** (`FATAL ERROR: Reached heap limit`), וגם הגיעו למצב OOM של cgroup בנפח 16 GiB. ראו [#7849](https://github.com/diegosouzapw/OmniRoute/issues/7849).

הגדירו את **`--memory` של cgroup מעל גודל הערימה** — מאגרים טבעיים, SQLite ותוצרי ביניים של דחיסה נמצאים מחוץ ל-V8.

| עומס עבודה                              | `OMNIROUTE_MEMORY_MB`       | קונטיינר / cgroup      | הערות                                                                                        |
| --------------------------------------- | --------------------------- | ---------------------- | -------------------------------------------------------------------------------------------- |
| לוח בקרה, צ'אט קל אחד                   | `1024` (ברירת המחדל בתמונה) | ≥2 GiB                 |                                                                                              |
| סוכן תכנות אחד (Claude/Codex/Grok)      | `8192`                      | ≥10 GiB                | הפעלה טיפוסית של `/v1/responses` בסשן יחיד                                                   |
| שתי בקשות `/v1/responses` ארוכות במקביל | `10240`–`12288`             | ≥12–16 GiB             | נמדדה הפסקת V8 בערימת זיכרון של כ-12 GiB                                                     |
| שלושה הקשרים ארוכים או יותר במקביל      | אין להריץ בתהליך יחיד       | עיבוד סדרתי / יותר RAM | ברירת המחדל לקבלת עומסים כבדים היא בקשה פעילה אחת; הגדלתה ללא RAM נוסף גורמת לכשל להתרחש שוב |

`omniroute serve` על מערכת פיזית מכייל לכ-35% מה-RAM (מוגבל לטווח `[512, 4096]`) כאשר `OMNIROUTE_MEMORY_MB` **אינו מוגדר**. Docker תמיד מגדיר אותו כ-`1024`, ולכן הכיול הזה לעולם אינו מופעל בתמונה הרשמית.
