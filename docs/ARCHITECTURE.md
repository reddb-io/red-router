...
- `src/lib/localDb.js`
- file: `${DATA_DIR}/db.json` (or `~/.red-router/db.json` when `DATA_DIR` is unset)
- entities: providerConnections, providerNodes, modelAliases, combos, apiKeys, settings, pricing

Usage DB:

- `src/lib/usageDb.js`
- files: `~/.red-router/usage.json`, `~/.red-router/log.txt`
...
- main state: `${DATA_DIR}/db.json` (or `~/.red-router/db.json`)
- usage stats: `~/.red-router/usage.json`
- request log lines: `~/.red-router/log.txt`
...
1. `usageDb` currently stores under `~/.red-router` and does not follow `DATA_DIR`.
