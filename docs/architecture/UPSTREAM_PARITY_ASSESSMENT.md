...
The upstream registry also declares its `api/web_fetch` path; this worktree adds that
...
the upstream Windsurf registry module exactly (sorted-set
...
upstream's Qoder `attachments.js` converts OpenAI file and
Claude document blocks into omission text, and
upstream's Qoder executor (`qoder.js`) does the same if blocks survive
