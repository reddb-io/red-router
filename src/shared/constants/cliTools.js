...
export const CLI_TOOLS = {
  claude: {
    ...
    defaultModels: [
      ...
      { id: "opus", name: "Claude Opus", alias: "opus", envKey: "ANTHROPIC_DEFAULT_OPUS_MODEL", defaultValue: "claude-code/claude-opus-5-5" },
  ...
  copilot: {
    ...
    color: "#1F6FEB",
    description: "GitHub Copilot in VS Code via RedRouter extension",
    configType: "guide",
    docsUrl: "https://marketplace.visualstudio.com/items?itemName=hotrungnhan.red-router-for-github-copilot",
    guideSteps: [
      {
        step: 1,
        title: "Install Extension",
        desc: "In VS Code, open Extensions (Ctrl+Shift+X or Cmd+Shift+X), search for 'RedRouter for Github Copilot' and click Install.",
      },
      {
        step: 2,
        title: "Configure Server",
        desc: "Press Cmd+Shift+P (or Ctrl+Shift+P), run 'RedRouter: Configure Server', then enter your Server URL and API Key:",
        value: "{{baseUrl}}",
        copyable: true,
      },
      {
        ...
        desc: "Open Copilot Chat, click the model picker at the bottom → 'Manage Models...' → check the RedRouter models to use.",
      },
    ],
  },
  opencode: {
  ...
  amp: {
    ...
    guideSteps: [
      ...
      { step: 5, title: "Add Shorthands", desc: "Map Amp shorthand names such as g25p or cs45 to RedRouter aliases in your local config." },
    ],
    codeBlock: {
      language: "bash",
...
# cs45 -> claude-code/claude-sonnet-4-5-20250929`,
  ...
  qwen: {
    ...
    color: "#10B981",
    description: "Alibaba Qwen Code CLI — supports OpenAI, Anthropic & Gemini providers via RedRouter",
    docsUrl: "https://qwenlm.github.io/qwen-code-docs/en/users/configuration/model-providers/",
    configType: "guide",
    defaultCommand: "qwen",
    notes: [
      { type: "info", text: "Qwen Code supports multiple provider types (openai, anthropic, gemini) via modelProviders in settings.json. RedRouter works as an OpenAI-compatible endpoint." },
      { type: "info", text: "Any model available in RedRouter can be used — not just Qwen models. Select from Qwen, Claude, Gemini, GPT, and more." },
      { type: "warning", text: "Config path: Linux/macOS ~/.qwen/settings.json • Windows %USERPROFILE%\\.qwen\\settings.json" },
      { type: "error", text: "Qwen OAuth free tier was discontinued on 2026-04-15. Use RedRouter with alicode/openrouter/anthropic/gemini providers instead." },
    ],
  ...
  "deepseek-tui": {
    ...
    notes: [
      { type: "info", text: "DeepSeek TUI uses ~/.deepseek/config.toml for configuration. RedRouter will update the provider to 'openai' mode with your base_url, api_key, and model." },
  ...
  jcode: {
    ...
    notes: [
      ...
      {
        type: "info",
        text: "Configure red-router as an OpenAI-compatible provider to route all jcode requests through red-router's optimization layer."
      },
      {
        type: "warning",
  ...
  "grok-build": {
    ...
    notes: [
      {
        type: "info",
        text: "Grok Build uses ~/.grok/config.toml. RedRouter writes a [model.red-router] custom model and sets it as the default.",
      },
      {
        type: "info",
        text: "After Apply, run grok (or /model red-router) to use the routed model. Switch back anytime with /model grok-build.",
      },
      {
        type: "warning",
...
devin --version`,
  ...
  opendesign: {
    ...
    configType: "guide",
    notes: [
      { type: "info", text: "OpenDesign ships as a plugin/skills pack installed into Claude Code, Cursor, OpenAI Codex, Gemini CLI, or OpenCode. It inherits the host agent's model config, so once your host points at RedRouter, /opendesign design sessions route through RedRouter automatically — no extra env vars needed." },
    ...
    guideSteps: [
      ...
      { step: 2, title: "No config needed", desc: "OpenDesign runs inside your host agent and uses its model config. If the host already routes through RedRouter, /opendesign traffic does too." },
...
# Fetch and follow .opencode/INSTALL.md from the repo`,
  ...
  pi: {
    ...
    notes: [
      {
        type: "info",
        text: "Pi uses ~/.pi/agent/models.json. RedRouter is configured under providers.red-router as an OpenAI-compatible endpoint.",
      },
    ],
  },
  omp: {
    ...
    notes: [
      {
        type: "info",
        text: "Oh My Pi uses ~/.omp/agent/models.yml and agent.db. RedRouter is configured with proxy discovery so all models appear automatically under /model.",
      },
    ],
  },
  crush: {
    ...
    notes: [
      {
        type: "info",
        text: "Crush uses ~/.config/crush/crush.json. RedRouter registers as an openai-compat provider.",
      },
    ],
  },
  forge: {
    ...
    notes: [
      {
        type: "info",
        text: "ForgeCode uses ~/.forge/config.toml. RedRouter updates the [openai] section with your baseUrl, apiKey, and model.",
      },
    ],
  },
  smelt: {
  ...
  codewhale: {
    ...
    notes: [
      {
        type: "info",
        text: "CodeWhale uses ~/.codewhale/config.toml. RedRouter configures the [openai] provider with your base_url, api_key, and model.",
      },
    ],
  },
  // HIDDEN: gemini-cli
