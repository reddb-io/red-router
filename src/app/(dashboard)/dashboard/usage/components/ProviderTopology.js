...
// Center RedRouter node — pulse/glow on card only (no expanding rings)
  ...
  return (
    <div
      className={`relative z-[1] flex items-center justify-center px-5 py-3 rounded-xl border-2 min-w-[130px] ${
        powering
          ? "topology-router-core border-feedback-warning-border bg-gradient-to-br from-primary/30 via-yellow-400/20 to-cyan-400/25"
    ...
    >
      ...
      <img
        ...
        decoding="async"
      />
      <span className={`text-sm font-bold ${powering ? "topology-router-label text-feedback-warning-foreground" : "text-primary"}`}>
        RedRouter
      </span>
      {data.activeCount > 0 && (
...
export default function ProviderTopology({ providers = [], activeRequests = [], lastProvider = "", errorProvider = "" }) {
  ...
  const activeSet = useMemo(() => {
    // eslint-disable-next-line react-hooks/purity -- time-based memo: Date.now() is the point, tick re-runs it
    const now = Date.now();
    const filtered = new Set();
    for (const p of rawActiveSet) {
      const ts = firstSeenRef.current[p];
      // eslint-disable-next-line react-hooks/refs -- time-based memo: firstSeenRef is read on purpose, tick re-runs it
