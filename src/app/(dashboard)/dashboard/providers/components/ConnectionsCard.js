...
function CooldownTimer({ until }) {
  ...
  return <span className="text-xs text-[var(--reddb-color-feedback-warning-foreground)] font-mono">⏱ {remaining}</span>;
}
...
function ConnectionRow({ connection, proxyPools, isOAuth, isFirst, isLast, onMoveUp, onMoveDown, onToggleActive, onUpdateProxy, onEdit, onDelete }) {
  ...
  return (
    <div className={`group flex flex-col gap-3 p-2 rounded-lg sm:flex-row sm:items-center sm:justify-between hover:bg-muted/50 transition-colors ${connection.isActive === false ? "opacity-60" : ""}`}>
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex flex-wrap items-center gap-2 mt-1">
            ...
            {connection.lastError && connection.isActive !== false && (
              <span className="text-xs text-[var(--reddb-color-feedback-danger-foreground)] truncate max-w-[300px]" title={connection.lastError}>{connection.lastError}</span>
            )}
          ...
          {hasAnyProxy && (
            <div className="mt-1 flex flex-wrap items-center gap-2">
              ...
              {maskedProxyUrl && <code className="text-[10px] font-mono bg-muted/50 px-1 py-0.5 rounded text-text-muted">{maskedProxyUrl}</code>}
            ...
            <div className="relative" ref={proxyDropdownRef}>
              ...
              {showProxyDropdown && (
                <div className="absolute right-0 top-full mt-1 z-50 bg-bg border border-border rounded-lg shadow-lg py-1 min-w-[160px]">
                  <button onClick={() => handleSelectProxy("__none__")} className={`w-full text-left px-3 py-1.5 text-sm hover:bg-muted/50 ${!boundProxyPoolId ? "text-primary font-medium" : "text-text-main"}`}>None</button>
                  {(proxyPools || []).map((pool) => (
                    <button key={pool.id} onClick={() => handleSelectProxy(pool.id)} className={`w-full text-left px-3 py-1.5 text-sm hover:bg-muted/50 ${boundProxyPoolId === pool.id ? "text-primary font-medium" : "text-text-main"}`}>{pool.name}</button>
                  ))}
                </div>
              )}
            </div>
    ...
    <>
      <Card>
        ...
        {connections.length === 0 ? (
          ...
          <>
            <div className="flex flex-col divide-y divide-muted max-h-[500px] overflow-y-auto pr-1">
