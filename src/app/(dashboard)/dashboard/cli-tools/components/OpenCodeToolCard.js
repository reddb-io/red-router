...
export default function OpenCodeToolCard({ tool, isExpanded, onToggle, baseUrl, apiKeys, activeProviders, cloudEnabled, initialStatus, tunnelEnabled, tunnelPublicUrl, tailscaleEnabled, tailscaleUrl }) {
  ...
  useEffect(() => {
    ...
    if (status?.config?.agent?.explorer?.model?.startsWith("red-router/")) {
      setSubagentModel(status.config.agent.explorer.model.replace("red-router/", ""));
    }
  }, [status]);
  ...
  const currentBaseUrl = status?.config?.provider?.["red-router"]?.options?.baseURL || "";

  const getConfigStatus = () => {
    if (!status?.installed) return null;
    if (!status.config) return "not_configured";
    if (!status.hasRedRouter) return "not_configured";
    const url = status.config?.provider?.["red-router"]?.options?.baseURL || "";
  ...
  const getManualConfigs = () => {
    ...
    return [{
      ...
      content: JSON.stringify({
        ...
        model: `red-router/${activeModelToShow}`,
        agent: {
          explorer: {
...
>>>>>>> feat/ds-v2026.09
                  <div className="flex-1">
                    <p className="font-medium text-feedback-warning-foreground">OpenCode CLI not detected locally</p>
          ...
          {!checking && status?.installed && (
            <>
              <div className="flex flex-col gap-2">
                ...
                {/* Current configured */}
                {status?.config?.provider?.["red-router"]?.options?.baseURL && (
                    ...
                    <div className="flex flex-wrap gap-1.5 min-h-[28px] px-2 py-1.5 bg-surface rounded border border-border">
                      {selectedModels.length === 0 ? (
                        ...
                        selectedModels.map((model) => (
                          ...
                          >
                            {model === activeModel && <Icon name="star" size={10} />}
                            {model}
                            <button
                              onClick={async (e) => {
                                e.stopPropagation();
                              ...
                              className="ml-0.5 hover:text-feedback-danger-foreground"
                            >
                    ...
                    <div className="grid grid-cols-1 gap-1.5 sm:grid-cols-[8rem_auto_1fr_auto] sm:items-center sm:gap-2">
                      ...
                      <span className="text-xs text-text-muted">
                        {selectedModels.length > 0 && activeModel ? (
                          ...
                          <span className="text-feedback-warning-foreground">Click a model to set/clear active</span>
                        ) : (
                ...
                <div className="grid grid-cols-1 gap-1.5 sm:grid-cols-[8rem_auto_1fr_auto] sm:items-center sm:gap-2">
                  ...
                  {subagentModel && (
                    <button
                      ...
                      className="p-1 text-text-muted hover:text-feedback-danger-foreground rounded transition-colors"
