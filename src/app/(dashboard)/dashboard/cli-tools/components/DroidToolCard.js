...
export default function DroidToolCard({
  ...
  const currentBaseUrl = droidStatus?.settings?.customModels?.find((m) => m.id?.startsWith("custom:RedRouter"))?.baseUrl || "";

  const getConfigStatus = () => {
    if (!droidStatus?.installed) return null;
    // Check for any RedRouter model entry (support multi-model: custom:RedRouter-0, custom:RedRouter-1, ...)
    const currentConfig = droidStatus.settings?.customModels?.find(m => m.id?.startsWith("custom:RedRouter"));
  ...
  useEffect(() => {
    if (droidStatus?.installed && !hasInitializedModel.current) {
      ...
      const existingModels = (droidStatus.settings?.customModels || [])
        .filter(m => m.id?.startsWith("custom:RedRouter"))
      ...
      if (existingModels.length > 0) {
        ...
        const legacy = droidStatus.settings?.customModels?.find(m => m.id === "custom:RedRouter-0");
        if (legacy?.model) {
              ...
              <div className="flex flex-col gap-3 p-4 bg-[var(--reddb-color-feedback-warning-surface)] border border-[var(--reddb-color-feedback-warning-border)] rounded-lg">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    <p className="font-medium text-[var(--reddb-color-feedback-warning-foreground)]">Factory Droid CLI not detected locally</p>
                ...
                <div className="p-4 bg-surface border border-border rounded-lg">
                  ...
                  <div className="space-y-3 text-sm">
                    <div>
                      ...
                      <code className="block px-3 py-2 bg-muted/50 rounded font-mono text-xs">curl -fsSL https://app.factory.ai/cli | sh</code>
                    </div>
                    <p className="text-text-muted">After installation, run <code className="px-1 bg-muted/50 rounded">droid</code> to verify.</p>
                  </div>
                </div>
              )}
            </div>
          ...
          {!checkingDroid && droidStatus?.installed && (
            <>
              <div className="flex flex-col gap-2">
                ...
                {/* Current configured */}
                {droidStatus?.settings?.customModels?.find(m => m.id?.startsWith("custom:RedRouter"))?.baseUrl && (
              ...
              <div className="grid grid-cols-1 gap-2 sm:flex sm:items-center">
                ...
                <Button variant="outline" size="sm" onClick={handleResetSettings} disabled={!droidStatus?.hasRedRouter} loading={restoring}>
