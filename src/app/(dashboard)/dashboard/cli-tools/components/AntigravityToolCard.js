              ...
              <div className="grid grid-cols-1 gap-1.5 sm:grid-cols-[8rem_auto_1fr_auto] sm:items-center sm:gap-2">
                ...
                {apiKeys.length > 0 ? (
                  ...
                  <span className="min-w-0 rounded bg-surface/40 px-2 py-2 text-xs text-text-muted sm:py-1.5">
                    {cloudEnabled ? "No API keys - Create one in Keys page" : "sk_red-router (default)"}
                  </span>
                )}
              </div>
          ...
          {!isRunning && (
            <div className="flex flex-col gap-1.5 px-1">
              <p className="text-xs text-text-muted">
                <span className="font-medium text-text-main">How it works:</span> Intercepts Antigravity traffic via DNS redirect, letting you reroute models through RedRouter.
              </p>
              <div className="flex flex-col gap-0.5 text-[11px] text-text-muted">
                ...
                <span>3. Maps Antigravity models to any provider via RedRouter</span>
              </div>
            </div>
          )}
        </div>
