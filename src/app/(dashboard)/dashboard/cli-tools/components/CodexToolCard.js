...
export default function CodexToolCard({ tool, isExpanded, onToggle, baseUrl, apiKeys, activeProviders, cloudEnabled, initialStatus, tunnelEnabled, tunnelPublicUrl, tailscaleEnabled, tailscaleUrl }) {
  ...
  const handleApplySettings = async () => {
    ...
    setMessage(null);
    try {
      // Use sk_red-router for localhost if no key, otherwise use selected key
  ...
  const getManualConfigs = () => {
    ...
    const configContent = `# RedRouter Configuration for Codex CLI
model = "${selectedModel}"
model_provider = "red-router"

[model_providers.red-router]
name = "RedRouter"
base_url = "${getEffectiveBaseUrl()}"
wire_api = "responses"

[model_providers.red-router.http_headers]
              ...
              <div className="flex flex-col gap-3 p-4 bg-[var(--reddb-color-feedback-warning-surface)] border border-[var(--reddb-color-feedback-warning-border)] rounded-lg">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    <p className="font-medium text-[var(--reddb-color-feedback-warning-foreground)]">Codex CLI not detected locally</p>
                ...
                <div className="p-4 bg-surface border border-border rounded-lg">
                  ...
                  <div className="space-y-3 text-sm">
                    <div>
                      ...
                      <code className="block px-3 py-2 bg-muted/50 rounded font-mono text-xs">npm install -g @openai/codex</code>
                    </div>
                    <p className="text-text-muted">After installation, run <code className="px-1 bg-muted/50 rounded">codex</code> to verify.</p>
                    <div className="pt-2 border-t border-border">
                      <p className="text-text-muted text-xs">
                        Codex reads custom providers from <code className="px-1 bg-muted/50 rounded">~/.codex/config.toml</code>.
