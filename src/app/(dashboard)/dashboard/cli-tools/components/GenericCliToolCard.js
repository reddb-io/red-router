...
export default function GenericCliToolCard({
  ...
  const [selectedModel, setSelectedModel] = useState(() => {
    ...
    return cfg?.model || cfg?.openai?.model || cfg?.providers?.["red-router"]?.models?.[0]?.id || "";
  });
  const [selectedModels, setSelectedModels] = useState(() => {
    const cfg = initialStatus?.config;
    const list = cfg?.providers?.["red-router"]?.models;
        ...
        .then((data) => {
          if (active) {
            ...
            if (tool.id === "pi") {
              ...
              const mod = cfg?.model || cfg?.openai?.model || cfg?.providers?.["red-router"]?.models?.[0]?.id;
  ...
  const getCurrentBaseUrl = () => {
    ...
    if (typeof cfg.providers?.["red-router"]?.base_url === "string") return cfg.providers["red-router"].base_url;
    if (typeof cfg.providers?.["red-router"]?.baseUrl === "string") return cfg.providers["red-router"].baseUrl;
    return "";
  };
  ...
  const getManualConfigContent = () => {
    ...
    const key = selectedApiKey || "sk_red-router";
        ...
        return [
          {
            ...
            content: `# Forge config — managed by RedRouter\n\n[openai]\napi_key = "${key}"\nbase_url = "${effectiveUrl}"\nmodel = "${mod}"`,
          },
        ];
      case "smelt":
        return [
          {
            ...
            content: JSON.stringify({ baseUrl: effectiveUrl, apiKey: key, model: mod, _managedBy: "red-router" }, null, 2),
          },
        ];
      case "codewhale":
        return [
          {
            ...
            content: `# CodeWhale config — managed by RedRouter\n\n[openai]\nbase_url = "${effectiveUrl}"\napi_key = "${key}"\nmodel = "${mod}"`,
          },
        ];
      default:
        return [];
        ...
        <div className="flex min-w-0 items-center gap-3">
          <div className="size-8 flex items-center justify-center shrink-0">
            {tool.image ? (
              ...
              <Icon name={tool.icon} size={28} style={{ color: tool.color }} />
            ) : (
          ...
          <div className="min-w-0">
            <div className="flex min-w-0 flex-wrap items-center gap-2">
              ...
              {configStatus === "configured" && (
                <span className="px-1.5 py-0.5 text-[10px] font-medium bg-feedback-success-surface text-feedback-success-foreground rounded-full">
                  Connected
                </span>
              )}
              {configStatus === "not_configured" && (
                <span className="px-1.5 py-0.5 text-[10px] font-medium bg-feedback-warning-surface text-feedback-warning-foreground rounded-full">
                  Not configured
                </span>
              )}
              {configStatus === "other" && (
                <span className="px-1.5 py-0.5 text-[10px] font-medium bg-feedback-info-surface text-feedback-info-foreground rounded-full">
                  Other
                </span>
              )}
            </div>
              ...
              <div className="flex flex-col gap-3 p-4 bg-feedback-warning-surface border border-feedback-warning-border rounded-lg">
                ...
                <div className="flex items-center gap-2 pl-9">
                  <Button
                    ...
                    className="!bg-feedback-warning-surface !border-feedback-warning-border !text-feedback-warning-foreground dark:!text-feedback-warning-foreground hover:!bg-feedback-warning-surface"
                  >
            ...
            <>
              <div className="flex flex-col gap-2">
                ...
                {tool.id === "pi" && (
                  <div className="grid grid-cols-1 gap-1.5 sm:grid-cols-[8rem_auto_1fr] sm:items-start sm:gap-2">
                    ...
                    <Icon name="arrow_forward" size={14} className="hidden text-text-muted sm:inline mt-1.5" />
                          ...
                          selectedModels.map((modelId) => (
                            ...
                            >
                              ...
                              <button
                                ...
                                className="text-text-muted hover:text-feedback-danger-foreground rounded p-0.5"
                              >
                      ...
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        {selectedModels.length > 0 && (
                          <button
                            ...
                            className="text-xs text-text-muted hover:text-feedback-danger-foreground ml-auto"
                          >
                            Clear all
                          </button>
                        )}
                ...
                <div
                  className={`p-3 rounded-lg text-sm ${
                    message.type === "success"
                      ? "bg-feedback-success-surface text-feedback-success-foreground border border-feedback-success-border"
                      : "bg-feedback-danger-surface text-feedback-danger-foreground border border-feedback-danger-border"
                  }`}
                >
                  {message.text}
                </div>
              ...
              <div className="flex flex-wrap items-center justify-between gap-3 pt-2">
                <div className="flex items-center gap-2">
                  ...
                  {status?.hasRedRouter && (
                    <Button
                      ...
                      className="text-feedback-danger-foreground hover:text-feedback-danger-foreground hover:border-feedback-danger-border"
                    >
                ...
                <Button
                  ...
                  onClick={() => setShowManualConfigModal(true)}
                >
                  <Icon name="code" size={18} className="mr-1" />
                  Manual Config
                </Button>
              </div>
            </>
