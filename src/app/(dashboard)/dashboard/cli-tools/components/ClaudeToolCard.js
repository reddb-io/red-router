...
import { Card, Button, ModelSelectModal, ManualConfigModal, Tooltip, Icon } from "@/shared/components";
...
export default function ClaudeToolCard({
  ...
  const handleApplySettings = async () => {
    ...
    try {
      ...
      // Get key from dropdown, fallback to first key or sk_red-router for localhost
...
>>>>>>> feat/ds-v2026.09
                  <div className="flex-1">
                    <p className="font-medium text-feedback-warning-foreground">Claude CLI not detected locally</p>
                ...
                <div className="p-4 bg-surface border border-border rounded-lg">
                  ...
                  <div className="space-y-3 text-sm">
                    <div>
                      ...
                      <code className="block px-3 py-2 bg-muted/50 rounded font-mono text-xs">npm install -g @anthropic-ai/claude-code</code>
                    </div>
                    <p className="text-text-muted">After installation, run <code className="px-1 bg-muted/50 rounded">claude</code> to verify.</p>
                  </div>
                </div>
              )}
            </div>
...
<<<<<<< HEAD
                      {modelMappings[model.alias] && <button onClick={() => onModelMappingChange(model.alias, "")} className="absolute right-1 top-1/2 -translate-y-1/2 p-0.5 text-text-muted hover:text-feedback-danger-foreground rounded transition-colors" title="Clear"><Icon name="close" size={14} /></button>}
||||||| e6e8d110
                      {modelMappings[model.alias] && <button onClick={() => onModelMappingChange(model.alias, "")} className="absolute right-1 top-1/2 -translate-y-1/2 p-0.5 text-text-muted hover:text-[var(--reddb-color-feedback-danger-foreground)] rounded transition-colors" title="Clear"><span className="material-symbols-outlined text-[14px]">close</span></button>}
=======
                      {modelMappings[model.alias] && <button onClick={() => onModelMappingChange(model.alias, "")} className="absolute right-1 top-1/2 -translate-y-1/2 p-0.5 text-text-muted hover:text-feedback-danger-foreground rounded transition-colors" title="Clear"><span className="material-symbols-outlined text-[14px]">close</span></button>}
>>>>>>> feat/ds-v2026.09
                    </div>
...
>>>>>>> feat/ds-v2026.09
              ...
              <div className="grid grid-cols-1 gap-2 sm:flex sm:items-center">
                ...
                <Button variant="outline" size="sm" onClick={handleResetSettings} disabled={!claudeStatus?.hasRedRouter} loading={restoring}>
