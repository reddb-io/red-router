...
import { getRequestIdentity, getScopeFilter, resolveDefaultOwner, scopeVisible } from "@/lib/auth/resourceScope";
...
async function resolvePresetItems(source) {
  const [filter, identity, targetOwner] = await Promise.all([
    getScopeFilter(),
    getRequestIdentity(),
    resolveDefaultOwner(),
  ]);
  const hidden = new Set(identity.isAdmin || !identity.owner ? [] : await getHiddenComboNames(identity.owner));
  const combos = scopeVisible(await getCombos(), filter).filter((combo) => {
    const owner = combo.owner ?? null;
    if (owner === null && hidden.has(combo.name)) return false;
    return owner === null || owner === (targetOwner ?? null);
  });
  ...
  try {
    ...
    for (const item of items) {
      ...
      const combo = await createCombo({
        name: item.name,
        models: item.models,
        owner: await resolveDefaultOwner(),
      });
      created.push(combo);
    }
