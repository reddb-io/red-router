...
const hasRedRouterConfig = (config) => {
  ...
  return !!config.provider["red-router"];
};
...
export async function GET() {
  try {
    ...
    const providerConfig = config?.provider?.["red-router"];
    ...
    return NextResponse.json({
      ...
      configPath: getConfigPath(),
        opencode: {
          ...
          activeModel: config?.model?.startsWith("red-router/") ? config.model.replace(/^red-router\//, "") : null,
...
// POST - Apply RedRouter as openai-compatible provider (multi-model support)
export async function POST(request) {
  try {
    ...
    const keyToUse = apiKey || "sk_red-router";
    const effectiveSubagentModel = subagentModel || modelsArray[0];

    // Ensure provider object
    if (!config.provider) config.provider = {};

    // Preserve any existing red-router provider entry and its models
    const existingProvider = config.provider["red-router"] || { npm: "@ai-sdk/openai-compatible", options: {}, models: {} };
    ...
    config.provider["red-router"] = existingProvider;
    ...
    if (activeModel === "") {
      ...
      if (finalActive) {
        config.model = `red-router/${finalActive}`;
      }
    }
    ...
    config.agent.explorer = {
      ...
      mode: "subagent",
      model: `red-router/${effectiveSubagentModel}`,
    };
...
export async function PATCH(request) {
  try {
    ...
    if (clearActiveModel === true) {
      ...
      if (config.model?.startsWith("red-router/")) {
        config.model = "";
      }
    }
...
// DELETE - Remove RedRouter provider or specific models from config
export async function DELETE(request) {
  try {
    ...
    if (modelToRemove && config.provider?.["red-router"]?.models) {
      delete config.provider["red-router"].models[modelToRemove];
      
      // If no models left, remove the provider
      if (Object.keys(config.provider["red-router"].models).length === 0) {
        delete config.provider["red-router"];
        if (config.model?.startsWith("red-router/")) delete config.model;
      } else if (config.model === `red-router/${modelToRemove}`) {
        // If removed model was active, switch to first remaining model
        const remainingModels = Object.keys(config.provider["red-router"].models);
        config.model = `red-router/${remainingModels[0]}`;
      }
    } else {
      // No specific model - remove entire red-router provider
      if (config.provider) delete config.provider["red-router"];
      if (config.model?.startsWith("red-router/")) delete config.model;
    }

    // Remove subagent configuration
    if (config.agent?.explorer?.model?.startsWith("red-router/")) {
    ...
    return NextResponse.json({
      success: true,
      message: modelToRemove ? `Model "${modelToRemove}" removed` : "RedRouter settings removed from OpenCode",
    });
  } catch (error) {
