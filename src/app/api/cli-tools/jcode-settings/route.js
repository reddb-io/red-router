...
const getProviderEnvPath = () => {
  ...
  return path.join(configDir, "jcode", "provider-red-router.env");
};
...
export async function GET() {
  ...
  const hasRedRouter = hasRedRouterConfig(config);
...
export async function POST(request) {
  try {
    ...
    env.JCODE_RED_ROUTER_API_KEY = apiKey;
    await writeProviderEnv(env);

    return NextResponse.json({
      success: true,
      message: "jcode configured successfully. Use: jcode --provider-profile red-router",
      configPath: getConfigPath(),
    });
  } catch (error) {
...
export async function DELETE() {
  try {
    ...
    delete config.providers["red-router"];

    await writeConfig(config);

    const env = await readProviderEnv();
    delete env.JCODE_RED_ROUTER_API_KEY;
    await writeProviderEnv(env);

    return NextResponse.json({
      success: true,
      message: "red-router configuration removed from jcode",
    });
  } catch (error) {
