...
// Check if settings has RedRouter config
const hasRedRouterConfig = (settings) => {
  if (!settings || !settings.models || !settings.models.providers) return false;
  return !!settings.models.providers["red-router"];
};

// Read per-agent models.json and return current model id (without "red-router/" prefix)
const readAgentModel = async (agentDir) => {
  try {
    const modelsPath = path.join(agentDir, "models.json");
    const content = await fs.readFile(modelsPath, "utf-8");
    const data = JSON.parse(content);
    const models = data?.providers?.["red-router"]?.models;
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      hasRedRouter: hasRedRouterConfig(settings),
...
// POST - Update RedRouter settings (merge with existing settings)
export async function POST(request) {
  try {
    ...
    const fullModelId = `red-router/${model}`;

    // Remove all old red-router/* entries from agents.defaults.models
    Object.keys(settings.agents.defaults.models)
      .filter((k) => k.startsWith("red-router/"))
    ...
    // Add fresh red-router models to allowlist
    allModelIds.forEach((m) => {
      settings.agents.defaults.models[`red-router/${m}`] = {};
    });

    // Remove old red-router model from each agent in agents.list. The
    ...
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        if (resolveAgentModel(agent.model).startsWith("red-router/")) {
    ...
    // Update models.providers.red-router with all models
    settings.models.providers["red-router"] = {
    ...
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        ...
        if (agentModel) return { ...agent, model: `red-router/${agentModel}` };
        return agent;
      });
...
export async function DELETE() {
  try {
    ...
    // Remove RedRouter from models.providers
    if (settings.models && settings.models.providers) {
      delete settings.models.providers["red-router"];
    ...
    // Remove red-router models from agents.defaults.models allowlist
    if (settings.agents?.defaults?.models) {
      const keysToRemove = Object.keys(settings.agents.defaults.models).filter((k) => k.startsWith("red-router/"));
    ...
    // Reset agents.defaults.model.primary if it uses red-router
    if (settings.agents?.defaults?.model?.primary?.startsWith("red-router/")) {
