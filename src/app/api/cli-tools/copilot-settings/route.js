...
const hasRedRouterConfig = (config) => {
  ...
  return config.some((entry) => entry.name === "RedRouter");
};

const getRedRouterEntry = (config) => {
  if (!Array.isArray(config)) return null;
  return config.find((entry) => entry.name === "RedRouter") || null;
};
...
export async function GET() {
  try {
    ...
    const entry = getRedRouterEntry(config);
...
// POST - Apply RedRouter config to chatLanguageModels.json
export async function POST(request) {
  try {
    ...
    // Replace existing RedRouter entry or append
    const idx = config.findIndex((e) => e.name === "RedRouter");
    if (idx >= 0) {
      config[idx] = newEntry;
    } else {
...
// DELETE - Remove RedRouter entry from chatLanguageModels.json
export async function DELETE() {
  try {
    ...
    config = config.filter((e) => e.name !== "RedRouter");
    await fs.writeFile(configPath, JSON.stringify(config, null, 2));

    return NextResponse.json({
      success: true,
      message: "RedRouter removed from Copilot config",
    });
  } catch (error) {
