...
// Check if settings has RedRouter customModels
const hasRedRouterConfig = (settings) => {
  if (!settings || !settings.customModels) return false;
  return settings.customModels.some(m => m.id?.startsWith("custom:RedRouter"));
};
...
// POST - Update RedRouter customModels (merge with existing settings)
...
export async function POST(request) {
  try {
    ...
    // Remove all existing RedRouter configs
    settings.customModels = settings.customModels.filter(m => !m.id?.startsWith("custom:RedRouter"));
...
// DELETE - Remove RedRouter customModels only (keep other settings)
