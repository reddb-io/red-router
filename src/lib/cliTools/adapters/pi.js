...
const hasRedRouterConfig = (settings) => {
  ...
  const p = settings.providers["red-router"];
  if (p && p.baseUrl) return true;
  for (const prov of Object.values(settings.providers)) {
    if (prov.baseUrl && prov.baseUrl.includes("25050")) return true;
  }
  return false;
};
...
export async function POST(request) {
  ...
  try {
    ...
    return NextResponse.json({
      success: true,
      message: "Pi settings applied! Use /model in Pi to select the RedRouter model.",
      configPath,
    });
  } catch (err) {
    return NextResponse.json({ error: { message: err.message } }, { status: 500 });
...
export async function DELETE() {
  try {
    ...
    if (existing.providers && existing.providers["red-router"]) {
      delete existing.providers["red-router"];
      if (Object.keys(existing.providers).length === 0) delete existing.providers;
      await fs.writeFile(configPath, JSON.stringify(existing, null, 2), "utf-8");
    }

    return NextResponse.json({ success: true, message: "RedRouter removed from Pi" });
  } catch (err) {
    return NextResponse.json({ error: { message: err.message } }, { status: 500 });
  }
}
