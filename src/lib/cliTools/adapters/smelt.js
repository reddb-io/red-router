...
const hasRedRouterConfig = (settings) => {
  ...
  return (
    settings._managedBy === "red-router" ||
    (typeof settings.baseUrl === "string" && settings.baseUrl.length > 0 && settings.baseUrl.includes("25050"))
  );
};
...
export async function DELETE() {
  try {
    ...
    return NextResponse.json({ success: true, message: "Smelt RedRouter settings removed" });
  } catch (err) {
    return NextResponse.json({ error: { message: err.message } }, { status: 500 });
  }
}
