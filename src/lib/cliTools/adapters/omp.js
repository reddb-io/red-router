...
const hasRedRouterInYml = (content) => {
  if (!content) return false;
  return content.includes("red-router:") || content.includes("localhost:25050");
};

// Build standard RedRouter provider block for models.yml
const buildOmpProviderYaml = (baseUrl, apiKey) => {
  const normalizedBaseUrl = baseUrl.endsWith("/v1") ? baseUrl : `${baseUrl}/v1`;
  const key = apiKey || "sk_red-router";
  return `  ${PROVIDER_ID}:
...
export async function GET() {
  try {
    ...
    const hasRedRouter = hasRedRouterInYml(ymlContent);
...
export async function POST(request) {
  ...
  try {
    ...
    // Remove existing red-router provider if present
    ...
    try {
      ...
      if (Database) {
        ...
        db.prepare(
          ...
          PROVIDER_ID,
          "api_key",
          JSON.stringify({ apiKey: apiKey || "sk_red-router", baseUrl }),
    ...
    return NextResponse.json({
      success: true,
      message: "Oh My Pi settings applied! Run 'omp' and all RedRouter models appear under red-router in /model.",
      configPath: getOmpModelsYmlPath(),
    });
  } catch (err) {
    return NextResponse.json({ error: { message: err.message } }, { status: 500 });
...
export async function DELETE() {
  try {
    ...
    return NextResponse.json({
      success: true,
      message: "RedRouter removed from Oh My Pi",
    });
  } catch (err) {
    return NextResponse.json({ error: { message: err.message } }, { status: 500 });
  }
