// x-rr-real-ip is only trustworthy when custom-server.js stamped it from the TCP socket.
...
export function hasTrustedPeerHeaders(request) {
  const token = process.env.REDROUTER_PEER_TOKEN;
  return Boolean(token) && request.headers.get("x-rr-peer-token") === token;
}
