...
// Collect PIDs of all red-router-related processes (excluding current)
  ...
  if (platform === "win32") {
    try {
      ...
      lines.forEach(line => {
        ...
        // Match anything running from red-router install dir or wrapper cli.js
        const isAppProcess = lower.includes("red-router") ||
    ...
    try {
      ...
      output.split("\n").forEach(line => {
        const isAppProcess = line.includes("red-router") ||
...
// Resolve npx/red-router binary to relaunch after update (cross-platform)
function resolveRelaunchCommand() {
  const isWin = process.platform === "win32";
  // Prefer `npx red-router` — works regardless of global bin path changes after npm i -g
