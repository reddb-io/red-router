...
export function applyOutboundProxyEnv(
  ...
  if (!enabled) {
    if (process.env.RED_ROUTER_PROXY_MANAGED === "1") {
  ...
  const wasManaged = process.env.RED_ROUTER_PROXY_MANAGED === "1";
  let managed = false;
  ...
  if (proxyUrl) {
    ...
    if (validated) {
      ...
      process.env.RED_ROUTER_PROXY_URL = validated;
      managed = true;
    }
  }
  ...
  if (noProxy) {
    ...
    process.env.RED_ROUTER_NO_PROXY = noProxy;
    managed = true;
  }

  if (managed) {
    process.env.RED_ROUTER_PROXY_MANAGED = "1";
  } else if (wasManaged) {
