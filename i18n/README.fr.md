<div align="center">
  <img src="../images/red-router.png?1" alt="Tableau de bord RedRouter" width="800"/>
  
  # RedRouter - Routeur IA GRATUIT et économiseur de tokens
...
[🚀 Démarrage rapide](#-démarrage-rapide) • [💡 Fonctionnalités](#-fonctionnalités-clés) • [📖 Configuration](#-guide-dinstallation) • [🌐 Site web](https://github.com/reddb-io/red-router)
...
> 🎬 **Vous avez réalisé une vidéo sur RedRouter ?** Envoyez une [Pull Request](https://github.com/reddb-io/red-router/pulls) en ajoutant votre vidéo à cette section — nous la fusionnerons !

---

## 🛠️ Outils CLI pris en charge

RedRouter fonctionne parfaitement avec tous les principaux outils de codage IA :
...
Headroom est optionnel et s'exécute séparément. RedRouter appelle l'endpoint local `/v1/compress` de Headroom, puis conserve le routage, le repli, l'authentification et le suivi d'utilisation normaux :

```
Client → RedRouter → Headroom /v1/compress → RedRouter → fournisseur
```
...
- Votre outil CLI envoie le format OpenAI → RedRouter le traduit → Le fournisseur reçoit le format natif
...
> RedRouter lui-même **ne vous facture jamais** quoi que ce soit. Vous ne payez que les fournisseurs directement (si vous utilisez des services payants).
>
...
### 📊 Comprendre les coûts et la facturation de RedRouter

**La réalité de la facturation de RedRouter :**

✅ **Le logiciel RedRouter = GRATUIT pour toujours** (open source, ne facture jamais)  
...
❌ **RedRouter n'envoie jamais de factures** et ne débite pas votre carte
...
- **Fournisseurs bon marché** (GLM, MiniMax) : Payez-les directement, RedRouter se contente de router
- **Fournisseurs GRATUITS** (iFlow, Kiro, Qwen) : Vraiment gratuits pour toujours, sans frais cachés
- **RedRouter** : Ne facture jamais rien, jamais
...
<details>
<summary><b>💳 RedRouter me facturera-t-il ?</b></summary>

**Non.** RedRouter est un logiciel gratuit et open source qui s'exécute sur votre propre ordinateur. Il ne vous facture jamais rien.

**Vous ne payez que :**

- ✅ **Fournisseurs par abonnement** (Claude Code 20 $/mois, Codex 20 à 200 $/mois) → Payez-les directement sur leurs sites web
- ✅ **Fournisseurs bon marché** (GLM, MiniMax) → Payez-les directement, RedRouter route simplement vos requêtes
- ❌ **RedRouter lui-même** → **Ne facture jamais rien, jamais**

RedRouter est un proxy/routeur local. Il n'a pas votre carte bancaire, ne peut pas envoyer de factures et ne possède aucun système de facturation. C'est un logiciel entièrement gratuit.
...
RedRouter se contente de router vos requêtes vers eux — il n'y a pas de "piège" ni de facturation future. Ce sont de véritables services gratuits, et RedRouter les rend faciles à utiliser avec le support du repli.
...
3. **Utilisez les fournisseurs par abonnement en dernier :**
   ...
   - RedRouter vous aide à maximiser leur valeur grâce au suivi de quota
...
Le repli intelligent de RedRouter évite les frais surprises :
...
**Vous gardez le contrôle :** Définissez des limites de dépenses par fournisseur dans le tableau de bord, et RedRouter les respecte.
...
**Astuce pro :** Utilisez Opus pour les tâches complexes et Sonnet pour la vitesse. RedRouter suit le quota par modèle !
...
Settings → Models → Advanced :
  ...
  OpenAI API Key : [depuis le tableau de bord red-router]
...
```json
{
  ...
  "anthropic_api_key": "votre-clé-api-red-router"
}
```
...
export OPENAI_API_KEY="votre-clé-api-red-router"
...
> **Remarque :** OpenClaw ne fonctionne qu'avec un RedRouter local. Utilisez `127.0.0.1` au lieu de `localhost` pour éviter les problèmes de résolution IPv6.
...
| ---------------------------------------------------- | ------------------------------------------- | -------------------------------------------------------------------------------------------- |
| `JWT_SECRET`                                         | Généré automatiquement (`~/.red-router/jwt-secret`) | Secret de signature JWT pour le cookie d'authentification du tableau de bord (remplacez-le pour partager entre instances) |
...
| `NEXT_PUBLIC_CLOUD_URL`                              | `https://github.com/reddb-io/red-router`                       | URL cloud publique/rétrocompatible (préférez `CLOUD_URL` pour le runtime serveur)             |
...
- Renouvelé automatiquement par RedRouter
...
- **Site web** : [github.com/reddb-io/red-router](https://github.com/reddb-io/red-router)
- **GitHub** : [github.com/reddb-io/red-router](https://github.com/reddb-io/red-router)
- **Problèmes** : [github.com/reddb-io/red-router/issues](https://github.com/reddb-io/red-router/issues)

---

## 👥 Contributeurs

Merci à tous les contributeurs qui ont contribué à améliorer RedRouter !
...
**[OmniRoute](https://github.com/diegosouzapw/OmniRoute)** — Un fork TypeScript complet de RedRouter. Ajoute plus de 36 fournisseurs, un repli automatique à 4 niveaux, des API multimodales (images, embeddings, audio, TTS), un disjoncteur, un cache sémantique, des évaluations de LLM et un tableau de bord soigné. Plus de 368 tests unitaires. Disponible via npm et Docker.
...
- **[RTK](https://github.com/rtk-ai/rtk)** ![Stars](https://img.shields.io/github/stars/rtk-ai/rtk?style=flat&color=yellow) — économiseur de tokens en Rust. RedRouter porte son pipeline de compression en JS → **−20 à 40 % de tokens d'entrée** sur chaque requête.
- **[Caveman](https://github.com/JuliusBrussee/caveman)** ![Stars](https://img.shields.io/github/stars/JuliusBrussee/caveman?style=flat&color=yellow) par **[@JuliusBrussee](https://github.com/JuliusBrussee)** — viral _"why use many token when few token do trick"_. RedRouter adapte son prompt → **−65 % de tokens de sortie**.
- **[Ponytail](https://github.com/DietrichGebert/ponytail)** ![Stars](https://img.shields.io/github/stars/DietrichGebert/ponytail?style=flat&color=yellow) par **[@DietrichGebert](https://github.com/DietrichGebert)** — compétence de _"dev sénior fainéant"_. RedRouter injecte sa hiérarchie YAGNI-d'abord → **moins de tokens, moins de code, des diffs plus courts**.

Un immense merci à ces auteurs — sans leur travail, les fonctionnalités d'économie de tokens de RedRouter n'existeraient pas. Mettez-leur une ⭐ sur GitHub !
