...
Tích hợp RedRouter với extension Cline VSCode để định tuyến request AI qua hệ thống routing thông minh của RedRouter.

## Yêu cầu

- Visual Studio Code đã cài đặt
- Extension Cline đã cài đặt từ VSCode marketplace
- RedRouter đang chạy cục bộ hoặc cloud endpoint đã cấu hình
...
Đặt base URL tới endpoint RedRouter:
...
**Các bước:**
1. Trong field **Base URL**, nhập endpoint RedRouter
2. Đảm bảo bao gồm `/v1` ở cuối

### 4. Thêm API Key

1. Trong field **API Key**, nhập API key RedRouter của bạn
2. Bạn có thể tìm API key trong RedRouter dashboard tại **Settings → API Keys**
3. Key bắt đầu bằng `sk-red-router-`

### 5. Chọn Model

1. Trong dropdown **Model**, bạn có thể:
   - Chọn từ model có sẵn (nếu Cline auto-detect)
   - Nhập tên model thủ công từ cấu hình RedRouter
...
3. Nhấn Enter để gửi
4. Cline sẽ dùng RedRouter để xử lý request

### Tạo Code

1. Yêu cầu Cline tạo code: "Create a React component for a login form"
2. Cline sẽ tạo code qua RedRouter
...
3. Nhận giải thích AI qua RedRouter

### Thao tác File

1. Yêu cầu Cline tạo, sửa hoặc xóa files
2. Cline sẽ dùng RedRouter để hiểu context và thực hiện thay đổi
...
2. Xác minh RedRouter instance đang chạy và healthy
3. Thử reload cửa sổ VSCode (Cmd/Ctrl + Shift + P → "Reload Window")
4. Kiểm tra logs RedRouter để xem lỗi

## Cấu hình Nâng cao

### Dùng Cloud Endpoint

Để dùng RedRouter cloud endpoint thay vì localhost:

1. Trong Cline settings, đặt Base URL: `https://github.com/reddb-io/red-router`
2. Đảm bảo bạn đã cấu hình API key trong RedRouter cloud dashboard
...
## Tích hợp với Tính năng RedRouter

### Định tuyến Model

RedRouter tự động định tuyến request đến provider tốt nhất hiện có dựa trên:
...
Nếu một provider thất bại, RedRouter tự động fallback sang provider khác đã cấu hình trong dashboard.

### Theo dõi Usage

Giám sát usage Cline qua RedRouter dashboard:
- Tổng request
- Token usage
- Chi phí mỗi model
- Phân bổ provider
