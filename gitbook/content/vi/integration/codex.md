...
Tích hợp RedRouter với OpenAI Codex CLI để định tuyến request API OpenAI qua hệ thống routing thông minh của RedRouter.
...
RedRouter cung cấp các model Codex sau:
...
1. Xác minh API key đúng trong RedRouter dashboard
...
1. Xác minh tên model khớp với cấu hình RedRouter
2. Kiểm tra kết nối provider OpenAI đang hoạt động trong RedRouter dashboard
...
Đảm bảo bạn đã cấu hình API key trong RedRouter cloud dashboard.
