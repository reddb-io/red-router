...
RedRouter es compatible con cualquier herramienta que soporte el formato de API de OpenAI. Esta guía cubre patrones de integración genéricos para varias herramientas y aplicaciones personalizadas.

## Resumen

RedRouter proporciona un endpoint de API compatible con OpenAI que funciona con:
...
Cualquier herramienta compatible con OpenAI puede conectarse a RedRouter usando estas configuraciones:

**RedRouter local:**
```
Base URL: http://localhost:25050/v1
API Key: your-api-key-from-dashboard
Model: cualquier modelo de RedRouter (cc/*, cx/*, glm/*, etc.)
```

**RedRouter en la nube:**
```
Base URL: https://github.com/reddb-io/red-router/v1
API Key: your-api-key-from-dashboard
Model: cualquier modelo de RedRouter (cc/*, cx/*, glm/*, etc.)
```
...
client = OpenAI(
    ...
    base_url="http://localhost:25050/v1"
)
...
**Solicitud:**
```
POST http://localhost:25050/v1/chat/completions
```
...
llm = ChatOpenAI(
    ...
    openai_api_base="http://localhost:25050/v1",
    temperature=0.7
)
...
llm = OpenAI(
    model="cc/claude-sonnet-4-20250514",
    api_key="your-api-key-from-dashboard",
    api_base="http://localhost:25050/v1"
)
...
openai.api_base = "http://localhost:25050/v1"
...
ROUTER_BASE_URL=http://localhost:25050/v1
...
**Problema:** No se puede conectar a RedRouter
```bash
# Verifica si RedRouter está corriendo
...
**Solución:**
- Verifica que RedRouter esté corriendo
- Verifica que el puerto 25050 no esté bloqueado
...
- Verifica los modelos disponibles: `curl http://localhost:25050/v1/models`
...
- Verifica la conexión de red a RedRouter
