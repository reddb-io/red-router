...
RedRouter is compatible with any tool that supports the OpenAI API format. This guide covers generic integration patterns for various tools and custom applications.

## Overview

RedRouter provides an OpenAI-compatible API endpoint that works with:
...
Any OpenAI-compatible tool can connect to RedRouter using these settings:
...
**Problem:** Cannot connect to RedRouter
```bash
...
**Solution:**
- Verify RedRouter is running
- Check port 25050 is not blocked
...
- Check available models: `curl http://localhost:25050/v1/models`
...
- Check network connection to RedRouter
