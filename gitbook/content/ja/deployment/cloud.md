...
リモートアクセスと本番利用のため、VPSまたはDockerにRedRouterをデプロイ。
...
| `DATA_DIR` | `~/.red-router` | データベースとデータの保存パス |
...
`/etc/nginx/sites-available/red-router` を作成:
...
# リバースプロキシを使用しない場合、RedRouterポートを許可
