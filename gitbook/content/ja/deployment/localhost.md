...
開発と個人利用のため、ローカルマシンでRedRouterを実行。

---

## 📦 インストール

npm経由でRedRouterをグローバルインストール:
...
- **APIエンドポイント**: `http://localhost:25050/v1`
- **データディレクトリ**: `~/.red-router`
...
APIポート(25050)とダッシュボードポート(3000)はアプリケーションで設定されています。変更するにはソースコードを修正するか、サポートされている場合は環境変数を使用してください。

---

## 🛑 サーバーの停止

RedRouterが実行されているターミナルで `Ctrl+C` を押します。

```bash
# red-routerを実行しているターミナル
^C  # Ctrl+Cを押す
```
