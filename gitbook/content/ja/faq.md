...
**RedRouterは、サブスクリプションの価値を最大化し、コストを最小限に抑えるAIモデルルーターです。**
...
**はい、RedRouter自体は100%無料でオープンソースです。**
...
**RedRouterはすべてのプロバイダーのクォータをリアルタイムで追跡します:**
...
## RedRouterをセルフホストできますか?

**はい! RedRouterは複数のデプロイメントオプションをサポートします:**
...
- `DATA_DIR` - データベース保存パス(デフォルト: `~/.red-router`)
...
**はい、RedRouterはセキュリティとプライバシーを優先します:**

**ローカルストレージ:**
- すべてのデータは`~/.red-router`(またはカスタム`DATA_DIR`)にローカル保存
- RedRouterサーバーへのデータ送信なし
- OAuthトークンはJWTで暗号化
...
**破壊的変更:**
- [CHANGELOG.md](https://github.com/reddb-io/red-router/blob/main/CHANGELOG.md)を確認
- メジャー更新前に`~/.red-router`をバックアップ
- メジャーバージョンの移行ガイドを確認
...
詳細は[CONTRIBUTING.md](https://github.com/reddb-io/red-router/blob/main/CONTRIBUTING.md)を参照。

---

## さらにヘルプが必要?

- **ドキュメント:** [github.com/reddb-io/red-router/docs](https://github.com/reddb-io/red-router/docs)
