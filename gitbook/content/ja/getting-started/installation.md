...
トラブルシューティングのヒント付きのRedRouter詳細インストールガイド。
...
どこからでも使用できるようにRedRouterをグローバルインストール:
...
**利点:**
- ✅ どのディレクトリからでも実行
- ✅ シンプルなコマンド: `red-router`
- ✅ `npm update -g red-router` で自動更新
...
**何が起こるか:**
1. サーバーが `http://localhost:25050` で起動
2. ダッシュボードが自動的にブラウザで開く
3. `~/.red-router` にデータディレクトリが作成される
4. APIキーが自動生成される
